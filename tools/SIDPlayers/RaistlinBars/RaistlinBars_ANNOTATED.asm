...
#if D400_SHADOW && D401_SHADOW && D404_SHADOW && D406_SHADOW && D407_SHADOW && D408_SHADOW && D40B_SHADOW && D40D_SHADOW && D40E_SHADOW && D40F_SHADOW && D412_SHADOW && D414_SHADOW {	#define USE_INBUILT_SID_REGISTER_MIRRORING	.print "Using inbuilt SID register mirrors!" }#else {	#define USE_MANUAL_SID_REGISTER_MIRRORING }#endif* = PlayerADDR	jmp Initialize					//; Entry point for the player//; =============================================================================//; CONFIGURATION CONSTANTS//; =============================================================================.const NUM_FREQUENCY_BARS = 40//; Display layout.const TOP_SPECTRUM_HEIGHT = 16.const BOTTOM_SPECTRUM_HEIGHT = 3.const SONG_TITLE_LINE = 0.const ARTIST_NAME_LINE = 23.const SPECTRUM_START_LINE = 3.const REFLECTION_SPRITES_YVAL = 50 + (SPECTRUM_START_LINE + TOP_SPECTRUM_HEIGHT) * 8 + 3.eval setSeed(55378008)//; Memory configuration.const VIC_BANK = 3						//; $C000-$FFFF.const VIC_BANK_ADDRESS = VIC_BANK * $4000.const SCREEN_0_OFFSET = 12				//; $F000.const SCREEN_1_OFFSET = 13				//; $F400.const CHARSET_OFFSET = 7				//; $F800.const SPRITE_BASE_INDEX = $80//; Calculated addresses.const SCREEN_0_ADDRESS = VIC_BANK_ADDRESS + (SCREEN_0_OFFSET * $400).const SCREEN_1_ADDRESS = VIC_BANK_ADDRESS + (SCREEN_1_OFFSET * $400).const CHARSET_ADDRESS = VIC_BANK_ADDRESS + (CHARSET_OFFSET * $800).const SPRITES_ADDRESS = VIC_BANK_ADDRESS + (SPRITE_BASE_INDEX * $40).const SPRITE_POINTERS_0 = SCREEN_0_ADDRESS + $3F8.const SPRITE_POINTERS_1 = SCREEN_1_ADDRESS + $3F8//; VIC register values.const D018_VALUE_0 = (SCREEN_0_OFFSET * 16) + (CHARSET_OFFSET * 2).const D018_VALUE_1 = (SCREEN_1_OFFSET * 16) + (CHARSET_OFFSET * 2)//; Calculated bar values.const MAX_BAR_HEIGHT = TOP_SPECTRUM_HEIGHT * 8 - 1.const WATER_REFLECTION_HEIGHT = BOTTOM_SPECTRUM_HEIGHT * 8.const MAIN_BAR_OFFSET = MAX_BAR_HEIGHT - 8.const REFLECTION_OFFSET = WATER_REFLECTION_HEIGHT - 7//; Color palette configuration.const NUM_COLOR_PALETTES = 3.const COLORS_PER_PALETTE = 8//; =============================================================================//; EXTERNAL RESOURCES//; =============================================================================.var file_charsetData = LoadBinary("CharSet.map").var file_waterSpritesData = LoadBinary("WaterSprites.map")//; Song metadata.var SONG_TITLE_LENGTH = min(SIDName.size(), 40).var ARTIST_NAME_LENGTH = min(SIDAuthor.size(), 40)//; =============================================================================//; INITIALIZATION//; =============================================================================Initialize: {	sei  ; Set Interrupt Disable
...
!loop:	sta barHeights - 2, y  ; Store Accumulator
	...
	bit $d011  ; Voice 1 Frequency High
	bmi *-3  ; Branch if Minus
	//; Enable display	lda #$1b  ; Load Accumulator
	sta $d011  ; Voice 1 Frequency High
...
}//; =============================================================================//; Macros//; =============================================================================.macro CallSubroutinesButAvoidCallingThemOnTopOfThemselves(list_of_subroutines_to_call) {are_the_subroutines_still_running_lda: lda #0    bne !+  ; Branch if Not Equal
    // Protect against us being invoked again on top of our own invocation    lda #1  ; Load Accumulator
    sta are_the_subroutines_still_running_lda + 1  ; Store Accumulator
    // Allow interrupts to occur on top of the subroutines we now call    cli  ; Clear Interrupt Disable
    .for (var i = 0; i < list_of_subroutines_to_call.size(); i++) {        jsr list_of_subroutines_to_call.get(i)  ; Jump to Subroutine
    }    lda #0  ; Load Accumulator
    sta are_the_subroutines_still_running_lda + 1  ; Store Accumulator
!:}//; =============================================================================//; CODE SEGMENT//; VIC INITIALIZATION//; =============================================================================.const SKIP_REGISTER = $e1InitializeVIC: {	//; Apply VIC register configuration	ldx #VICConfigEnd - VICConfigStart - 1  ; Load X Register
...
!skip:	dex  ; Decrement X
	...
	//; Setup raster interrupt	lda D012_Values  ; Load Accumulator
	sta $d012  ; Voice 1 Frequency High
	lda $d011  ; Voice 1 Frequency High
	and #$7f  ; Logical AND
	ora D011_Values  ; Logical OR
	sta $d011  ; Voice 1 Frequency High
	//; Enable raster interrupts	lda #$01  ; Load Accumulator
	sta $d01a  ; Voice 1 Frequency High
	sta $d019  ; Voice 1 Frequency High
	//; Initialize IRQ counter	lda #$00  ; Load Accumulator
	sta NextIRQ + 1  ; Store Accumulator
	rts  ; Return from Subroutine
}//; =============================================================================//; MAIN INTERRUPT HANDLER//; =============================================================================MainIRQ: {	pha	txa  ; Transfer X to A
	pha	tya  ; Transfer Y to A
	pha	lda $01  ; Load Accumulator
	pha	lda #$35  ; Load Accumulator
	sta $01  ; Store Accumulator
	//; Update display if needed	ldy currentScreenBuffer  ; Load Y Register
	lda D018Values, y  ; Load Accumulator
	cmp $d018  ; Voice 1 Frequency High
	beq !skip+  ; Branch if Equal
	sta $d018  ; Voice 1 Frequency High
!skip:	//; Signal visualization update	inc visualizationUpdateFlag  ; Increment Memory
	//; Frame counter	inc frameCounter  ; Increment Memory
	bne !skip+  ; Branch if Not Equal
	inc frame256Counter  ; Increment Memory
!skip:	//; Setup next interrupt	jsr NextIRQ  ; Jump to Subroutine
	//; Acknowledge interrupt	lda #$01  ; Load Accumulator
	sta $d019  ; Voice 1 Frequency High
	//; Update bar animations and play music and analyze	//; And do it with interrupts acknowledged, so another interrupt can happen while we do so	CallSubroutinesButAvoidCallingThemOnTopOfThemselves(List().add(PlayMusicWithAnalysis))	CallSubroutinesButAvoidCallingThemOnTopOfThemselves(List().add(UpdateBarDecay, UpdateColors, UpdateSprites))	pla	sta $01  ; Store Accumulator
	pla	tay  ; Transfer A to Y
	pla	tax  ; Transfer A to X
	pla	rti}//; =============================================================================//; MUSIC-ONLY INTERRUPT HANDLER//; =============================================================================MusicOnlyIRQ: {	pha	txa  ; Transfer X to A
	pha	tya  ; Transfer Y to A
	pha	lda $01  ; Load Accumulator
	pha	lda #$35  ; Load Accumulator
	sta $01  ; Store Accumulator
	jsr NextIRQ  ; Jump to Subroutine
	lda #$01  ; Load Accumulator
	sta $d019  ; Voice 1 Frequency High
    // Call the subroutines after acknowledging the VIC interrupt, so that subsequent interrupts can happen on top of the call. The CallSubroutinesButAvoidCallingThemOnTopOfThemselves() macro will ensure that we do not call the subroutines on top of themselves.	CallSubroutinesButAvoidCallingThemOnTopOfThemselves(List().add(PlayMusicWithAnalysis))    pla    sta $01  ; Store Accumulator
	pla	tay  ; Transfer A to Y
	pla	tax  ; Transfer A to X
	pla	rti}//; =============================================================================//; INTERRUPT CHAINING//; =============================================================================NextIRQ: {NextIRQLdx: ldx #$00						//; Self-modified	inx  ; Increment X
	cpx #NumCallsPerFrame  ; Compare with X
	bne !notLast+  ; Branch if Not Equal
	ldx #$00  ; Load X Register
!notLast:	stx NextIRQLdx + 1  ; Store X Register
	//; Set next raster position	lda D012_Values, x  ; Load Accumulator
	sta $d012  ; Voice 1 Frequency High
	lda $d011  ; Voice 1 Frequency High
	and #$7f  ; Logical AND
	ora D011_Values, x  ; Logical OR
	sta $d011  ; Voice 1 Frequency High
	//; Set appropriate IRQ vector	cpx #$00  ; Compare with X
	bne !musicOnly+  ; Branch if Not Equal
	lda #<MainIRQ  ; Load Accumulator
	sta $fffe  ; Store Accumulator
	lda #>MainIRQ  ; Load Accumulator
	sta $ffff  ; Store Accumulator
	rts  ; Return from Subroutine
!musicOnly:	lda #<MusicOnlyIRQ  ; Load Accumulator
	sta $fffe  ; Store Accumulator
	lda #>MusicOnlyIRQ  ; Load Accumulator
	sta $ffff  ; Store Accumulator
	rts  ; Return from Subroutine
}//; =============================================================================//; MUSIC PLAYBACK WITH ANALYSIS//; =============================================================================PlayMusicWithAnalysis: {	#if USE_MANUAL_SID_REGISTER_MIRRORING		//; First playback - normal music playing with state preservation		jsr BackupSIDMemory  ; Jump to Subroutine
...
!skip:	sty $d010  ; Voice 1 Frequency High
	...
	rts  ; Return from Subroutine
}//; =============================================================================//; UTILITY FUNCTIONS//; =============================================================================ClearScreens: {	ldx #$00  ; Load X Register
	lda #$20							//; Space character!loop:	sta SCREEN_0_ADDRESS + $000, x  ; Store Accumulator
	sta SCREEN_0_ADDRESS + $100, x  ; Store Accumulator
	sta SCREEN_0_ADDRESS + $200, x  ; Store Accumulator
	sta SCREEN_0_ADDRESS + $300, x  ; Store Accumulator
	sta SCREEN_1_ADDRESS + $000, x  ; Store Accumulator
	sta SCREEN_1_ADDRESS + $100, x  ; Store Accumulator
	sta SCREEN_1_ADDRESS + $200, x  ; Store Accumulator
	sta SCREEN_1_ADDRESS + $300, x  ; Store Accumulator
	sta $d800 + $000, x  ; Store Accumulator
	sta $d800 + $100, x  ; Store Accumulator
	sta $d800 + $200, x  ; Store Accumulator
	sta $d800 + $300, x  ; Store Accumulator
	inx  ; Increment X
	bne !loop-  ; Branch if Not Equal
	rts  ; Return from Subroutine
...
!loop:	lda #$01							//; White for title	sta $d800 + (SONG_TITLE_LINE * 40), x  ; Store Accumulator
	lda #$0f							//; Light gray for artist	sta $d800 + (ARTIST_NAME_LINE * 40), x  ; Store Accumulator
	dex  ; Decrement X
	bpl !loop-  ; Branch if Plus
...
!titleDone:	//; Display artist name	ldy #0  ; Load Y Register
!artistLoop:	lda artistName, y  ; Load Accumulator
	beq !artistDone+  ; Branch if Equal
	sta SCREEN_0_ADDRESS + (ARTIST_NAME_LINE * 40) + ((40 - ARTIST_NAME_LENGTH) / 2), y  ; Store Accumulator
	sta SCREEN_1_ADDRESS + (ARTIST_NAME_LINE * 40) + ((40 - ARTIST_NAME_LENGTH) / 2), y  ; Store Accumulator
	ora #$80							//; Reversed for second line	sta SCREEN_0_ADDRESS + ((ARTIST_NAME_LINE + 1) * 40) + ((40 - ARTIST_NAME_LENGTH) / 2), y  ; Store Accumulator
	sta SCREEN_1_ADDRESS + ((ARTIST_NAME_LINE + 1) * 40) + ((40 - ARTIST_NAME_LENGTH) / 2), y  ; Store Accumulator
	iny  ; Increment Y
	cpy #40  ; Compare with Y
	bne !artistLoop-  ; Branch if Not Equal
!artistDone:	rts  ; Return from Subroutine
