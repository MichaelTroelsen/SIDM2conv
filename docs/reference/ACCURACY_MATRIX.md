...
| **Blackbird / lft** (16-file v1.2-exact corpus) | `bin/build_blackbird_native_song.py` | Corpus **mean 99.67** overall, **8 of 16 at exactly 100.0**, none below 98.1. Note the denominator differs from every other row here: this is measured **against the validated Python simulator** (`bin/blackbird_everyframe_sim.py`), not directly against the original SID — the simulator is itself trace-validated, but that is one inference step further out. Glyptodont **162/162 note-ons** (0 missing, 0 spurious), hard-restart arming **100% on every file**. Laggards `To_Die_For_II` **98.2** (was 94.2 before E4; residual is now **entirely filter, 88.9** — its pulse/wf/adsr are all exactly 100.0) and `Into_the_Unknown` **98.1** (3 parts; wf/adsr already 100.0, so freq/filter) are **not** envelope problems. | ✅ [BLACKBIRD.md](../players/BLACKBIRD.md) — **not wired into `DriverSelector`**. Corpus sweep runs from **untracked scratch** (`scratchpad/sweep.py`) with no tracked test asserting it: **not reproducible from a fresh clone**. E3f combo fx values were disabled for a day on an SF2II-crash report that was later **falsified** by a 10-trial screenshot-verified editor play batch (`BB_NO_COMBO=1` disables); the crash originally reported by the user remains **unexplained** |
