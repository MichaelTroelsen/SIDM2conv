...
| **Blackbird / lft** (16-file v1.2-exact corpus) | `bin/build_blackbird_native_song.py` | Corpus **mean 99.84** overall, **10 of 16 at exactly 100.0**, and only ONE file (`Into_the_Unknown`, 98.1) below 99. Note the denominator differs from every other row here: this is measured **against the validated Python simulator** (`bin/blackbird_everyframe_sim.py`), not directly against the original SID — the simulator is itself trace-validated, but that is one inference step further out. Glyptodont **162/162 note-ons** (0 missing, 0 spurious), hard-restart arming **100% on every file**. `To_Die_For_II` reached **100.0** in one session (94.2 -> 98.2 via E4 -> 100.0 via E5) and `Revolutions_Delivered` **99.0 -> 100.0** with it. The sole remaining laggard is `Into_the_Unknown` **98.1** (3 parts; wf/adsr already 100.0, so freq/filter) -- **not** an envelope problem. | ✅ [BLACKBIRD.md](../players/BLACKBIRD.md) — **not wired into `DriverSelector`**. Corpus sweep is tracked and tested — `pyscript/blackbird_sweep.py` (+ `blackbird_crash_probe.py`, 34 tests in `test_blackbird_sweep.py` / `test_blackbird_crash_probe.py`): **reproducible from a fresh clone**, and the mean quoted here is asserted by a test. Regenerate with `py -3 pyscript/blackbird_sweep.py <label>`, diff with `--compare`. E3f combo fx values were disabled for a day on an SF2II-crash report that was later **falsified** by a 10-trial screenshot-verified editor play batch (`BB_NO_COMBO=1` disables); the crash originally reported by the user remains **unexplained** |
