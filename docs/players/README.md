...
| **HardTrack Composer** (Longhair/Brush, Elysium 1992) | [HARDTRACK.md](HARDTRACK.md) | `bin/` only (Stage A + Stage B), not wired | parser 88.4% / Stage A 90.6% on sequencer-pitch notes (Stage A > parser is an artifact, see matrix); 6 of the 8 parser-100% files are Stage A 100.0%. **Stage B** (native, trace-driven, no new driver): per-frame freq net of the driver's structural note-on frame — Love_tune_2 99.97/99.79/99.66% over the full 117 s | `SID/Shogoon/` (38 of 150 files; corpus is mixed-player) |
