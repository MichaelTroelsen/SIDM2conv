...
**Status:** native SF2 driver, **100% byte-exact on freq + waveform + pulse + filter, all 3 voices, full song length** for Hawkeye subtunes 2 & 3; Cybernoid, Cybernoid II, Myth, and Supremacy all build at ~95-100% per register. Long/dense tunes split into multiple SF2 "parts" (see *Part-count*, the open frontier).
...
```
py -3 bin/build_mon_native_song.py SID/Tel_Jeroen/Supremacy.sid 2 auto   # auto = adaptive parts
...
| **Hawkeye** | 0 (6.4-min theme) | 13×30s | ~100/~100/~100/~75 | dense; window-boundary filter seams |
| **Cybernoid** | 0 | 11-18 | 95-100 / ~100 / ~100 / ~99 | GUI-confirmed; see *SBC bug* below |
| **Cybernoid II** | 0 | 20 | 100/99/100/98.8 | third orderlist variant |
| **Myth** | 0 (main) | 7 | 100 / ~99.7-100 / 100 / ~90 | emulation-extracted; ear-confirmed |
| **Myth** | 2 | 2 | 100/99-100/100/96 | sub1 is empty (NOP'd in the wrapper) |
| **Supremacy** | 0 | 34 | 96-98 / ~99.8-100 / ~99.8-100 / 100 | ear-confirmed |
| **Supremacy** | 1 / 2 | 24 / 70 | ~97-99 / ~99.8-100 / ~99.8-100 / 100 | sub2 arp-heavy → 70 parts (see frontier) |
...
## Open frontier — lossless part-count reduction (structural RE)

Dense tunes blow three SF2II hard caps **simultaneously** (per SF2 file: 63 command bundles, 32 instruments, 256 wave rows), forcing e.g. Supremacy sub2 into ~70 parts. Root cause (proven): the trace-driven build captures **per note**, unrolling the player's compact looping synth tables — the original fits in 32K precisely because of table loops/jumps the trace cannot recover. **No trace-based method reduces this losslessly** (per-note bundles and free-running streams both measured infeasible).

The lossless path is **structural RE of the synth engines**, and both are already cracked (Supremacy): the arp engine is a ~16-program looping semitone table (parser committed: `MON.arp_program`), and the waveform is a fixed attack + loopable steady. Remaining, in order (full detail in `whats-next.md`):

1. **Note-timing reconciliation** (the current blocker): the parser's note durations drift ~9 frames from real note lengths; the trace path self-corrects, the structural path desyncs.
2. Re-apply the FM semitone+loop wiring (prototyped: bundles 178→127, first note byte-exact).
3. Wave `[attack][steady + $7F loop]` programs → instruments 87→~5.
4. Pulse, if still bound; then all three caps drop together and the part count falls.
