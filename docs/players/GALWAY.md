...
**Status:** trace-driven native driver reaches **~100% on every SID register** for validated tunes in stock SID Factory II. **SF2II-validated: Wizball · Ocean Loader 1 · Comic Bakery · Terra Cresta · Rambo** (5 / 40). A full-corpus batch build (`out/galway_sf2/`) now covers **all 40**: **37 trace-faithful** (≈100% *headless* pitch) and **3 near** (90-94%) — **0 blocked**. **The headless "37 faithful" figure is superseded** by the objective per-voice metric below (**30/40 objectively clean in real SF2II**) — the headless number overstated, per the 2026-06-27 objective validation. Quote the objective figure, not this one. The `play=$0000` self-installed-IRQ tunes (Arkanoid, Game Over, …) are now traceable: the tracer derives the handler from the installed vector ($FFFE or KERNAL CINV $0314) and drives each frame as a simulated 6502 IRQ. Current headline figures: `docs/reference/ACCURACY_MATRIX.md`.
