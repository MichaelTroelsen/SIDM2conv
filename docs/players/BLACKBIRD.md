...
| **v1.2 + REPEAT=1** ("variant A", RESOLVED 2026-07-22) | 5 | Crank_Crank_Airwolf, Fugue_on_a_Theme_by_D_M_Hanlon, Quintessence, To_Die_For_II, Trinket | **NOT a different tool version** — the SAME v1.2 source, compiled with player.s's `#if REPEAT` flag (loop-on-end support) — see "REPEAT=1 locate support" below. **Locate is fully solved for these too**; `decode_streams` is not (open bug, see that section) |
...
**NOT yet working: `decode_streams`** (the note-stream decompressor) throws on all 5 REPEAT=1 files — genuinely open, not fully root-caused, but real further progress made in a follow-up round the same day. Diagnosed initially: the crash is early (the 7th `_emit_piece` call — voice 1's SECOND piece) and structural — a genuine `copy` op decoding to `dist=19` while voice 1 has only `L=9` bytes decoded so far, an out-of-range back-reference. `streamstart` itself was independently confirmed correct.

**Live-tracing infrastructure fix (real, project-wide value even though the bug itself is still open)**: RetroDebugger (the MCP-driven real C64 debugger) was unavailable in this environment (`retro_start_platform` never returned after 10+ minutes) — fell back to a py65-based CPU trace of the ACTUAL compiled player code (not this project's own translation), the same technique this project's native-driver work already uses for ITS OWN 64tass-assembled output. This surfaced a real py65 gap: **`MPU.instruct` silently no-ops the illegal/undocumented 6502 opcodes `LAX` and `SBX`/`AXS`** (falls back to a PC-only-advance stub) — both used throughout `player.s` (`LAX` to read `zp_master`, `SBX` for the `zp_master-7` tick countdown). A trace on unpatched py65 looks entirely plausible (no error, no crash) while silently producing WRONG register values from the very first `LAX`/`SBX` onward — this cost the better part of an investigation session before being isolated (a hand-derived expected register value disagreed with what py65 actually produced, right at the first `SBX`). Fixed generally, not just for this investigation: **`bin/py65_illegal_opcodes.py`** (`import` before constructing any `MPU()` — patches `LAX` in all 6 addressing modes + `SBX #imm`), tested in `pyscript/test_py65_illegal_opcodes.py` (8 tests, including the exact real-hardware sequence that exposed the bug). **Any future py65 trace of an ORIGINAL compiled 6502 binary (not this project's own 64tass output, which never uses illegal opcodes) should load this patch first.**

With the patch in place, re-tracing confirmed: `initroutine`'s own 2-call priming sequence (voice1 directly, matching `ldx #7; jsr stx_unpackvoice`, then voice0 via the general `jsr playroutine` dispatch) DOES match `decode_streams`' own assumption exactly — that part of the model is correct. But a longer real trace (init + several `play()` calls, breaking at the control-byte fetch inside `unpackvoice`) diverges almost immediately after: real hardware refills **voice1 three times in a row** (`ctrl=0x07,0x07,0x07`) before voice0 or voice2 ever get a first turn, where `decode_streams`' own model assumes a strict `voice1,voice0,voice2,voice1,voice0,voice2,...` rotation. This contradicts a naive "unpackvoice's `X` parameter is just `zp_master`'s own decrementing value, cycling 14→7→0 once per tick" reading of `playroutine`'s dispatch code too (confirmed structurally: `stx_unpackvoice`'s first instruction IS `stx zp_master`, i.e. `X` and `zp_master` really are the same value at that point) — the real per-voice refill cadence is evidently more particular than either model captures, and this is exactly where the investigation stopped this round. **Recommended next step**: with the py65 patch now in hand, trace a much longer real sequence (50+ control-byte fetches) and diff it position-by-position against `decode_streams`' own model output (both now buildable cleanly with the corrected emulator) to find the actual governing rule, rather than reasoning from `player.s` source structure alone.
