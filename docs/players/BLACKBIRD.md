...
| **v1.2 + REPEAT=1** ("variant A", RESOLVED 2026-07-22) | 5 | Crank_Crank_Airwolf, Fugue_on_a_Theme_by_D_M_Hanlon, Quintessence, To_Die_For_II, Trinket | **NOT a different tool version** — the SAME v1.2 source, compiled with player.s's `#if REPEAT` flag (loop-on-end support) — see "REPEAT=1 locate support" below. **Locate solved. `decode_streams`'s IndexError crash also fixed** (copy-op source formula + ring-buffer zero-fill, same day); a separate `prepare3` grammar-violation bug remains open, see that section |
...
**Result on the 5 REPEAT=1 files**: the original `IndexError` crash is gone on all 5 — decoding now proceeds MUCH further (200-3,000+ bytes per voice vs. crashing at byte 9 before). **A separate, deeper bug remains**: all 5 still eventually hit a `prepare3` grammar violation (an "out-of-grammar byte" the `_run_prep1/2/3` note-event state machine doesn't recognize) at varying points (Trinket rpos=203 voice0; Crank_Crank_Airwolf rpos=354 voice1; the other 3 similarly, all in the low hundreds to low thousands of bytes). One specific hypothesis was checked and RULED OUT: the REPEAT-specific EOS loop-rewind OOB signal (`zp_pendoob` bit0, `player.s` lines 412-426) never fires before any of these crashes (traced `pend_oob` values directly — only saw `0xfa`/`0xfc`, both bit0=0), so this is NOT the loop-rewind mechanism firing unhandled. The offending bytes themselves (spot-checked on Trinket: a run of `0xe6` bytes) trace back to a `copy` piece whose `transp2` happens to equal `0xe6` — plausibly still a CORRECT decode (a real, deliberate transpose result), meaning the bug is more likely in `_run_prep1/2/3`'s own byte-consumption/priority logic for REPEAT=1 than in the LZ decompression layer itself, but this was not confirmed via live trace this round. **Next step**: live-trace `_run_prep1/2/3`'s real hardware equivalents (the per-voice arpeggio/gate/instrument/wait dispatch inside `execute()`) the same way this round's copy-formula bug was solved — reuse the same `LDX #$07; JSR $5009`-style breakpoint techniques against the REAL prep-stage code addresses (not yet identified this round).
