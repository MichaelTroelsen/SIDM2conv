...
**This is now a confirmed, root-caused, well-understood bug, not just a strong hypothesis** — the concrete fix is a 3-line addition to `decode_streams()`'s per-frame OOB check (alongside the existing `if pend_oob[0] & 0x02:` block): on `pend_oob[0] & 0x04`, consume 2 bytes via `rd.next()`/`rd.next()` and reposition `rd.ptr = (byte1 << 8) | byte2`. **Deliberately NOT implemented this round** — this touches `decode_streams()`, the shared per-voice decoder used by the already-shipped, extensively-validated 11-file v1.2-exact bucket (99.93-100% accuracy), so the fix needs the full test suite + `BB_DIAG_BIN` re-validation across BOTH buckets before landing, which is real, non-trivial follow-up work, not a same-round drive-by edit. Left as the single, concrete, fully-verified next step for whoever picks this up.
