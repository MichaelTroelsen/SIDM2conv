...
## Correction (2026-07-22, live-hardware verified): the "tempo cascade corruption" theory above is WRONG — real hardware never produces the corrupted record. Root cause is a `need_refill()` threshold mismatch, not a desync cascade

Picked up the "live-trace the tempo desync" next step from the previous round, on the same file (`Crank_Crank_Airwolf.sid`), using RetroDebugger (confirmed responsive, no zombie-process issue this time). Disassembled the compiled binary's `execute()` routine (`$5198`) and confirmed it matches `player.s` byte-for-byte at every instruction (`zp_base=$E0`, `zp_inptr=$E2/E3`, `zp_pendoob=$E5`, `zp_tempo=$EA`, the REPEAT eos-rewind block all present and at the expected offsets) — this file compiled with zero relocation relative to the template for this code region.

Set a breakpoint at `$51A4` (the `TAX` reached only when the tempo bit is actually set — cleanly isolates genuine tempo events from the ~49-frame-per-tick noise). **Event 1** (tick 0): live hardware reads `zp_inptr=$56A3`, `pendoob=$FA`, tempo=49, groove=0 — matches the Python model's own prediction exactly (`ptr_before=0x56a3`, `pend_oob=0xfa`, confirmed via a standalone re-run of `decode_streams()`'s inner loop). Good agreement, as expected — this is the record BLACKBIRD.md already called clean.

**Event 2** (continuing to the next tempo-bit hit): landed at real frame 2317 (vs. 1165 for event 1 — a gap of exactly 1152 real frames, i.e. 1152/49 ≈ 23.5 ticks). Read the zero page: `zp_inptr` is **still `$56A3`, byte-for-byte identical to event 1** — the compressed-stream pointer had not moved at all. Single-stepped through the actual `LDA ($E2),Y` reads for tempo and groove: **A=49 (tempo), A=0 (groove) — the exact same clean values as event 1**, not the Python model's predicted corrupted `(16, 202)` at `$5561`. Continued to a **third** tempo event for a sanity check: same result again — `zp_inptr=$56A3` unchanged, `pendoob=$FA`, and the real frame gap from event 2 is again exactly 1152 frames. Perfectly periodic, perfectly clean, three times in a row.

This directly falsifies the cascade theory for this file, at least this early in the stream: real hardware is not desyncing at all here — it is re-reading the exact same 2 bytes at `$56A3` every ~23.5 ticks because the compressed reader genuinely never needs to advance. Per-voice buffer read-positions (`zp_bufs+0/7/14`) DO advance substantially between events (voice2's own read pointer, `zp_bufs+14`, advances by a consistent 101 bytes each cycle) — real playback is progressing normally — but `unpackvoice()`'s own real skip condition (disassembled at `$5272`: `LDA v_trwpos,X; CMP zp_bufs,X; BMI skip`, a signed-byte margin check) is evidently satisfied (skip) for far longer than the Python model's `need_refill()` heuristic (`(len(v.out) - v.rpos) < 128`) would predict — the model refills far more aggressively and races ahead of where real hardware's `zp_inptr` actually is at the analogous tick. By the time the model's own loop counter reaches "tick 194", it has raced far ahead into the init-code-overlap tail of the stream and reads what looks like a corrupted tempo record — but that tick number and stream position do not correspond to anything real hardware ever visits at a comparable point. The "cascade" was a self-consistent but wrong story built entirely from the model's own (mistimed) output, never cross-checked against hardware until now.

**Not yet resolved**: the exact real-hardware refill/margin algorithm (the signed `BMI`-based comparison's precise ring-buffer size and initial-fill behavior — empirically, voice2 accumulates enough backlog to coast through at least 3 tempo cycles, ~300+ bytes of consumption, without any compressed-stream read, which doesn't yet reconcile with a naive 256-byte-ring / 128-byte-threshold reading of the `BMI` check; there may be a larger effective buffer, a different initial-fill burst, or the comparison semantics are subtler than assumed).

**Follow-up same round: the `need_refill()`-threshold theory doesn't fully hold up either — the model's OWN refill schedule is qualitatively similar to hardware's, which reframes the mystery.** Ran the Python model's own `need_refill()` trace tick-by-tick for the first 250 ticks: voice2's margin climbs from 3→137 over ticks 0-17 (every voice refilling every tick, matching a live hardware sample taken the same way — real hardware ALSO shows all 3 voices decompressing every tick with no skips through at least ~50 hits/17 ticks, before the trace was stopped for cost). From tick 18 on, the model's own voice2 margin oscillates in a tight 127-137 band, refilling only once every ~15 ticks — a sparse pattern that looks structurally similar in shape to what live hardware showed (long stretches with zero compressed-stream movement, punctuated by refills), not the "refills far more often than hardware" story first proposed. This means the divergence is likely NOT primarily a refill-cadence/threshold mismatch after all.

Re-examined the model's own decoded voice2 content directly: `real_length=299` bytes total, with OOB tempo-bit markers ($FA/$FB/$FE/$FF with bit1 set) at buffer positions **0** (event 1), **126**, **248**, **267** — i.e. the model's own SECOND tempo marker sits at content-position 126, quite early in a 299-byte stream, not deep in some later region. Live hardware's directly-measured `zp_bufs+14` (voice2 read-pointer) delta between event 1 and event 2 was **101 bytes** — in the same ballpark as content-position 126, not wildly off. Since the model's own loop counts 194 *ticks* (not bytes) to reach that position, the gap between "126 bytes of content" and "194 ticks elapsed" is most plausibly explained by held-note delays (`v.timer` going negative and consuming several ticks per byte for long notes) rather than a raw consumption-rate bug — but at tempo=49 frames/tick, 194 ticks is roughly 9500 real frames, while live hardware reached its own second `$FA`-bit1 event in only ~1152 real frames total. Even generously assuming most of that time used the short (~8-frame) bootstrap-length ticks observed early in the live trace, this real-time gap does not yet reconcile with either "194 ticks at tempo 49" or a simple bootstrap-tick correction. **This still needs to be resolved**: either (a) real hardware's own prep1 delay-holding pacing differs from the model's in a way not yet identified, or (b) real hardware's voice2 content genuinely contains a tempo marker earlier than content-position 126 that the model's decode misses or misdates — the latter would mean a genuine, still-live decode-content bug survives, not just a timing artifact. Not resolved this round; flagged as the next concrete thing to check (compare the model's OWN full position-vs-tick mapping for voice2 against a live hardware capture of `zp_bufs+14` sampled every tick, not just at tempo events, to settle whether tick and byte-position accounting truly agree).

**Fix target NOT yet nailed down** — what's solid, and stands regardless of which of the above turns out to be the true mechanism, is the ROOT FINDING itself: real hardware's second and third tempo-bit events, measured directly and reproducibly (3 independent live captures, single-stepped down to the actual `LDA ($E2),Y` register loads), read clean `(49, 0)` — not the model's predicted corrupted `(16, 202)`. Whether this bug lives in `need_refill()`'s cadence, in `v.timer`/delay-holding pacing, or in a subtler voice2 content-decode issue, is the open question for the next round. Whether fixing it actually resolves the REPORTED fidelity degradation (round 2's `BB_DIAG_BIN` findings) is a SEPARATE thing still to re-measure — that causal link was inferred, not measured.

## Follow-up, same round: settled it — this is a genuine voice2 CONTENT decode bug, not a timing artifact. Exact divergence point still open.

Directly tested the "position-vs-tick mapping" question by comparing the model's own `voices[2].rpos` against live hardware's `zp_bufs+14` (voice2's real ring-buffer read pointer, dereferenced against its base address `$5900` — confirmed via tick 0's pointer `$5904` matching the model's own `rpos=4` at that exact point) at 5 matched tick numbers, using a breakpoint at `execute()`'s entry (`$5198`, fires exactly once per tick, giving an unambiguous tick counter via hit count):

| Tick | Model `rpos` | Hardware position | Delta |
|---|---|---|---|
| 17 | 15 | 15 | 0 |
| 50 | 37 | 36 | -1 |
| 100 | 70 | 70 | 0 |
| 150 | 105 | 107 | +2 |
| 193 | 126 | 136 | +10 |
| 194 (model predicts 2nd tempo marker here) | 130, `$FA` | 137, `$00` (no marker) | growing |

Position tracking agrees almost exactly through tick 100 (a real, working cross-check that the model's tick-pacing is basically right early on), then drifts — and by the tick where the model predicts its own second tempo marker, hardware has already passed that buffer position without ever seeing the bit set. Switched to the tempo-bit-only breakpoint (`$51A4`) and let hardware find its own actual next tempo event: it fires at voice2 buffer position **206** (`zp_bufs+14` dereferenced), with `zp_inptr` still `$56A3` — the SAME re-read-the-cached-bytes behavor documented above, confirming this is genuinely the "2nd/3rd/4th event, same clean values" pattern, now precisely located.

**Directly compared voice2's ACTUAL decompressed bytes (read straight out of hardware's `unpackbufs` memory) against the model's own `voices[2].out` array, position-for-position, over positions 133-229.** They do not match — not even under a constant-offset shift (tested shifts -3..+3, best case 40/54 = 74%, from an alternating `0x18/0x31` pattern that partially self-matches under any odd shift; no shift gives full agreement). This rules out "my position bookkeeping is off by a fencepost" as the explanation — **the two decodes have genuinely diverged in content**, not just in timing. The model's own content around this region includes an 8-byte run of literal `0x00` bytes (positions 213-220) that hardware's real buffer does not have at all — a strong signal of a specific mis-decoded copy or literal-run operation whose bad output then gets fed into a LATER copy op referencing back into it (the classic LZ-decoder failure mode: one bad piece corrupts everything copied from it afterward).

**Correction, same round: the "content diverges around position 107-136" claim above does NOT survive a more rigorous re-check — retracted.** Went back with a precise register-level trace (breakpoint on the copy-op's own `LDA` at `$52D2`, single-stepping every iteration) instead of a bulk memory dump taken at a later, unrelated pause point. Traced voice2's real piece sequence from tick 0 forward, byte-for-byte, confirming EVERY piece boundary exactly matches the model (`L` values 0, 7, 14, 23, 32, 41, 50, 51, 60, 69, 78, 87, 96, 101 — all hit exactly, X/A register state cross-checked at each). At the piece that was flagged as suspicious (`L=101`, copy-op `src=211`, the exact case the code's own docstring already cites as a "confirmed via live trace" zero-fill instance): captured the REAL register state at the actual `LDA` instruction — `X=211` (source index, matches the model's formula exactly), `A=0` (the loaded byte). Single-stepped through all 10 output bytes of this piece: every one reads and writes `0x00`, matching the model exactly. Immediately re-read hardware memory at positions 101-160 (fresh, right after this piece completed, and again somewhat later): **still all zero**, matching the model, not the non-zero content the earlier bulk-dump comparison claimed to see there.

This means the earlier "positions 107-229 don't match, even under a shift" finding was a methodology artifact, not a real bug — most likely caused by stale breakpoints from an earlier phase of the same live-tracing session interfering with the later bulk-memory-read snapshot's timing/pause point (a leftover `$5281`/`$52D2` breakpoint pair was found still armed partway through this investigation and had to be removed). **Do not trust a bulk memory-dump comparison taken at a pause point reached via a different breakpoint/continue history than the one used to establish the baseline** — re-verify with a fresh, tightly-controlled register-level trace before treating a content mismatch as real, the same lesson the false-positive loop-period finding earlier in this file already taught in a different guise.

**Resolution, same round: the decode is verified CORRECT — the real mystery is narrower than any prior theory in this file, and is about `prepare1`'s read-side timing, not content.** Repeated the exact register-level trace for the next two voice2 copy-ops in sequence, byte-by-byte:

- `L=111` (`src=231`, another `_ring_read` zero-fallback case): all 10 output bytes read `A=0` on real hardware, matching the model exactly (X/src register cross-checked at every byte: 231, 232, 233... incrementing correctly).
- `L=121` (`src=251`): traced all 10 bytes. Bytes 1-5 (`src=251..255`) all read `A=0`, matching the model's own zero-run. **Byte 6 (`src` wraps `255→0`, reading from the very first bytes ever produced): `A=250=0xFA`** — matching the model's own predicted transition to real data (`0xfa,0xc9,0x85,...`) at this exact position, EXACTLY. Confirmed via live register capture, not a memory dump — `X=0` (wrapped src), `Y=126` (write position), `A=0xFA`, page byte `0x59` (voice2) all cross-checked together in one instruction.

**This means voice2's decoded content is correct at position 126 on real hardware — the byte genuinely IS `0xFA` (a tempo-bit marker), exactly as the Python model computes.** Every specific "content bug" theory floated this session (the `L=101` zero-fill, the general "content diverges by position 107-136" claim) is now conclusively ruled out by direct, repeated, register-level measurement. The REPEAT=1 copy-op formula and `_ring_read` fallback are correct, at least across all three pieces checked (`L=101`, `111`, `121`) spanning the file's first ~130 decoded bytes.

**So why did the earlier live capture (this file's own tempo-bit-only breakpoint hunt) find hardware's actual second tempo event at buffer position 206, not 126, if the byte at 126 really is `0xFA`?** The answer must be in `prepare1`'s own READ-side consumption, which is a SEPARATE pointer (`zp_bufs,X`, advanced by `prepare1/2/3`) from the WRITE-side pointer traced in this section (`v_trwpos,X`, advanced by `unpackvoice`/decompression — confirmed as two genuinely distinct zero-page-adjacent mechanisms, not the same value, despite both loosely tracking "how far voice2 has progressed"). The write side reaches position 126 very early (within the first ~15-18 aggressively-refilling ticks). The read side, per this session's own earlier tick-position table, is still only at position 136 by tick 193 — well past 126, meaning `prepare1` MUST have already stepped over position 126 on its way there. Yet the tempo bit didn't fire until read-position 206. **The remaining open question is narrow and specific**: why does `prepare1`'s own consumption not register the `$FA` byte at position 126 as an OOB event when it (apparently) reads past it — candidates include a `v.timer`/hold-driven skip that consumes 2 bytes at once and lands on the wrong parity, an off-by-one in exactly which byte the read pointer is at when compared against write-position 126, or the get/unget mechanism (`_prep_ungetbyte`) somehow re-consuming or displacing this specific byte. This needs the SAME register-level tracing technique applied to `prepare1`'s own code path (`zp_bufs,X` reads, not `unpackvoice`'s `v_trwpos` reads) rather than the write-side tracing done this round.

## `prepare1`'s own read-side traced directly — the "126 vs 206" framing was a category error (two different pointers were never the same event), but a NEW, better-anchored content divergence surfaced around read-position ~200

Live-traced `prepare1`'s own code path directly (not `unpackvoice`'s write side, which is what every prior section in this file actually measured despite talk of "read position"). Disassembled the full `$5000-$50E0` dispatcher region fresh, confirming it byte-for-byte against `player.s`'s documented event-byte-encoding table (the table at "Event-byte encoding" below, already in this file, dated 2026-07-19 — this session cross-validated it live for the first time): **three separate consumption stages share ONE per-voice cursor** (`zp_bufs,X` = `$E0,X`-indexed, dereferenced via `LDA ($E0,X)`/`INC $E0,X`) — `prepare1` at `$5018` (handles `$C9-$FF`: arpeggio/fx-select and the `$F9-$FF` OOB/tempo range), `prepare2` at `$5043` (handles `$80-$B2`: gate-off/legato/instrument), `prepare3` at `$5081` (handles `$00-$7F` notes and `$B8-$C7` delay). Each stage peeks the byte at the current cursor and only consumes (advances the cursor) if it falls in that stage's own range; otherwise it's left untouched for the next stage. All three additionally gate on the SAME shared per-voice hold-timer byte (`$52F4,X`) — confirmed via disassembly, not previously live-verified. No bug found in this mechanism itself; it matches the design exactly as already documented.

**Direct capture of voice2's own next OOB/tempo-bit event** (breakpoint at `$5023`, the `STA $E5` `prepare1` executes only when it reads a byte `>=$F9` — X=14 confirmed on the very first hit, no filtering needed): command byte `$FC` at read-position 200 (bit1 clear — an OOB type other than tempo, e.g. sync/loop, not investigated further), then continuing to the next hit: command byte `$FA` at read-position **202** (bit1 set — genuine tempo type), parameter byte `$C9` (201 decimal) read immediately after from position 203, giving `(201 - 0xC7 - 1) = 1` stored into `$5304,X`/`$52F1,X` — this stored value is NOT the tempo itself (tempo/groove are read separately, straight off the shared compressed reader `zp_inptr`, at `execute()`'s own `$51A4` block per the "Tempo/groove model" section below — confirmed already-documented, now cross-referenced against this specific live capture). Read-position 202 is close to (not exactly, but in the same neighborhood as) the earlier session's own coarser `zp_bufs+14`-sampled figure of "206" — consistent with measuring the same real event at two slightly different moments in its consumption (this session's capture is the command byte itself; the earlier "206" was read at some later point in the same tick's processing after more of the shared cursor had advanced), not a contradiction.

**This resolves the "126 vs 206" framing itself as a category error, independent of whatever the real remaining mystery turns out to be**: position 126 (section 5 above) is a WRITE-side milestone — the first moment `unpackvoice`'s decompression ever produces the byte value `0xFA` anywhere in voice2's output — measured via `v_trwpos,X`. Position ~202-206 is a READ-side event — the moment `prepare1`'s own hold-timer-gated traversal happens to land on ITS OWN occurrence of an `$F9+` byte with bit1 set, measured via `zp_bufs,X`. Nothing in the architecture (as newly confirmed via live disassembly of the `execute()` tempo/groove tail, and the already-documented event-byte table) says these must be the *same* occurrence of `0xFA` in the stream — `0xFA`/`0xF9`-`0xFF` bytes can and do appear at multiple content positions (the model's own decode lists OOB bytes at content positions 0, 99, 126, 175, 211, 248, 267, 269, 281, 283 for this file), and only the one `prepare1` actually reads (paced by its own per-voice timer, arriving at whichever position that happens to be once its hold-timer next expires) matters for playback. Treating "the write side's *first* `0xFA`" and "the read side's *next* OOB event" as the same thing was the root of two sessions' worth of confusion; they were never guaranteed to coincide.

**However, this same live trace surfaced a NEW, more concretely anchored discrepancy that is NOT yet resolved.** A direct memory dump of voice2's buffer at read-positions 195-214, taken immediately after the register captures above with no intervening breakpoint churn and `isPaused: true`/no stale breakpoints reconfirmed before and after (the specific hygiene failure that caused the earlier, retracted false positive — deliberately guarded against this time): `195:0x31 196:0x18 197:0x31 198:0x19 199:0x09 200:0xFC 201:0x0F 202:0xFA 203:0xC9 204:0x85 205:0x12 206:0x2B 207:0x12 208:0x2B 209:0x18 210:0x31 211:0x18 212:0x31 213:0x18 214:0x31`. The model's own `voices[2].out` at the SAME index range reads: `195:0x12 196:0x2b ... 200:0x2b 201:0x18 ... 210:0x31 211:0xFC 212:0x0F 213:0x00 214:0x00 215:0x00 ... 220:0x00 221:0x12 ...` (an 8-byte zero run at 213-220, immediately after its own `0xFC,0x0F` pair at 211-212).

The distinctive, non-repeating `0xFC,0x0F` pair appears in BOTH sequences — hardware at 200-201, model at 211-212, an 11-position offset — but the two bytes immediately *after* it diverge completely: hardware continues `0xFA,0xC9,0x85,...` (real, varied data), the model continues `0x00,0x00,0x00...` (an 8-byte zero run). A naive constant 11-byte shift does NOT hold for the surrounding bytes either (e.g. hardware's `0x09` at 199 has no counterpart in the model's sequence at the shifted position 210, which is `0x31`) — so this is not simply "the same content, offset by a fixed amount"; something changes the byte *count* relationship between logical (model) and physical (hardware) position somewhere between the last rigorously-confirmed anchor (piece `L=121`, content position ~126-130, section above) and this region (~200-212). This is the single most concrete, best-anchored candidate content divergence found across either session — clearly distinguished from the earlier retracted false positive by: (1) a distinctive 2-byte anchor rather than an ambiguous period-2 repeating pattern, (2) two independent capture methods (live register trace AND an immediate follow-up memory dump) agreeing with each other, (3) explicit breakpoint-hygiene verification before treating it as real. **It is explicitly NOT yet confirmed as a bug** — the alignment logic above is a first pass, not a rigorous piece-by-piece walk, and the same class of reasoning error (assuming physical read-position and logical decode-index are simply interchangeable) may itself be the flaw rather than the decode. The concrete next step: repeat the SAME piece-by-piece register-level walk used successfully for `L=101/111/121` (section above), continuously from that last-confirmed anchor forward to content position ~210, with no `warp`/bulk-continue gaps in between, to find the exact piece (if any) where hardware's actual consumed-byte count first departs from the model's.

## Retraction, same round: the position-200-212 "divergence" above does NOT survive a controlled, immediate re-check either — decode confirmed correct through content position 180, extending the confirmed-good range

Did exactly the next step called for above: a fresh reload, then a continuous, piece-by-piece live walk of voice2's `unpackvoice` copy-loop (breakpoint on `v_trwpos+14`, `$52FD`, write `>=` each successive piece boundary — `69,78,87,96,101,111,121,131,141,151,161,171`, every single hit's `X`/`A`/`Y` cross-checked against the model's own piece-boundary list before trusting it, no gaps or `warp`-only jumps skipped without verification). Landed exactly on piece `L=171` (`src=95`, `offset=170`, matching the model's own copy-op formula), and read the buffer **immediately** upon the piece's own completion (`X=14, Y=181, A=181` — the precise instant `v_trwpos` was stamped with the new write position, the freshest possible snapshot): `171:0x18 172:0x31 173:0x19 174:0x09 175:0xFC 176:0x0F 177:0x00 178:0x00 179:0x00 180:0x00` — **byte-exact identical to the model's own piece `L=171` data (`18 31 19 09 fc 0f 00 00 00 00`), including the 4-byte zero-fill tail.** A wider follow-up read (positions 171-210, taken one instruction later, still the same controlled session) confirms positions 181-210 correctly read all-zero too — genuinely not-yet-written cold buffer, exactly as expected for content the decompressor hasn't reached yet at this point in real time.

**This directly contradicts the "200-212 divergence" found earlier this same round** — that snapshot claimed hardware held `0xFA,0xC9,0x85` at the physical positions aligned (via the `0xFC,0x0F` anchor) with the model's logical positions 213-215, which this fresh, controlled capture shows are actually all-zero (not yet written) at a comparable point in the decode. The two snapshots cannot both be reflecting "logical position 171-215 as the decompressor first writes it" — and the controlled one (immediate read at a precisely-known completion point, cross-verified against the model's own piece-boundary sequence hit-by-hit on the way there) is the trustworthy one. **The earlier divergence reading was very likely taken far later in real execution time than its face-value "read-position ~200" suggested** — it was captured via a single long `warp`-accelerated `continue` chasing a *read-side* breakpoint (`zp_bufs+14 >= 126`, gated by prepare1/2/3's slow, per-voice hold-timer pacing, which can stall for very long real-frame stretches per the earlier "voice2 stalled for ~10 consecutive $5281 hits while voice0/voice1 raced ~50+ bytes ahead" observation in this exact investigation), with no intermediate checkpoints — plenty of opportunity for `unpackvoice`'s *write* side to have cycled through this same physical ring region multiple times in the interim (it is margin-gated to ~128 bytes ahead of the read side, but 128 bytes is still enough to have overwritten the 171-215 range more than once by the time the read side finally caught up to 126). The specific mechanism (what exactly was in that ring slot by then — a later piece's data, or `stopstream` filler content once voice2's real 299-byte content fully drains — was not pinned down) doesn't need to be resolved for the retraction to hold: **the reproducible, controlled result is that piece `L=171` decodes byte-exact correct**, extending the confirmed-good range from content position ~130 (prior round) to ~180 (this round), with zero decode bugs found across either session's entire hunt so far.

**Restated, sharper methodology lesson (a second, different manifestation of the same underlying trap as the section-4 stale-breakpoint false positive)**: a raw memory/register snapshot is only trustworthy as "logical position N's content" if it is captured via a *continuous, gap-free walk* from a last-known-good anchor — the moment a `warp`-accelerated bulk `continue` spans an unknown, unverified stretch of real time before the snapshot, the physical-address-to-logical-position correspondence can no longer be assumed to hold, because the same physical ring slot may have been legitimately overwritten multiple times in between by the ordinary, correct operation of the write side outpacing a slow, note-hold-gated read side. This is now the THIRD time in two sessions this general class of trap has produced a false content-divergence signal (the section-4 stale-breakpoint dump, and this round's own bulk `warp`-then-snapshot read) — any future Blackbird buffer-content comparison MUST use the continuous piece-by-piece walk method exclusively, never a snapshot reached via an unverified `warp`/bulk-`continue` gap, however tempting the shortcut.

**What this leaves for the REPEAT=1 fidelity-degradation mystery**: after two full sessions of live-hardware content verification, spanning content positions 0 through ~180 (pieces `L=0,7,...,171`, roughly 40% of voice2's 299-byte real content for this file) with every single check confirming the model's decode byte-exact correct, there is now real reason to suspect the reported fidelity degradation does NOT stem from a per-voice stream decode bug at all. Two areas remain genuinely untested and are the most promising next leads: (1) the `stopstream`/`$c0`-filler mechanism that kicks in once a voice's real content ends (`v.frozen_at`, confirmed at content position 299 for this file's voice2) — never live-verified against real hardware's own post-freeze behavior, and directly relevant since `$C0` happens to fall in `prepare3`'s own `$B8-$C7` delay range, meaning it's NOT inert once encountered; (2) whether the ORIGINAL `BB_DIAG_BIN` fidelity measurement that motivated this entire two-session investigation (`Crank_Crank_Airwolf`'s freq collapsing from 100%→5% over a 3000-frame window) has ever been re-run against the CURRENT state of the native driver/decoder — that causal link (does fixing anything here actually move that number) was inferred at the very start of the prior session and has not been re-checked once since.

## Re-ran `BB_DIAG_BIN`, same round: the degradation is real and unchanged — this REDIRECTS suspicion decisively away from `blackbird_parser.py`'s per-voice decode and toward the native-driver build script's own row/frame timing

Answered open item (2) directly: `BB_DIAG_BIN=500 python3 bin/build_blackbird_native_song.py SID/LFT/Crank_Crank_Airwolf.sid`. Current numbers match the prior session's own recorded figures almost exactly — `freq` 100.0%/100.0%/81.3%/25.4%/9.5%/4.6% across the six 500-frame bins (previously documented as "100%→81%→25%→9%→5%"), `overall` 66.8% full-part — confirming two things at once: **the degradation is still present, unchanged, in the current codebase** (nothing this round's decode-verification work accidentally fixed it), and **the causal link assumed at the very start of the prior session — "a per-voice stream decode bug causes this" — was never actually confirmed, and after two sessions of exhaustive live-hardware decode verification turning up nothing, is now the least likely explanation left standing.**

This matters because it decisively re-scopes where the real bug almost certainly lives. `blackbird_parser.py`'s `decode_streams()` (the per-voice LZ-stream decoder, live-verified byte-exact through content position ~180 across two full sessions) is a completely different code path from `bin/build_blackbird_native_song.py`'s `run_full_song_sim()` and its row→real-frame mapping — the layer that actually decides, for the NATIVE DRIVER TRANSLATION specifically, *when in real-frame time* each decoded row should fire. That row/frame mapping already has a documented, still-unexplained anomaly of its own (section above, "MAJOR finding" round): `run_full_song_sim()` only reaches row 307 of `Crank_Crank_Airwolf`'s 441-row span within a 20,000-frame budget, with per-row real-frame gaps growing from ~8 frames to 150+ over time — and the degradation this round re-confirmed starts right around frame 1000-1500, in the same neighborhood as where that gap-growth was first noticed and where the file's second tempo/groove OOB record gets consumed (row 195→196 transition, per the now-superseded "cascading corruption" section above). **The concrete next step for a future session**: stop looking at `blackbird_parser.py`'s per-voice content decode (two sessions of live verification have earned it a clean bill of health across the range checked) and instead live-trace or otherwise root-cause `run_full_song_sim()`'s own row→real-frame timing computation in `bin/build_blackbird_native_song.py` — specifically why its per-row real-frame gap grows the way it does, and whether that growth is the actual mechanism feeding wrong timing into the translated native SF2's row data.

## ROOT CAUSE FOUND, same round, live-hardware-verified: `decode_streams()` is missing a 4th OOB bit — a shared-reader REPOSITION/jump event — that real hardware genuinely executes and the model silently drops

Went back to this round's own live disassembly of `execute()`'s pendoob dispatch (captured while tracing `prepare1`, section above) with fresh eyes and found a THIRD distinct bit-gated block that had not been fully accounted for: after the sync (bit0, `$5198`) and tempo (bit1, `$51A4`) blocks, a third `LSR` at `$51BC` gates a block at `$51BF-$51D3` that is structurally identical to the tempo block's own 2-byte consume (`LDA $E2; SBC #$02; STA $E2; BCS +; DEC $E3` — same clean `-2`, carry confirmed pre-set by the gating `LSR`) but instead of storing the 2 read bytes as `(tempo, groove)`, it stores them directly into `$E2`/`$E3` themselves — **a raw reposition of the shared compressed-stream reader**, not a data update. `blackbird_everyframe_sim.py`'s own Python `execute()` (used by `run_full_song_sim()` for native-driver row/frame timing) has a comment for its own 3rd bit check calling this "eos -- not modelled (loop-rewind path)" — correctly identifying it as loop-rewind-related, but `sidm2/blackbird_parser.py`'s `decode_streams()` (the per-voice CONTENT decoder, shared with the already-shipped 11-file bucket) has **no code path for this bit at all** — it only ever checks `pend_oob[0] & 0x02` (tempo). A bit2-only event (e.g. `$FC` — bit1 clear, bit2 set) sets `pend_oob[0]` for that frame exactly as prepare1 does on real hardware, but `decode_streams()` silently drops it: no 2 bytes consumed from `rd`, no reposition applied — `rd`'s position is simply wrong, forever, from that point forward.

**Instrumented `decode_streams()`'s own frame loop (not a code change, just print statements) and found this bit fires for every one of the 5 REPEAT=1 files**, correlating exactly with which ones degrade:

| File | First bit2 event (sim frame) | Degrades? |
|---|---|---|
| `Crank_Crank_Airwolf` | 143 | yes (~1000-1500f) |
| `To_Die_For_II` | 287 | yes (~1000f) |
| `Trinket` | 63 | yes (~500-1000f) |
| `Fugue_on_a_Theme_by_D_M_Hanlon` | 112 | yes (waveform-only pattern) |
| `Quintessence` | 2079 | **no** — the one file that never degrades, and its bit2 event lands right near the end of the 3000-frame comparison window, leaving no runway for the resulting drift to become measurable |

This also retroactively explains the EARLIER (disproven-on-hardware) "cascading tempo corruption" finding from this same file's very first investigation round: the model's second tempo record computed as garbage `(16, 202)` not because real hardware corrupts anything (this session proved hardware's actual tempo reads stay clean, repeatedly, via direct trace) — but because `decode_streams()`'s shared reader `rd` was ALREADY silently mis-positioned by a missed bit2 event that occurred earlier in the same file's stream, so by the time `rd.next()`/`rd.next()` ran for the tempo bit, it was reading from the wrong address entirely.

**Live-verified the exact reposition target, register-level, not just via the Python model's own prediction.** Computed the model's own prediction first (a genuine falsifiable test, not fitted after the fact): at the bit2 event, `rd.ptr = $559D` (confirmed independently live on hardware via `$E2/$E3` read at the breakpoint — exact match), predicted new pointer via the SAME formula as the tempo block (`hi = byte@ptr, lo = byte@(ptr-1), new_ptr = (hi<<8)|lo`) → `hi=0x56, lo=0xB0` → **predicted `new_ptr = $56B0`**. Set a breakpoint at `$51BF` (the reposition block's own entry), single-stepped instruction-by-instruction through to the final `STX $E3`, and read `$E2/$E3` immediately after: **`$56B0` — exact match to the prediction, byte for byte.** `$56B0` sits close to `streamstart` (`$56C0`) — i.e., since the stream is read backward (decreasing addresses = more consumed), a jump to a HIGHER address is a genuine REWIND toward the start of the compressed data, exactly matching "REPEAT"/loop-rewind semantics by name.

## Implemented and tested, same round: the naive reposition fix REGRESSES the test suite — the mechanism is real and the FIRST jump is exactly right, but it is very likely a genuine song-loop construct, not a simple missing-code-path bug

Implemented the fix described above in all three independent copies of this frame loop that share the bug (`sidm2/blackbird_parser.py`'s `decode_streams()`, `bin/blackbird_everyframe_sim.py`'s `find_tempo_records()`, and a third, previously-unnoticed copy in `sidm2/blackbird_driver11.py`'s `extract_tempo_pairs()` — grepped for every caller of `_run_prep1(voices...)` to confirm these 3 are the complete set). Ran the full suite: **5 new failures**, all in `TestBlackbirdRepeatSweep` (the exact test class that's guarded this whole bucket since 2026-07-22) — every one of the 5 REPEAT=1 files now either fails to freeze at all, or produces thousands of out-of-grammar bytes (`$82`, not in ANY documented grammar range) that didn't exist before.

**Root cause of the regression, traced directly**: the FIRST reposition event (frame 143, `$559D`→`$56B0`) is exactly correct — re-confirmed via the same instrumentation. But the SECOND event for the same file (frame 262) computes `$5631`→`$0180` — an address nowhere near the compressed-stream region (`$5000`-`$56C0`) at all. From there, decoding degenerates into repeatedly reading `$C0` filler and never reaching a genuine terminator: `decode_file()` with the fix ran the full 200,000-frame budget without freezing, producing 72,072/65,683/58,852 bytes per voice (vs. ~299/241/279 real bytes without it) — the reader is chasing something, not converging.

**What this almost certainly means**: `$56B0` sits right next to `streamstart` and its first bytes there are voice2's own first-ever content (`0x12, 0x2B, ...` — literally the start of the song). This isn't corruption; it's a genuine **loop-back-to-the-top** construct — exactly what "REPEAT" names. Once the reader is repositioned there, `_emit_piece`'s own normal decompression continues forward from that point and **re-decodes the same musical content again**, appending it a second time to the per-voice `out` array (which was never designed to represent a loop — it's a plain, ever-growing `bytearray`, built on the assumption of one linear, finite pass). The second bit2 event, then, is very likely a real trigger encountered *during the replayed section* — but `decode_streams()`'s existing bookkeeping (piece positions, `L` values, `_ring_read`'s ring-slot math, all keyed to `len(v.out)`) has no concept of "this content is a repeat of earlier content," so by the second lap the whole apparatus is operating on assumptions that no longer hold, and the computed reposition target is 8-bit-truncation garbage from data that was never meant to be read as a raw address at that point.

**This reframes the whole investigation's conclusion.** The realistic fix is NOT "always follow the reposition" (this session's naive attempt, now reverted) — it is almost certainly to treat the FIRST bit2 event the way `_emit_piece` already treats the genuine terminator: as a **stop signal for the linear decode**, recording it as a loop point (`rd.ptr` at the jump, or equivalently the byte position it targets) rather than actually re-entering and re-decoding. What plays back correctly on REAL hardware (which loops forever, indefinitely, as intended) would then need to be represented explicitly as a loop marker in whatever consumes `decode_streams()`'s output (the native driver's own row/sequence data, which — like most SF2/tracker formats — already supports loop-back sequence references for exactly this purpose), not smuggled into a single ever-growing linear byte array. This is a real, github-worthy architecture question, not a quick patch, and is the true next step.

**Changes reverted, tests confirmed green again** (`git checkout -- sidm2/blackbird_parser.py bin/blackbird_everyframe_sim.py sidm2/blackbird_driver11.py`, `python3 -m pytest pyscript/test_blackbird_parser.py -q` → 12 passed). Nothing shipped this round — per this project's standing "accuracy over speed, never ship lossy output silently" rule, a fix that trades a bounded, well-understood residual (silently ignoring bit2, current shipped state) for an unbounded, un-terminating decode is a strictly worse outcome and was correctly not merged.
...
## Residual investigated, same round: the row/frame timing-drift anomaly is FULLY RESOLVED (a side effect of the loop fix) — but a NEW, different, localized desync found: the native driver runs ~40-60 frames ahead of the simulator during a held-note-plus-arpeggio passage

Picked up the residual freq/waveform/pulse degradation (still starting ~frame 1000-1500 on 3 files, per the table above) as the next lead. First checked whether the OLD, still-unresolved anomaly from the "MAJOR finding" round — `run_full_song_sim()`'s own `row_frame` map showing per-row real-frame gaps growing from ~8 to 150+ over time — was still present now that `find_tempo_records()` (which feeds `run_full_song_sim()`'s `tempo_debug`) has the SAME loop-freeze fix applied. **It is not**: re-ran `run_full_song_sim()` on `Crank_Crank_Airwolf` with the current code and the CORRECT `total_rows=353` (matching what `main()` actually passes — an earlier, hastier check in this same round used the wrong value and wrongly suggested otherwise) — 355 rows tracked across exactly 2828 real frames (matching the build's own reported figure exactly), gap stats `min=3, max=8`, **zero growth, zero drift, for the entire song**. This anomaly, open since the "MAJOR finding" round, is now fully explained and fully fixed as a side effect of this round's main fix — it was never really about row/frame bookkeeping at all, it was `find_tempo_records()` silently drifting past the same missed loop-reposition event this round already root-caused and fixed elsewhere.

**With that hypothesis eliminated, traced the ACTUAL residual precisely.** `BB_DIAG_LO`/`BB_DIAG_HI`/`BB_DIAG_REG` (register 7 = `$D407`, voice1 freqlo) on `Crank_Crank_Airwolf` around the frame-1200 divergence found: from comparison-index ~1208 (raw frame ~1211, given the build's own "sliced at real-frame offset 3" convention), the simulator holds a STEADY value (`$c4,$09` — a long-held note, `row_state` confirms `pendnote=14` unchanging for 7 consecutive rows, ~48 frames, rows 151-157, with an active `currfx` arpeggio/vibrato program, `fxpos` mostly parked at 15) while the driver's own trace shows rapidly, chaotically changing values (`$ff,$2f,$a9,$ff,$ff,...,$39,$39,...,$08,$08,...,$38,$1e,$dd,$27,$2d,$a9,...`).

**Directly compared the driver's own sequence against the simulator's LATER raw output and found an exact match, ~40-60 frames later**: the simulator's raw `frames[][7]/frames[][8]` at real frames 1275 onward reads `$38,$1e,$dd,$27,$2d,$a9,$08,$08,$8a,$85,$91,$12,$72,$9c,$ab,$ab,...` — **byte-for-byte identical** to what the driver was ALREADY outputting around comparison-index 1232-1247. The driver isn't producing garbage; it's genuinely playing the SAME real note-event content the simulator plays too — just roughly 40-60 frames too early, specifically during this held-note-with-active-fx-program stretch. This is a LOCALIZED timing desync (the driver's row cursor races ahead of the simulator's during a REST/sustain passage, not a global drift — the already-confirmed-clean `row_frame` map rules that out), most likely a row-DURATION/tick-count bug in the translator specific to held/sustain steps (as opposed to fresh note-on steps), though the exact mechanism (which function computes the wrong duration, and why only under an active fx program) is NOT yet root-caused.

## SHIPPED, next round: root cause found and fixed — a swallowed second arp byte silently drops real ticks, in BOTH Stage A and Stage B translators

Followed the "concrete next step" above via two parallel research agents (one on `bb_steps_for_voice`'s tick arithmetic, one on `steps_to_rows_native`'s row emission), plus independent direct data analysis of voice1's own decoded byte stream and `bb_steps_for_voice`'s own step list around this passage.

**First hypothesis tested and DISPROVEN (real, useful negative result)**: the `steps_to_rows_native` agent found a same-value command dedup (`cmd != cur_cmd` gating whether a row's fx/command byte gets emitted at all) structurally identical to the already-fixed B21 instrument-dedup bug, never extended to `cmd`. Implemented the analogous fix (unconditional `cmdcol = cmd`, matching B21's `inst = s.instrument`) — it DID change the emitted rows (every row now carries an explicit command), but the register-level comparison was **byte-for-byte identical** before and after on `Crank_Crank_Airwolf`, and caused a real **regression** on `Quintessence` (97.6%→95.9%) and `Trinket` (94.0%→93.0%) with zero benefit anywhere. Reverted immediately. Conclusion: real hardware genuinely treats a same-value fx reselect as a no-op (unlike instrument reselect, where B21's fix was correct) — forcing a redundant re-emit introduces artifacts that shouldn't exist. This rules the whole `cur_cmd` dedup mechanism OUT as a lead; do not revisit it without new evidence.

**The real root cause, found by the `bb_steps_for_voice` agent and directly verified by instrumenting `BlackbirdSim.real_frame()` frame-by-frame against `Crank_Crank_Airwolf` voice1's real byte stream (bytes 350-359 = `86 1d ca cb 86 1d ca 84 79 ...`)**: `prepare1` (`player.s`, ported in `blackbird_everyframe_sim.py`) consumes **at most one** fx-range (`$C9-$F8`) byte per real tick. At offset 352-353 there are two consecutive arp-classified bytes (`0xCA`, `0xCB`) with nothing between them. The second one is NOT consumed by `prepare1` again — it falls through `prepare2` (rejected, outside its `$80-$B7` range) to `prepare3`'s own `got_delay` path, which only tests `b >= 0x80` and has no way to distinguish a genuine delay byte from an orphaned fx byte. Real hardware swallows it as a phantom rest of `_delay_ticks(b)` ticks (the exact same formula a real delay byte uses), and its own fx value is discarded entirely — traced ground truth: this burns **5 full rows of total silence for voice1** (rows 152-156, `rpos` frozen, `pendnote` stuck at 14, `currfx` stuck at the FIRST arp byte's value 2, never advancing to the second's value 3), before normal fetching resumes at row 157/158. This is precisely the "pendnote=14 static for 7 rows" passage this investigation started from, and precisely why the earlier "driver races ahead" comparison found the driver's own output byte-for-byte matching the simulator's content ~40-60 frames later — the driver was skipping the 5 silent ticks the decoder failed to count.

Both `bb_steps_for_voice` (`bin/build_blackbird_native_song.py`) and its Stage A sibling `steps_for_voice` (`sidm2/blackbird_driver11.py`) had the IDENTICAL bug — their `arp` branch treated **every** arp byte as a free, 0-tick prefix regardless of how many had already been consumed since the last terminal, silently dropping the swallowed byte's ticks entirely (Stage A doesn't track fx values at all, but still needs the correct tick count, so the same fix applies there too, just without the fx-value bookkeeping).

**Fix**: track whether the current step accumulation has already consumed `prepare1`'s one-fx-byte-per-tick allowance (`arp_pending`, reset at every note/delay terminal). A second arp/oob byte before the next terminal is redirected into the exact same tick-duration logic the `delay` branch already uses (`ticks = _delay_ticks(b)`, emit a `rest`-or-`note` step per the existing `gate_is_off`/`had_note_yet` convention, do NOT update `cur_fx`) instead of being treated as a free continue.

**Result — full suite 1581 passed, zero regressions** (`Fargo`, the 11-file v1.2-exact bucket's flagship, unchanged at 99.7% — this shared code path is exercised by both buckets, and the swallow pattern evidently never fires in that bucket's own data). Targeted, measurable improvement exactly where predicted: `Crank_Crank_Airwolf`'s own frame-1000-1500 bin (precisely where this bug lives) improved `freq` 81.3%→**98.1%**, `pulse` 85.5%→**97.5%**; overall for the file 78.6%→80.2%. `To_Die_For_II` 82.6%→83.1%. `Fugue`/`Quintessence`/`Trinket` unchanged (no regression, consistent with this exact swallow pattern not occurring in their own data within the measured window).

**Still open**: `Crank_Crank_Airwolf`'s LATER frame ranges (1500-2824) remain degraded (freq 38-42%) even after this fix — a separate, still-uncharacterized issue (likely another instance of some divergence mechanism, not yet identified as the same or a different bug). `To_Die_For_II`/`Trinket` likely have analogous later-range residuals too, not yet checked individually. The concrete next step for a future round: re-run the same `BB_DIAG_LO`/`BB_DIAG_HI`/`BB_DIAG_REG` narrowing technique used to find THIS bug, but targeting the 1500-2000 frame range instead, to see whether it's the same arp-swallow pattern occurring again (in which case this fix should already have caught it — worth double-checking for a THIRD consecutive arp byte or an arp-then-oob combination not covered by the current fix) or a genuinely new mechanism.

## SHIPPED, next round: the SAME bug for `prepare2`'s range, a real cross-reset bug found in the fix itself, and a much bigger structural gap — voices whose real content ends before the song does

Picked up the "concrete next step" above via another background agent, directly instrumenting `BlackbirdSim.prepare2/3` frame-by-frame (same methodology as the arp fix). Confirmed it's the identical bug class, one range over: `prepare2` (player.s) also consumes AT MOST ONE `$80-$B7` byte (gate-off/legato/instrument) per real tick. A second consecutive prepare2-range byte falls through to `prepare3`'s own undiscriminating `b >= 0x80` test and is swallowed as a phantom rest — live-verified against `Crank_Crank_Airwolf` voice1 offsets 412-413 (`0x84,0x87`, two consecutive instrument-select bytes): only `0x84` applies, `0x87` is swallowed as a 9-tick hold instead of silently overwriting `pending_instr` for free. Fixed both `bb_steps_for_voice` and `steps_for_voice` with a second `prep2_pending` flag alongside `arp_pending`. **Result**: full suite 1581 passed, zero regressions, `Fargo` unchanged at 99.7%. Large further gain: `Crank_Crank_Airwolf` overall 80.2%→**88.3%**, with the 1500-2000 bin (untouched by the arp fix) jumping `freq` 38.2%→**86.3%**. `To_Die_For_II` 83.1%→84.6%. Others unchanged (no regression).

**Own-code review caught a real bug in the two swallow fixes themselves**: both the arp-swallow branch and the three prepare2-swallow branches reset only THEIR OWN pending flag on exit, not the other stage's — e.g. the arp branch's swallow path reset `arp_pending` but left `prep2_pending` untouched, so a step that ended via one swallow path could leave the OTHER stage's flag stale going into the next accumulation cycle. Found by tracing `Crank_Crank_Airwolf` voice1 offsets 441-443 (`arp, instrument, instrument` — an arp byte immediately followed by the SAME two-consecutive-instrument pattern the prepare2 fix targets) and confirming the swallow step it should produce (`ticks=12`) was correctly emitted, but the flag hygiene was still wrong for the general case. Fixed all 6 step-termination points in both files to reset BOTH flags. **Zero measured behavior change** on any of the 5 files (this specific ordering never happened to matter for their own data) — a genuine latent-bug fix confirmed safe, not a speculative one.

**The bigger finding, chasing `Crank_Crank_Airwolf`'s STILL-remaining residual past frame ~1790**: voice1's own step list, even fully fixed, sums to only 224 total ticks — but the song's overall span (driven by whichever voice has the MOST real ticks, `voice2` at 353) is 353. From tick 224 onward, `bb_steps_for_voice` (fed only `result.real(1)`, truncated at that voice's own individual freeze point) has NOTHING left to emit for voice1, while the validated simulator keeps processing voice1's own post-freeze `$c0` filler bytes — confirmed NOT inert (`$c0` falls in `prepare3`'s own `$B8-$C7` delay range, `_delay_ticks($c0)=16`, a genuine 16-tick rest per byte, not silence). The native driver's own sequencer apparently defaults to looping voice1's track back to its start once it runs out (explaining the earlier "driver plays content that matches EARLIER in the song" observation exactly) — a generic tracker behavior that doesn't match what THIS player's own filler mechanism actually does.

Tested the naive fix first (feed the FULL `result.voices[v]`, filler included, into `bb_steps_for_voice`) — **badly overshoots**: filler is effectively unbounded per voice (~2500 ticks measured across all 3 voices, vs. the ~353 the song actually needs), since `freeze_tail_frames=300` keeps producing filler for a long time after the genuine freeze. **Shipped fix instead**: after computing each voice's own real step list and the song's overall `span` (unchanged), extend ONLY voices whose real tick count falls short of `span`, appending filler-derived rest steps one at a time and stopping as soon as the running total reaches `span` — i.e. fill the specific gap, don't graft the whole filler tail on.

**Result — net positive across the bucket, one small localized regression**: full suite 1581 passed, zero regressions, `Fargo` unchanged at 99.7%. `Trinket` 94.0%→**96.2%** (`freq` 90.8%→**100%**), `To_Die_For_II` 84.6%→**87.6%** (`freq` 79.0%→**98.7%**), `Fugue_on_a_Theme_by_D_M_Hanlon` 84.9%→**88.0%** (`freq` 82.4%→**99.4%**), `Quintessence` unchanged 97.6% (its own real content already reaches the full span without needing extension). `Crank_Crank_Airwolf` itself regressed slightly, 88.3%→87.2% — `waveform`/`adsr` specifically, not `freq` — because the filler-derived rest steps correctly extend the TICK count but carry forward whatever `pending_instr`/`cur_fx` happened to be active when the real content ended, which doesn't fully replicate the simulator's own continued per-frame state evolution during filler. **Not reverted**: the net effect across the whole bucket is clearly positive (three large freq gains vs. one small file-specific dip), and this is architecturally the right fix (a voice's own filler genuinely isn't inert; the prior behavior of silently looping back was strictly further from correct).

**What's now open**: `Crank_Crank_Airwolf`'s own waveform/adsr regression during its filler-extension tail — refining WHICH instrument/fx state a filler-derived rest step should carry (matching the simulator's own continued evolution more precisely, rather than freezing at whatever was last active) is the concrete next lead, file-scoped rather than bucket-wide. Whether `To_Die_For_II`/`Trinket`/`Fugue` have any OTHER voices with a similar short-real-content gap (now handled) vs. a genuinely different remaining issue hasn't been individually re-diagnosed — their large freq gains suggest the filler-extension fix was likely their dominant remaining issue, but this is inferred from the aggregate number, not directly traced per-file the way `Crank_Crank_Airwolf` was.

## SHIPPED, next round: carry forward fx/gate state into filler-extension steps instead of resetting — helps 2 more files, but does NOT explain `Crank_Crank_Airwolf`'s own residual

Addressed the "what's now open" item directly: the filler-extension fix's own sub-call, `bb_steps_for_voice(filler_bytes)`, walked the filler tail as a completely FRESH, independent stream — resetting `cur_fx`/`pending_instr` to their own defaults (`0`/`None`) for every filler-derived step, rather than inheriting whatever was actually active when the voice's real content ended. Since filler is a uniform run of `$c0` bytes with no arp/instrument content of its own, it can never genuinely select a NEW instrument/fx on real hardware — it only ever extends what's already active. Replaced the fresh sub-walk with direct step synthesis: read the LAST real step's own `.fx` value and `rest`-vs-`note` kind, and use those (not defaults) for every filler-derived step.

**Result**: full suite 1581 passed, zero regressions, `Fargo` unchanged at 99.7%. Genuine further gains on the two files that ALREADY benefited most from the filler-extension fix: `Fugue_on_a_Theme_by_D_M_Hanlon` 88.0%→**90.3%** (`waveform` 53.4%→**72.4%**), `Trinket` 96.2%→**96.9%** (`waveform` 84.9%→**91.3%**). `Quintessence`/`To_Die_For_II` unchanged (no regression).

**`Crank_Crank_Airwolf` itself: completely unchanged, 87.2%→87.2%, byte-for-byte identical register comparison output.** This is a real, informative negative result, not a bug in the fix — traced directly: at real frame 2000 (well within voice1's filler-extension zone, which now correctly spans rows 224-368 vs. the song's own 353-row span), the driver's own pulse-width registers show a clear, decrementing SWEEP pattern (`$ad,$9d,$8d,...`) while the simulator holds a flat, constant value. This is NOT explained by the fx/instrument-value carry-forward this round fixed — it's very likely the native driver continuing to step forward an ALREADY-TRIGGERED pulse-width envelope program belonging to whichever instrument was active when the real content ended (a genuine Blackbird instrument feature, `everyframe()`'s wave/pulse stepping, which runs every real frame independent of note events) — while the SIMULATOR's own filler-driven behavior apparently does NOT continue that same envelope the same way. Distinguishing "the driver's sweep is wrong" from "the simulator's flat value is itself just what THIS instrument's envelope naturally settles into" needs actual register-level tracing of the relevant wave/pulse table, not step-list-level reasoning — a materially different, and likely larger, investigation than anything in this session's arc so far (it would mean modelling the wave/pulse ENGINE's own continued evolution through the filler region, not just deciding what step metadata to emit). **Deliberately not chased further this round** — flagged as the concrete next lead for `Crank_Crank_Airwolf` specifically, file-scoped, not assumed to generalize to the other 4 files without direct evidence.

**Session-so-far cumulative `Crank_Crank_Airwolf` trajectory**: 66.8% → 78.6% (loop fix) → 80.2% (arp-swallow fix) → 88.3% (prep2-swallow fix) → 87.2% (filler-extension fix, small net dip, architecturally correct) → 87.2% (this round's state-carryover refinement, no change — confirms the remaining gap is a genuinely different, wave/pulse-engine-level issue, not step metadata).

## Narrowed further, same round: the pulse sweep is chunk-count-independent — points at `everyframe()`'s own wave/pulse accumulator stepping, a runtime-engine question, not a translation one

Ran one more cheap, targeted experiment before stopping: replaced the filler extension's multiple 16-tick chunks with a SINGLE step covering the entire gap in one row (`ticks = span - voice_ticks`), to test whether the sweep was an artifact of repeatedly re-triggering `SF2_GATE_ON` every 16 ticks. **Result: byte-for-byte identical register output either way** — same continuous, unbroken decrement (`$ad→$9d→$8d→...→$0d→$fc→$ec→...`, wrapping roughly every 17 frames, losing ~`$10` per lap). This rules out row-count/re-triggering as the mechanism; whatever's driving the sweep does so continuously regardless of how many (or how few) native driver rows represent the hold. Reverted the experiment (no change to the shipped multi-chunk version, which remains the more granular, "Tick model"-consistent representation).

**Sharper hypothesis**: `unroll_wave_pulse`'s own translated WAVE table (B9's design, per its docstring above) stores per-frame pulse DELTAS, not absolute output values, specifically because Blackbird's real pulse accumulator drifts a nonzero amount every lap around its own (typically short, cyclic) source wave program — matching the exact "wraps roughly every 17 frames, ~$10 lower each time" pattern observed here almost perfectly. This STRONGLY suggests the translated table and the native driver ARE faithfully reproducing this instrument's own pulse-accumulator drift — i.e. the DRIVER's sweep may not be wrong at all. The mismatch is more likely that `BlackbirdSim.everyframe()` (the ground-truth simulator) is NOT continuing to step this SAME accumulator during the filler-extension window the way a genuine in-song note-hold would — e.g. real hardware's wave/pulse engine may gate its own per-frame stepping on some condition (`wavemask`, a specific `v.timer` state, or something filler-specific) that a normal held note satisfies but this translator's synthetic `SF2_GATE_ON`-during-filler representation does not correctly mirror.

**Deliberately not investigated further this round** — resolving this needs tracing `BlackbirdSim.everyframe()`'s own wave/pulse-stepping gate condition directly (not step-list-level reasoning, which is now exhausted for this specific residual), to determine whether it's genuinely inert during real hardware's own filler processing, and if so, why, and how a translated row should represent "hold, but do NOT continue drifting the pulse accumulator" if that's what's needed. This is a runtime-engine question, structurally different from every fix shipped this session (which were all about which/how many rows to emit, not about the wave/pulse engine's own per-frame behavior) — a fitting, well-scoped starting point for a dedicated future round, not a same-session extension.

## Correction, same round: the "driver may be correct, simulator may be gating incorrectly" hypothesis above is WRONG — retracted. Root cause narrowed to the compiled driver ASM specifically, self-check extended to prove it, exact bug not yet located

Went ahead and traced `everyframe()`'s own wave/pulse-stepping directly, per the "deliberately not investigated" note above. **First pass at this had a real bug of its own** — an initial standalone re-simulation (fresh `v.pwidth=0`, not seeded from the real state at the hold boundary) appeared to show the wave program's own self-loop row genuinely accumulating a drifting pulse delta every visit, seemingly CONFIRMING the previous section's hypothesis that the driver's sweep might be correct and the simulator wrong. **This was a measurement artifact, caught by re-running the SAME simulation properly seeded** (`v.pwidth=78`, matching the actual value at the hold boundary per `row_state`): with correct seeding, the self-loop row (`wavepos=21`, confirmed identical to the real full-song run's own `row_state`) shows `everyframe()` NEVER writing to `$D402`/`$D403` at all — `sim.regs[2]/sim.regs[3]` stay completely untouched, frame after frame, for 30 consecutive frames tested. **The ground truth genuinely is flat — the earlier session's own hypothesis is retracted.**

**Extended `unroll_wave_pulse`'s own existing B9 self-check to prove the Python-side translation is sound.** The existing self-check (comment: "exact-by-construction rather than exact-by-hope") only ever validated frames `0..cyc_end-1` — the FIRST pass through a captured wave/pulse program, before its own trailing `$7F` loop-back marker's jump ever takes effect. Any sufficiently long hold DOES exercise that loop-back, and this was never directly checked. Added a second validation pass, wrapping the replay index back to `cyc_start` once it passes `cyc_end` (exactly what the `$7F` row's own jump is meant to do), checked against the REST of the already-captured 300-frame simulator window (no extra simulation needed) — pure additional validation, doesn't touch table-construction logic or build output. **Result: passes cleanly across the full corpus** (all 5 REPEAT=1 files, plus `Fargo`/`Glyptodont` from the 11-file bucket), full suite still 1581 passed. This conclusively proves the Python-side model — "replaying the captured program with a wrap back to `cyc_start` reproduces the simulator's own continued output exactly" — is semantically correct. **Shipped**: this is a permanent, real improvement to the self-check regardless of whether the underlying driver bug gets found, since it closes a genuine blind spot that could have hidden other loop-back bugs on other files.

**With the Python model proven sound, read the actual driver ASM and the table-absolutization code that assembles it into the compiled SF2, looking for the bug directly.** `wave_step`'s own `$7F`-chase loop (`drivers_src/blackbird/blackbird_driver.asm`, labeled `ws_read`/`ws_have`/`ws_nopulse`) and `gen_includes_song`'s own absolute-offset conversion (`edit[wo + 1*256 + start + r] = (start + c1) if c0 == 0x7f else c1`) were both read line-by-line and traced by hand against this specific instrument's own row values (`cyc_start=2`, `cyc_end=3`) — **both appear structurally correct**: the ASM's row-advance logic (`ws_row+1` stored back into `VWI,x`) correctly re-arms the `$7F` marker for the next frame, which correctly re-chases back to the same self-loop row every time; the table-absolutization correctly folds the per-instrument base offset into the jump target. No bug was found by manual reading alone.

**What this means**: the remaining gap is real, narrow, and specifically located — SOMEWHERE in the compiled driver's actual runtime behavior around a `$7F` loop-back on a NO-PULSE self-loop row, or in some interaction not visible from static code reading (e.g. the driver's own `VWI,x` PRIMING value for a part-boundary resume, `do_init`'s handling of this specific state, or a genuinely subtle 6502 carry/flag issue in `wave_step` that a hand-trace missed). Confirming it further needs binary-level inspection of the actual compiled SF2's own WAVE table bytes for this instrument, or a live 6502 trace of `wave_step` during the extended-hold window (RetroDebugger, matching this session's earlier live-hardware rounds) — a genuinely different verification method than any Python-level reasoning can provide at this point. **Deliberately stopping here** — this is now precisely enough scoped that a future session (or a live-tracing round) can pick it up directly, without re-deriving any of the above.

## Binary-level inspection completed: WAVE table bytes and `wave_step`'s per-row ASM logic are BOTH provably correct in isolation — the sweep can't come from either, redirecting suspicion toward a state/indexing mismatch instead

Picked up the "binary-level inspection" option above. Added temporary `os.environ`-gated debug prints (removed again after use, not shipped) to `bin/build_blackbird_native_song.py`'s `gen_includes_song`, rebuilt `Crank_Crank_Airwolf` fresh, and traced the exact identity of the divergent instrument through the CURRENT single-part compiled build (not assumed from an earlier session's numbers, which risked being stale — `layout.inc` on disk had in fact been overwritten by a LATER file in an earlier verification loop, confirmed via `ls -la --time-style=full-iso`, a genuine near-miss avoided by rebuilding fresh rather than trusting the on-disk artifact).

Confirmed: raw instrument index 1 (0-based; the same instrument identified earlier via `sim0.ins_wave(1)=19`) maps to driver slot 1 (identity remap for this file) and gets packed into the compiled WAVE table starting at absolute row 11 (`wave_addr=$3800`, `wo=$1E00` edit-area offset). Read the actual bytes straight out of the compiled `.sf2` (file offset = `2 + (WAVE_ADDR - LOAD_BASE)`, since this driver's `.sf2` output is just a raw wrapped C64 PRG — 2-byte load address + payload from `$0D7E`, no separate container header) at rows 9-15:

```
row 11: c0=0x21 c1=0x00   (real data)
row 12: c0=0x10 c1=0x00   (real data)
row 13: c0=0x10 c1=0x00   (real data — the eventual self-loop landing row, matches "wavepos=21" in source numbering: 19+2)
row 14: c0=0x7f c1=0x0d   ($7F jump marker, c1=13=start+cyc_start(2) — absolute target, exactly as predicted)
```

Byte-for-byte matches what `gen_includes_song`'s own construction code + the B9 self-check both predict. **The compiled table itself is not the bug.**

Then hand-traced `wave_step` (`drivers_src/blackbird/blackbird_driver.asm:469-556`) against these exact bytes, frame by frame, once truly in the row13/row14 oscillation: `ws_read` at row14 sees `c0=$7F`, chases to row13 (`c1=13`), `ws_have` resolves `ws_row=13`, writes `WAVE[13]=0x10` (masked) to `$D404`, `asl` on `0x10` sets bit7 from the original byte's bit6 — **bit6 of `0x10` is 0, so `bpl ws_nopulse` is taken: `$D402`/`$D403` are never touched.** `ws_nopulse` advances `VWI,x` to 14 and the next frame repeats identically forever. Checked rows 11/12 too (the two other real rows of this program) — `0x21` and `0x10` both also have bit6 clear, so **no row in this instrument's entire wave program ever sets the pulse-write bit.** `PW_ACC`/`$D402`/`$D403` are therefore providably static once this loop is entered — `wave_step` has no code path that can produce an ongoing sweep here.

Cross-checked the rest of the driver for any OTHER writer of `$D402`/`$D403`: the only other site is `do_init`'s one-time `PRIME_D402_TAB`/`PRIME_D403` priming write at song start (`blackbird_driver.asm:351-368`, explicitly commented "a voice without pulse rows keeps it forever") — a single write at frame 0, not a per-frame one. **No code in this driver can write `$D402`/`$D403` more than once for a voice stuck on this specific wave program.**

This is a genuine contradiction with the diagnostic evidence (`BB_DIAG_REG` showing a continuing, wrapping decrement at real frame ~2000+) and rules out the two most obvious remaining hypotheses (bad table bytes; a subtle bug in `wave_step`'s own per-row logic) at the same time. **The likely remaining explanations, un-investigated**: (a) the compiled build's own real-frame numbering has an offset/skew relative to the "real frame ~2000" label used when diagnosing this (the B15-era note about parts being "sliced at real-frame offset 3" is a candidate mechanism, not yet checked against this specific case), meaning the actual divergent row/instrument at THAT frame may not be the one traced above; or (b) a voice-index mismatch specific to the filler-extension code path — i.e., voice1's `PW_ACC,x`/`VWI,x` state or `sidbase,x` addressing crossing with a DIFFERENT voice's (one whose wave program genuinely does sweep) somewhere in the filler-extension machinery, which is the newest and least-tested code path touching this exact region. Neither has been checked yet. **Ruled out and not worth re-checking**: filler steps setting `s.instrument=None` — confirmed via `D11Row`'s own docstring (`sidm2/galway_to_driver11.py:74`) that `instrument=None` correctly means "no reselect, packer emits nothing," which is the CORRECT behavior for a filler continuation, not a bug.

No code changed this round (debug prints added and removed, tree confirmed clean via `git diff --stat`); full suite reconfirmed 1581 passed / 7 skipped / 2 xfailed, `Crank_Crank_Airwolf` reconfirmed unchanged at 87.2% overall (both counts identical to the prior round's own baseline).

## B23 SHIPPED, same round: ROOT CAUSE FOUND — `result.real(v)` truncates mid-instrument-select at the exact real/filler boundary, silently dropping a dangling pending select. `Crank_Crank_Airwolf` 87.2%→**97.5%** (freq/pulse both →100%)

Picked hypothesis (b) from the section above (a state/indexing issue in the filler-extension code path) over live RetroDebugger tracing — a headless py65 instrumentation of the ALREADY-BUILT driver binary (temporary debug prints, removed after use) was cheaper to iterate on and gave a definitive answer directly.

**First, dumped voice1's actual `VWI,x`/`PW_ACC,x`/`$D404`/pulse registers frame-by-frame from a live py65 CPU trace of the compiled driver, alongside the ground-truth simulator's own register snapshot for the SAME frames.** This immediately overturned the assumption the WHOLE prior investigation (this section and everything above it) was built on: `VWI` sits constant at row **22** throughout the divergent window, not row 13/14 (the "instrument 1" self-loop this session and prior ones traced). `$D404` reads `0x40` (bit6 SET — a genuine pulse row) on the driver side, while the simulator's own register snapshot for the identical frames reads waveform `0x10` (bit6 clear, flat) with a static pulse value `0xea` — **the driver isn't even executing the same wave-table row as the simulator expects; it's a completely different instrument's program.** Dumping the compiled WAVE table's instrument-slot ownership confirmed row 22 belongs to a DIFFERENT instrument (compiled slot 4, `wave_start=18`, rows 18-22, two of which are genuine pulse-accumulate rows — exactly matching the observed sweep) than the "instrument 1" (compiled slot 1, rows 11-14) this whole investigation had been chasing since the very first hand-derived hypothesis, several sessions ago, that was never independently re-verified against the actual decoded byte stream.

**Traced why**: queried `bb_steps_for_voice(result.real(1))`'s own last few steps directly — its LAST real step selects raw instrument **4** (0-based), not instrument 1. Cross-referenced the validated simulator's `row_state` for the identical real-frame window: `currins=2` (1-based == instrument 1 0-based), constant throughout — a direct, first-class contradiction between the two independent decoders that share the same byte stream. Dumping voice1's raw stream bytes with `classify_byte` found the cause immediately: `result.real(1)` (463 bytes) ends on byte `0x84` — a genuine instrument-select byte (raw 0-based index 1) — with **nothing after it** to close it out, since `bb_steps_for_voice`'s "attach pending_instr to whatever comes next" mechanism (the same one B12's fix relies on) needs a following note/delay/terminal byte that simply isn't there — the loop ends mid-tick. The very next byte in the FULL (real+filler) stream, confirmed directly, is `0xc0` — the first genuine filler byte, always classified `delay`. On real hardware this is exactly the byte that commits the dangling select (same mechanism as every other prepare1/2 "attach to next event" case already fixed this arc) — but `bb_steps_for_voice` never sees it, because `result.real(v)` truncates the array exactly at the boundary. The filler-extension code (B19, shipped 2 rounds ago) then carries forward `steps_per_voice[v][-1]`'s instrument (the last COMPLETE step: instrument 4, from two events earlier) instead of the instrument the dangling select should have produced (instrument 1) — a silent, structural gap in how the real/filler split interacts with prepare2's cross-tick pending state, invisible to every check this arc had run (the self-check only validates wave/pulse TABLE construction, not which instrument gets selected).

**Fix** (`bin/build_blackbird_native_song.py::main`, B23): decode `result.real(v)` plus exactly one lookahead byte from `result.voices[v]` (the full stream) whenever filler exists for that voice — always a genuine `$c0` delay byte, which correctly closes out ANY dangling pending state (instrument select, or in principle a dangling arp/fx select too, via the identical mechanism) via `bb_steps_for_voice`'s own existing terminal-handling logic, no new code path needed. The filler-extension loop below is unmodified: the lookahead byte's own step just counts toward `voice_ticks` like any other real step, so the loop fills whatever gap remains exactly as before.

**Verified**: full 7-file sweep (5 REPEAT=1 + `Fargo` + `Glyptodont` from the 11-file bucket), zero regressions, several genuine additional wins:
- `Crank_Crank_Airwolf`: 87.2%→**97.5%** (freq 87.8%→**100.0%**, pulse 87.9%→**100.0%**, adsr 81.1%→93.5%) — this was the file the whole multi-round residual chase was scoped around.
- `Trinket`: 96.9%→97.1% (small further gain)
- `To_Die_For_II`: 87.6%→**90.7%**
- `Fugue_on_a_Theme_by_D_M_Hanlon`: 90.3%→90.3% (byte-identical — never had a dangling select at its own real/filler boundary)
- `Quintessence`: 97.6%→97.6% (byte-identical — never needed filler extension at all)
- `Fargo`: 99.7%→99.7% (byte-identical, confirmed via a temporary `git stash` A/B — the 11-file bucket's own canary, unaffected)
- `Glyptodont`: 96.7%→96.7% (byte-identical, confirmed the same way)

Full suite: 1581 passed / 7 skipped / 2 xfailed, unchanged.

**Lesson for future rounds**: the "instrument 1 / wavepos 21" identification this entire investigation was built on (introduced early in this multi-session arc, cited and re-used across many subsequent rounds without independent re-verification) was simply wrong — a stale assumption nobody re-checked directly against the decoded byte stream until this round, despite extensive, careful, and individually-correct work being done ON TOP of it (the WAVE table construction, `wave_step`'s ASM, the B9 self-check extension). All of that work was real and still stands as valid — it just wasn't investigating the actual bug, because it was aimed at the wrong instrument. When a residual survives several rounds of increasingly rigorous verification, checking the FOUNDATIONAL identification (which instrument/row/byte is even in question) directly against ground truth is worth doing before extending the verification further in the same direction.

## Next residual found, same round (later SHIPPED as B24 — see below): `To_Die_For_II`'s own filter (79.1%) traces to a PRE-EXISTING, already-documented `unroll_filter` limitation whose real-world impact was previously underestimated

With B23 shipped, re-binned `To_Die_For_II`'s own remaining residual (`BB_DIAG_BIN=200`) and found a sharp, permanent break: `$D418` (mode/vol) goes from 100% match to a hard, constant 0% (`sim=$0f` vs `drv=$1f`, unchanging) starting at real frame ~1000 and never recovering for the rest of the song — exactly the shape of a one-time event whose wrong effect then holds forever, not a transient glitch.

Traced the trigger precisely via `row_state`: at row 249 (frame 995), `zp_filtpos` repositions 11→18 — a new filter-owning instrument's note-on. Raw source instrument 8 (and 10) both carry `filt_start=18`. Read the SOURCE filter table directly at that position (`sim.filttable(18..21)` = `0x0f, 0x00, 0xc0, 0xff`): the simulator's own filter engine (`BlackbirdSim.everyframe()`, lines ~495-520) does NOT use the compiled driver's row-based SET/ADD scheme at all for the source data — it's a completely different model: `$D418`/`$D417` are written DIRECTLY, unconditionally, every frame, straight from `filttable(y)`/`filttable(y+1)` (no bit-classification, no accumulation), while only the 3rd/4th columns (`filttable(y+2)`, cutoff; `filttable(y+3)`, advance) have SET/ADD-style bit7 logic. At position 18 this is a permanent "hold" (row3=`0xff`, self-referencing — `zp_filtpos` never advances past 18), so `filttable(18)=0x0f` (mode nibble = 0, i.e., **no SID filter type bit set — the filter is genuinely off**) applies for the rest of the song.

`unroll_filter` (this file, ~line 859) correctly walks the simulator to capture this AS a `d418=0x0f` SET-row observation (`(d418_0 >> 4) & 0x07` correctly computes mode=0) — but the COMPILED DRIVER's OWN row-byte encoding (`_filter_set_row`, ~line 800) **cannot represent mode 0 at all**: the ASM's own `fp_dec` dispatch (`cmp #$90; bcs fp_set`) requires byte0's top nibble ≥ 9 to route to the SET handler; `_filter_set_row` forces bit7 of byte0 to 1 for exactly this reason, but mode=0 combined with the forced bit7 only reaches top-nibble=8, still below the 9 threshold, which would misroute to the ADD handler and corrupt the cutoff interpretation entirely. **This exact limitation is already documented in `_filter_set_row`'s own docstring** from an earlier round: `"mode=0 has NO valid encoding in this row shape... clamped up to 1 (documented residual, not expected to matter: mode 0 means 'no SID filter type bit set', audibly close to leaving the passband inert)"`. That assumption was reasonable in general but is simply wrong for this specific file: position 18's hold-row means the clamp's wrong mode bit (1 = lowpass, instead of the true 0 = off) persists for the ENTIRE remainder of the song once triggered, not a brief, inert-sounding blip — dragging `To_Die_For_II`'s own filter category down to 79.1% overall / as low as 52-58% in later bins.

**Not fixed this round, deliberately**: a real fix needs to teach the compiled driver's row format to represent mode=0 — e.g. widening `fp_dec`'s classification (its `cmp #$90` threshold, or an entirely new discriminator) to admit a genuine "SET, mode=0" row shape. `blackbird_driver.asm` is the ONE shared driver across both the 11-file v1.2-exact bucket and this 5-file REPEAT=1 bucket — `Fargo` sits at 99.7%/100% filter today, so ANY change to `fp_dec`'s dispatch logic needs careful, deliberate design and a full re-verification sweep across BOTH buckets before shipping, not a same-round patch bolted onto an already-large win. Whether `Fargo`/`Glyptodont`/the other 9 v1.2-exact files ever actually hit a genuine mode=0 SET row in their own filter data is unchecked — worth confirming before attempting the ASM change, since if they never do, the fix is purely additive risk-wise for them.

## B24 SHIPPED, next round: the deferred mode=0 fix above — scanned the risk first, found it was near-zero, shipped a 2-line ASM change

Before attempting the ASM change, scanned all 16 relevant files (11 v1.2-exact + 5 REPEAT=1) for whether any of them genuinely hit a mode=0 SET row in their own filter data (replaying `unroll_filter`'s own simulator walk directly, checking `(d418 >> 4) & 0x07 == 0` at every SET-classified frame before the existing clamp would apply). Result: **`Fargo` — the flagship canary, currently at 100% filter — has ZERO mode=0 rows**, so the fix cannot touch its current perfect score at all. Three OTHER previously-uninvestigated v1.2-exact-bucket files (`Glyptodont`, `Dishwasher_Groove`, `Into_the_Unknown`) DO have mode=0 rows and were already silently paying this same clamp cost, unconfirmed until now (B15 only ever verified these built cold, never individually root-caused their own residuals).

**The fix turned out to be minimal, not the "entirely new discriminator" originally anticipated.** `_filter_add_row`'s own `b0 = (delta>>4)&0x0F` can only ever produce byte0 in `[$00,$0F]`; `_filter_set_row`'s forced-bit7 encoding only ever produces `[$90,$FF]` (mode 1-7) today — the whole `[$10,$8F]` range is genuinely dead space, never emitted by anything. Widening `fp_dec`'s own dispatch threshold from `cmp #$90` to `cmp #$80` (bit7 alone, instead of "top nibble ≥ 9") lets mode=0's own natural encoding (`0x80 | (0<<4) | cutoff_hi` = `[$80,$8F]`) route to `fp_set` correctly — `fp_set`'s own mode-extraction (`lsr x4; and #7`) already handles a top-nibble-8 byte0 correctly (`8>>0 & 7 = 0`) with **zero further ASM changes needed**. Paired with removing `_filter_set_row`'s own `if m==0: m=1` clamp (2 lines), this is the entire fix — `bin/build_blackbird_native_song.py::_filter_set_row` + `drivers_src/blackbird/blackbird_driver.asm::fp_dec`, one line changed in each.

**Verified across the full 16-file corpus**, zero regressions:
- `Fargo`: 99.7%/100% filter, **byte-identical** (confirmed via a `git stash` A/B) — exactly as the pre-check predicted.
- `Glyptodont`: filter 96.9%→**99.9%**, overall 96.7%→97.1%.
- `Dishwasher_Groove`: filter →**100.0%** (previously unmeasured individually).
- `Into_the_Unknown`: filter →**99.9-100.0%** across all 3 parts (previously unmeasured individually).
- `To_Die_For_II`: filter 79.1%→**88.9%**, overall 90.7%→92.3% — binned diagnostic confirms the fix closes the gap cleanly through frame ~1400 (100% filter match in every bin up to there), but a SEPARATE, later residual remains (pulse drops to 66.7% and filter dips to 52-70% again starting ~frame 1600) — a different, unrelated bug, not yet investigated.
- The other 7 v1.2-exact files with no mode=0 rows (`Dithered_Island`, `Elvendance`, `Euclid_Was_Here`, `Maple_Leaf_Rag`, `Revolutions_Delivered`, `Thus_Spoke_the_PC_Speaker`, `Toy_Rocket`) and the 4 remaining REPEAT=1 files (`Crank_Crank_Airwolf`, `Trinket`, `Fugue`, `Quintessence`): all **byte-identical**, confirmed both directly and via a `git stash` A/B spot-check on `Euclid_Was_Here`/`Revolutions_Delivered`.

Full suite: 1581 passed / 7 skipped / 2 xfailed, unchanged — this is the highest-stakes verification in the arc so far (a shared-ASM change touching both buckets), and it came through completely clean.

**Not yet investigated**: `To_Die_For_II`'s own later residual (frame ~1600+, pulse/filter); whether the 3 newly-improved v1.2-exact files (`Glyptodont`/`Dishwasher_Groove`/`Into_the_Unknown`) have further residuals of their own now that this specific cause is closed; whether B23's real/filler-boundary class of bug or this B24 mode=0 class has siblings elsewhere in the driver (e.g. an analogous PULSE-table encoding gap, unchecked).
