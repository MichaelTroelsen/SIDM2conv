...
## MAJOR finding (same day, 2026-07-22): the reported "low fidelity" for at least one file is a measurement-window artifact, not a driver bug — real fidelity likely much higher across the bucket
...
**Root cause: the comparison window is simply longer than one meaningful pass through the song, for short REPEAT=1 tunes.** `Crank_Crank_Airwolf`'s real content spans 441 tick-rows; its `$D400` (voice0 freq-lo) register trace, when compared against ITSELF at different offsets, proves the compiled native driver **loops back to the start of its own sequence with a period of exactly 984 real frames** (`drv[200:210] == drv[1184:1194]` byte-for-byte) — completely ordinary Driver11-family/SF2 playback behavior once a track's sequence data runs out, nothing wrong with the driver at all. The validated Python simulator (`BlackbirdSim`, the fidelity oracle), by contrast, has **no loop-rewind modelling whatsoever** (its own `execute()` explicitly no-ops the REPEAT `eos` OOB bit: `if bit: pass  # eos -- not modelled`) — once past the song's real note content it just holds/rests forever on `$c0` filler bytes. Comparing a driver that's replaying the song's own opening against a simulator that's sitting silent was never going to match, **regardless of whether the driver has any real bug** — it's an apples-to-oranges comparison past the intended window, not evidence of anything wrong.

**Confirmed by direct re-measurement**: re-running the SAME comparison but capped to exactly one loop period (0-984 frames instead of the default 3000) gives `Crank_Crank_Airwolf` **96.6% overall, freq=100.0%** — matching its own "primary window (200f)" figure (96.7%) almost exactly, and nothing like the previously-reported 66.8%/53.5%. **The originally-reported low fidelity for this file was almost entirely a measurement artifact, not a real driver deficiency.**

**This does NOT generalize cleanly to every file, though** — checked next: `To_Die_For_II` (the bucket's other lowest scorer, 62.1%) does NOT have the same explanation. Its song is much longer (3255 tick-rows, ~13,000+ real frames for one pass at its tempo) — the 3000-frame comparison window doesn't even reach a loop point for it at all. Its `$D400` trace shows BOTH the simulator and the driver legitimately holding the same note for 300+ frames at a stretch (confirmed: both sides show only 1 distinct value across most 100-frame windows — genuine sustained-note music, not a stall in either), yet the HELD VALUES themselves differ between sim and drv — a real, different, as-yet-uninvestigated bug, not the loop-window artifact.

**A further, deeper, NOT-YET-UNDERSTOOD puzzle surfaced while checking this**: the full-song simulator's own `row_frame` row→real-frame mapping (`run_full_song_sim()`) only reached row 307 of `Crank_Crank_Airwolf`'s own 441-row span within its entire 20,000-real-frame simulation budget — and the real-frame gap between successive tracked rows GROWS dramatically over time (the last 5 tracked rows span frames 19267→19923, ~150+ real frames each, vs. roughly 8 frames/tick expected from the tempo chain). This is why the build script's own "full-part window" report shows a huge, misleading `part_span_frames` (19997f) instead of something close to the real 984-frame loop period — its own fallback (`row_frame[row1] if row1 < len(row_frame) else len(full_frames)`) silently substitutes the ENTIRE 20000-frame budget once `row_frame` runs out of tracked entries. **Root cause not found this round** — plausibly related to the simulator's own `execute()` not modelling REPEAT's loop-rewind (so once past real content, its own internal tempo/OOB bookkeeping may behave unexpectedly), but this wasn't confirmed. **Deliberately NOT fixed this round** — a robust fix (e.g. self-repetition detection on the driver's own compiled output, used to derive a correct comparison window per file) is a real, scoped, but non-trivial methodology change to the shared build script's reporting logic, which would change headline fidelity numbers for potentially all 16 files across both buckets and deserves careful validation before shipping, not a speculative same-session patch.

**Practical implication for anyone picking this up next**: treat EVERY currently-reported "full-part"/"steady-state"/"WEIGHTED AVERAGE" fidelity number for this 5-file bucket as a **lower bound, not a true measurement** — the real fidelity is very likely much closer to each file's own "primary window (first 200f)" figure (96.7%/97.6%/99.8%/98.3%/99.2% for the 5 files respectively) for however many real frames actually constitute "one pass" through that file's own content, which needs to be measured per-file (e.g. via the same self-comparison technique used here: find where a register trace starts repeating itself) rather than assumed. This reframes the whole "fidelity refinement" priority: the earlier "freq/waveform/adsr/pulse all need work" framing was itself downstream of a broken measurement for at least one (and probably more) of the 5 files.
