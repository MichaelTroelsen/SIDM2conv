...
**NOT yet working: `decode_streams`** (the note-stream decompressor) throws on all 5 REPEAT=1 files — genuinely open, not root-caused. Diagnosed this far: the crash is early (the 7th `_emit_piece` call — voice 1's SECOND piece, right after the initial 2-call priming + one round of per-voice refills) and structural, not a rare deep edge case. The failing piece is a genuine `copy` (`ctrl=0xa8`, top5 nonzero), decoding to `dist=19` while voice 1 has only `L=9` bytes decoded so far — an out-of-range back-reference (`src=L-dist=-10`), not an off-by-one. `streamstart` itself is independently confirmed correct (read directly off `initroutine`'s own `LDA #<streamstart` / `LDA #>streamstart` immediate operands in the actual file, matching `_extract_streamstart`'s value exactly) and `initroutine`'s own bytes are untouched by REPEAT (confirmed: `init_template_mismatch=False`, and REPEAT's only INIT-segment effect is the segment-name choice, no byte content change) — so this isn't a locate/streamstart bug bleeding into decode. The bug is somewhere in `decode_streams`'/`_emit_piece`'s own pipeline simulation, exposed by variant-A's specific data (an early large back-reference this pattern apparently never triggers on the 11 v1.2-exact files). **Recommended next step**: a live RetroDebugger/zig64 CPU trace of `unpackvoice` on one of these 5 files (matching this module's own original verification method against Fargo/Glyptodont — see "Compression — SOLVED" below) to get real ground-truth register/pointer state at the exact tick this diverges, rather than more static code reading.
