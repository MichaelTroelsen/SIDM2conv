...
**Deliberately not chased further this round** — root-causing this precisely (which of `bb_steps_for_voice`'s tick-duration arithmetic, `steps_to_rows_native`'s row emission, or the fx-engine's own per-frame stepping is actually responsible) is comparable in scope to a fresh B12-B22-style investigation and warrants its own dedicated round rather than a tail-end addition to an already-long session. **What's ruled out, concretely, so a future session doesn't re-check it**: this is NOT the row/frame timing-drift anomaly (fully fixed, verified with `total_rows=353` matching the real build exactly); the note-content itself is correct on both sides (same bytes, just misaligned in time); `verify_fx_engine()`'s own existing self-check (49,500 frame comparisons, 100% exact) evidently doesn't exercise a note held this long with an active fx program, since it didn't catch this. **Concrete next step**: find the specific translated row(s) covering `Crank_Crank_Airwolf` voice1's rows 151-157 in the native driver's own output and compare their encoded tick/duration values directly against `bb_steps_for_voice`'s own computed `ticks` for the same steps — the discrepancy should be locatable as a straightforward row-by-row comparison, not another live-hardware round.
