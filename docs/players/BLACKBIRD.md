...
## Next residual found, same round (NOT fixed — needs a shared-ASM change, deliberately deferred): `To_Die_For_II`'s own filter (79.1%) traces to a PRE-EXISTING, already-documented `unroll_filter` limitation whose real-world impact was previously underestimated
