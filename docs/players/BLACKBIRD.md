...
| **v1.2 + REPEAT=1** ("variant A", RESOLVED 2026-07-22) | 5 | Crank_Crank_Airwolf, Fugue_on_a_Theme_by_D_M_Hanlon, Quintessence, To_Die_For_II, Trinket | **NOT a different tool version** — the SAME v1.2 source, compiled with player.s's `#if REPEAT` flag (loop-on-end support) — see "REPEAT=1 locate support" below. **Locate AND `decode_streams` both fully solved, same day**: all 5 files decode cleanly to a genuine `frozen=True` end-of-stream, zero crashes. Stage A/B (native driver) not started yet for this bucket |
...
**Not done yet**: Stage A (Driver 11 transpile) and Stage B (native driver) — both still completely unstarted for this 5-file bucket, same scope caveat as before. `Crank_Crank_Revolution` ("variant A'") still unlocated. The older "variant B" bucket (10 files) still untouched.
