...
| `retro_load(path)` | Load a PRG/SID/XEX/ROM file — starts running immediately. **Does NOT correctly load `.sf2` (SID Factory II) files** — confirmed on both a Stage A build and the known-good stock template: both produce identical garbage bytes at the driver's expected entry point, while the same file loads correctly through `pyscript/sf2_viewer_core.py`'s own parser. Workaround: copy/rename the file to a `.prg` extension before `retro_load`-ing it — this loads correctly (real, disassemblable driver code at the right addresses). Even once loaded, actually *starting* playback via a raw `retro_cpu_jump` to the `SYS` entry point is unreliable (bypasses BASIC's own `SYS` call setup, so periodic-IRQ-driven playback may never start) — for real audio verification of an SF2 file, use the project's own `pyscript/sf2_open_in_editor.py` (launches the real SID Factory II editor) instead of RetroDebugger. |
