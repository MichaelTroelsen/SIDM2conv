...
## Re-measured 2026-08-22 (task `pr5-v3-5-7-decide`, head `0ec8d89`)

The 87%→92% figures above are from May and 114 commits stale — re-measured
against the current `SID/Laxity` corpus and current `master`, per the
`/runhuman` decision that authorised landing this work ("the human performs
the actual merge... re-measure it before it goes into any doc, and never
carry the May figure forward unmeasured").

Same corpus size (286 files) and the same fix (`sidm2/ch_seq_ptr_scanner.py`'s
all-6-in-reads floor) apply the identical **+13 files**, same 13 names, as
originally claimed. What moved is the *baseline* — other work landed on
`master` since May already lifted 2 more files independently of this change:

| | Files | % |
|---|---|---|
| Before this fix, on current master | 253 / 286 | 88.5% (was 251/286 = 87% in May) |
| After this fix, on current master | 266 / 286 | **93.0%** (was 264/286 = 92% in May) |

**Only 3 of `conversion_pipeline.py`, `annotate_asm.py`, `ch_seq_ptr_scanner.py`,
`test_ch_seq_ptr_scanner.py` and this doc were actually applied from the PR
branch as-is.** `sidm2/conversion_pipeline.py` was NOT applied: 8 commits have
touched that file since the PR's base (`94d9c4e`), including the
`native_builder_for` native-dispatch wiring (`8c76e23`) — the PR's copy of the
file predates that function entirely, so applying it wholesale would have
silently reverted 114 commits of unrelated work in that one file to fix a
`List`-import `NameError` that master's current `conversion_pipeline.py`
no longer has (confirmed: `from sidm2.conversion_pipeline import
native_builder_for` collects and imports cleanly on current master with no
change). The other 3 Python files plus this doc had **zero** commits touching
them since the PR's base, so applying those as-is is exact, not approximate.

Full suite with the 4 files (not 5) applied: **2602 passed, 8 skipped, 2
xfailed** (2 more passing than the 2600-passed baseline at `0ec8d89`, matching
the 2 new regression tests this PR adds).
