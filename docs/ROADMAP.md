...
The one call site that reads those fields, in `convert_sid_to_sf2()`: when automatic driver selection lands on `driver11` (the safe default for an unrecognised player) *and* `native_confident` is True *and* `native_builder` is non-empty, it logs a `logger.warning` naming the family and the `bin/` script to run by hand. That is the entire effect. `driver_type` is set earlier from `driver_selection.driver_name` and is never touched by the advisory — no code path reassigns it from `native_family`, and no code path calls a `bin/` builder from inside the pipeline.

This is a measured decision, not a cautious one. Two things have to both be true for a router to be safe, and over 734 files neither is: zero signature collisions did happen (no two probed families ever both claimed the same file), but ~26 of the *confident* answers are uncorroborated against the independent `player-id.exe` verdict — the 20 SID/Shogoon files that `sdi` ranks confidently are called `DMC` by 17 of `player-id.exe`'s verdicts and `Music_Assembler` by 3, not one `SIDDuzz'It`. Wired as a router those 26 would have produced wrong SF2s silently; as an advisory the worst case is a misleading log line. `bin/build_*_native_song.py` scripts are also not importable by construction, not just by choice: every one reads `SID = sys.argv[1]` at module scope and then siddumps/emulates/traces the tune for tens of seconds to minutes, so `import`ing one would both run it and run it against the caller's own argv — they are subprocess entry points, and the pipeline names one, it never runs one. See `sidm2/native_dispatch.py`, `sidm2/driver_selector.py` (`NATIVE_BUILDERS`, `identify_native_builder`), `sidm2/conversion_pipeline.py` (`native_builder_for`, and the warning site inside `convert_sid_to_sf2`), and `pyscript/test_native_dispatch_wiring.py`. Still **M, not S** — the underlying discrimination problem (dmc/mon accept nearly everything) is unchanged; only the wiring decision is done.
