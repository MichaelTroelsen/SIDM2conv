; =====================================================================
; sf2_native_driver.asm -- the SHARED native SF2 driver body.
;
; ONE engine, assembled per player via 64tass -D feature flags. Derived
; (R1, docs/CODE_REVIEW_2026-07.md) from drivers_src/romuzak/romuzak_driver.asm,
; which was the superset of the galway/romuzak pair: those two files were
; 1317 and 1357 lines and differed ONLY by the three feature blocks guarded
; below (plus one comment wording, normalised here). Everything else -- every
; equate, every state byte, every routine -- was already byte-identical, which
; is why this merge is expressible as three flags rather than a rewrite.
;
; Feature flags -- each MUST be defined by the including shim (an undefined
; symbol is a loud assembly error, never a silent wrong build):
;   FEAT_DRUM_ROWS   1 = wave-row col1 is a freq HIGH byte for $20-flagged
;                        (drum) instruments, giving off-grid drum pitch.
;   FEAT_SEEK_PULSE  1 = $10-flagged (SEEK) voices hold the pulse on a note's
;                        LAST frame instead of ramping through it.
;   FEAT_INSTR_PULSE 1 = pulse programs are PER-INSTRUMENT (instrument col4 ->
;                        IPULSE_LO/HI -> VIPUL). 0 = per-command (the $c0-$ff
;                        command selects the pulse program, Galway's model).
;
; Current users:  drivers_src/galway/galway_driver.asm    (all three 0)
;                 drivers_src/romuzak/romuzak_driver.asm  (all three 1)
;
; NOT merged here, deliberately -- see the R1 section of
; docs/CODE_REVIEW_2026-07.md for the measurements behind this:
;   drivers_src/mon/romuzak_driver.asm   diverges in 51 separate hunks
;   drivers_src/blackbird/blackbird_driver.asm  ~1962 diff lines
; Expressing either as flags would need dozens of interleaved .if blocks,
; which costs more in readability and mis-edit risk than the duplication saves.
;
; The shim -> body .include means 64tass resolves the body's own
; `.include "layout.inc"` / `"freqtable.inc"` relative to THIS directory, not
; the player's -- so the builders pass `-I <player_dir>`. Verified: without it
; the nested include fails outright (loud), it cannot silently pick a wrong file.
; =====================================================================
; ==========================================================================
...
ws_done:
        rts
.if FEAT_DRUM_ROWS   ; DRUM wave row: col1 = freq HIGH byte (off-grid drum pitch)
...
pl_set:
        ...
        sta VPADH,x
pl_apply:
.if FEAT_SEEK_PULSE   ; SEEK voices hold the pulse on the note's last frame
...
fp_dec:
        ...
        ; (the filter-table format is shared across the native drivers),
...
set_instr_v:
        ...
        sta VIFILT,x
.if FEAT_INSTR_PULSE   ; per-instrument pulse program (INSTR_PULSE -> IPULSE -> VIPUL)
