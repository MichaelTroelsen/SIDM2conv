...
def verify_tempo_table(data, load_addr):
    """Verify tempo table format (1 column × 256 rows)"""
    tempo_addr = 0x2224  # Driver 11 tempo table address
    offset = get_offset(load_addr, tempo_addr)

    print("\n" + "=" * 80)
    print("TEMPO TABLE at $2224 (1 column × 256 rows)")
    print("=" * 80)
    print("First 20 bytes:")
    tempo_data = [data[offset + i] for i in range(20)]
    print(f"  {' '.join(f'{b:02X}' for b in tempo_data)}")
    print("\n  Format: Speed values (frames per row), $7F XX = wrap to row XX")
    return True

def verify_hr_table(data, load_addr):
    """Verify HR (Hard Restart) table format (2 columns × rows)"""
    hr_addr = 0x1684  # Driver 11 HR table address
    offset = get_offset(load_addr, hr_addr)

    print("\n" + "=" * 80)
    print("HR TABLE at $1684 (2 columns × 32 rows, column-major)")
    print("=" * 80)
    print("Column 0 (AD) - first 8 bytes:")
    ad_data = [data[offset + i] for i in range(8)]
    print(f"  {' '.join(f'{b:02X}' for b in ad_data)}")

    print("\nColumn 1 (SR) - first 8 bytes (offset +32):")
    sr_data = [data[offset + 32 + i] for i in range(8)]
    print(f"  {' '.join(f'{b:02X}' for b in sr_data)}")

    print("\n  Format: Col0=Attack/Decay, Col1=Sustain/Release")
    print("  Default: 0F 00 (fast attack, no sustain)")
    return True

def verify_init_table(data, load_addr):
    """Verify Init table format (2 columns)"""
    init_addr = 0x1664  # Driver 11 init table address
    offset = get_offset(load_addr, init_addr)

    print("\n" + "=" * 80)
    print("INIT TABLE at $1664 (2 bytes)")
    print("=" * 80)
    init_data = [data[offset + i] for i in range(2)]
    print(f"  Bytes: {' '.join(f'{b:02X}' for b in init_data)}")
    print(f"\n  Byte 0 (Tempo ptr): ${init_data[0]:02X} (row {init_data[0]})")
    print(f"  Byte 1 (Volume):    ${init_data[1]:02X} (0-15, F=max)")
    return True

def verify_arp_table(data, load_addr):
    """Verify Arpeggio table format"""
    arp_addr = 0x2124  # Driver 11 arp table address
    offset = get_offset(load_addr, arp_addr)

    print("\n" + "=" * 80)
    print("ARP TABLE at $2124 (1 column × 256 rows)")
    print("=" * 80)
    print("First 20 bytes:")
    arp_data = [data[offset + i] for i in range(20)]
    print(f"  {' '.join(f'{b:02X}' for b in arp_data)}")
    print("\n  Format: Semitone offsets for chord patterns")
    return True
...
def main():
    ...
    results['tempo'] = verify_tempo_table(data, load_addr)
    results['hr'] = verify_hr_table(data, load_addr)
    results['init'] = verify_init_table(data, load_addr)
    results['arp'] = verify_arp_table(data, load_addr)
