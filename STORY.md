...
**Current version:** v3.28.0 (2026-08-22) — **The register nothing was scoring, and the four files two builders were fighting over.** 114 commits, and the through-line is a single register: `$D418`'s **mode bits** — the filter passband. Every "filter 100%" figure this project had ever published was **cutoff only**; the passband was scored by nothing, on any player. Scoring it produced a defect on every corpus it touched, and it kept producing them because the *measurement* was wrong three separate ways before the *builds* were. The offset fit bought agreement by discarding frames. The scorer's scratch probe was ONE shared file, so at `-j16` it scored the wrong song. Artifacts were compared over a window that crossed a part boundary, manufacturing 41 UNCONFIRMED rows — until the builder started recording each part's own span beside it. Only after those did the real numbers hold still: SDI **271 of 280**, HardTrack **32 of 33**, MoN **22 of 27**, Blackbird **16 of 16** — against its simulator, because siddump cannot drive an LFT rip and comparing against that silence had already produced three confident wrong readings. Then the last passband defect turned out to be in the driver itself: `F_MODE` is zeroed at init, so **every** native build opened with the filter off and declared its real passband only when the first filter program ran. `INIT_FMODE` now carries the original's opening passband — opt-in, because `emit_one` is shared by **eight** players and a 4-byte unconditional store would move all eight corpora at once. **The other half of the cycle was concurrency, and it was folklore.** Parallel corpus builds share not one file but **four** (PATTERNS F12): the two MoN includes, the hardcoded romuzak `layout.inc`, and `out/romuzak_driver.prg` — the assembler output read straight back, the worst of them, because the PRG *is* the driver image. DMC went 3.5 h → 14 min at `-j16`; SDI's 11-hour serial sweep got `--jobs` and longest-first scheduling. Three more contracts landed underneath: a **refused** native build must leave the disk as it found it (it did not — it emitted, then refused); a failed `siddump` returned `''` and six builders **built from it**; and every artifact now carries a `.prov` sidecar naming the commit and flags that made it, because "the corpus is rebuilt" was quotable and false. A native **dispatcher** shipped as *advisory, not authoritative*, ranked on whether a parser can **reject** rather than whether it can accept — 0 of 48 useful before that inversion, 6 after. **And the honest part: this cycle retracted more than it claimed.** "18 of 20" was a subset, not a smaller corpus. "All 88 DMC songs rebuilt" was false. "Every build opens on `$D418` off" was refuted by its own author the same day — 2 of 33, not 33 of 33. Myth's 77% filter did not reproduce. A1's premise and A4's replacement premise were both refuted. Each retraction came from running the thing rather than reading the doc that stated it.


**Previous:** v3.27.0 (2026-08-12) — **The fix was right, the reason was wrong, and the number that proved it was noise.** Matt Gray got a native Stage B — 16 tunes across three games, no new driver, because the slide, arpeggio, pulse-sweep and drum engines the old plan listed as four things to reverse-engineer are simply **captured** per frame. Then it got the first listening pass any Matt Gray build has ever had, and the pass immediately found something no headless number in the builder's own report could move: the original writes `$00` — no waveform bits, oscillator silent — on 192 gate-off frames per voice where our render kept releasing a tone. The audio feature summary had said the same thing without knowing about registers (silence fraction 0.2% → 0.0%). That one was cheap to fix and the waveform went 84.6% → 100.0%. **The interesting part is what happened next.** The commit that fixed it named the next suspect confidently — gate-off frames where we write freq `$010C` against the original's `$0000` — and it was **wrong**: those frames carry waveform `$00` on both sides, so the oscillator is silent and a frequency written to it cannot be heard. Frames where our oscillator sounds while the original's is silent: **zero, on all three voices.** The real defect only appears when every sounding gate-off run is attributed to the event that *owns* it — and then it is not subtle at all: all 138 runs inside a note's own span were byte-exact, and all 236 inside a **rest** were wrong, 76% of their frames an exact octave relation to the original. Matt Gray **arpeggiates through the release**, and the shared builder emits a rest as bare gate-off rows carrying no wave, pulse or FM program, so the pitch froze. One shim-layer merge — absorb a rest the original still sounds through into the note before it — and the audible wrong-frequency gate-off frames went 93/288/832 → **0/0/0**. Which raised the obvious question: why does the audio still not match? The answer was that **the measurement had been lying twice.** A 0.579 waveform correlation had been published as proof the audio really differs; rendering the original against *itself* at a different power-on delay — byte-identical registers, only a different oscillator phase — scores 0.58–0.74, so 0.579 is indistinguishable from identical material and was never evidence of anything. And the per-window figure quoted as "0.78 against a 0.98 floor" was an artifact of an alignment search stepping in 10 ms hops, coarser than a 1024-sample FFT window is sensitive to — caught because the *same* comparison scored 0.98 in one run and 0.87 in another. Refined, it is 0.855 against a floor two independent measurements agree on to 0.002. Then the hypothesis that had been named to explain the rest — registers written twice within a frame, invisible to any per-frame model — was tested with a cycle-accurate trace and **falsified**: our driver already reproduces the gate blips 105/21/16 against 104/20/16, and the write order on every note-on. On the way it nearly produced a *third* wrong number, because writes binned by the tracer's frame window mix the tail of one play call with the head of the next — 764-against-161 became 142-against-157 the moment they were grouped by call. Three claims retracted in one cycle, every one of them retracted by the same move: measure the floor before quoting the number. HardTrack's brightness gap got the same treatment and lost two of its three candidate registers — its "cutoff 94.6%" is **100.0% byte-exact** on the frames where a voice is actually routed through the filter, and the SR-tail hypothesis predicts we should be locally louder where we are uniformly 1.1 dB quieter. Honest state: Matt Gray's registers are essentially exact, its write pattern and order are reproduced, its audio still sits 0.13 below a floor measured on the same tune — and there is **no standing hypothesis**, which is written down as such rather than replaced with a fourth guess.

**Previous:** v3.26.0 (2026-08-11) — **Five of six answers were already written down, and five of six were wrong.** This cycle started from a handoff document listing six open items, each with its diagnosis and its prescribed fix. Every item got done; almost none got done the way it said. The tool that had produced two retracted rung-3 conclusions was to be given a **per-voice offset** — implemented, measured, reverted: it picked offsets 5/317/147 for three voices that in fact share −3, because a repetitive tune has many spurious alignment peaks. The offset was never the problem. The search ran over `range(0, 400)` and **could not express a negative offset at all**, while our render *leads* the original by three frames — so the true answer had never been in the search space, and the tool had been reporting the offset-0 value as though it were the build's fidelity. It also ranked candidates by **raw hit count**, which rewards whichever alignment simply leaves more frames inside the window: offset 157 beat −3 on hits at a rate of 67/14/62%. Fixed, `Love_tune_2` finds −3 by itself and reads 91/93/64% on frequency with waveform, pulse and AD/SR all **100%** — they had read 79/86/57% at the bogus offset. **Stage C's FM prong** was priced at a 9× collapse and it is real — 1,028 of 1,327 note events reduce to **8** distinct programs, parts fall 6 → 4 — but it costs 7–8 points of frequency, and guarded losslessly only **148 of 8,526** substitutions survive, buying nothing: HardTrack's captured pitch almost never follows the pure table arp, which is precisely why Stage B captures instead of modelling. Reverted. The second build's **47 residual frames** were filed as unmapped variables; they were one variable, `vib_depth`, the last thing in the model still placed by a **load+constant** — correct on 18 files, wrong on 15, and its bytes at `load+$1C` *were* the error (`$42 $23 $0b`, and those voices were wrong by +66/+35/+11). Recovered from the leg that reads it, the second build went **99.89% → 99.96%**. The Laxity orderlist address was blocked "pending ground truth"; the ground truth existed three ways, all agreeing the hardcoded `$1766` was wrong — and end-to-end through the text exporter, exported positions naming a sequence that does not exist went from **15,589 of 22,950 (67.9%) to 0 of 415**. "Laxity has its own parse path that currently works" had been true of nobody. And the **SR-only row** for the ADSR residual was built, assembled, and changed nothing — Stage B folds gate-offs into the preceding note, so the song holds 90 rest rows out of 30,477 and only 30 precede a note, against 112 note-ons in twenty seconds; a row type cannot express a write that lands between row dispatch and note-on. The through-line is not that the prior work was careless — it was careful, and it wrote its reasoning down, which is the only reason the errors were *findable*. It is that **a recorded diagnosis is a hypothesis with a citation, not a result**, and the cheapest thing a session can do is re-measure the premise before implementing the prescription. Two of the six items were closed by measurement alone; two shipped as fixes; two were built and reverted with their negative results written down so the next attempt starts from the wall rather than the start. Along the way the rung-3 comparison finally entered the repo (it had lived in a scratch file that would have vanished with the temp directory) and the tool stopped copying a 1 MB editor binary over a tracked file on every run — which is how "should this be committed?" had become a standing question nobody could answer: the tool, not a person, had been deciding to stage it. The previous story: v3.25.0 (2026-08-10) — **The filter that was global all along, and the control that moved nothing.** HardTrack's last unmodelled register group was the filter, listed for two releases as "the one thing `simulate_registers()` does not predict". Modelling it was not the hard part; deciding *what shape it was* was. The fields that seed it — 6, 7 and 12 — are read **per voice** at note-on, which invites a per-voice filter envelope; the engine that consumes them is **global**, and the player says so unambiguously once you look at the right instruction: it sits past the `dex / bmi` at `$1583` that ends the voice loop, so it runs **once per frame after all three voices**. Its cursor, cutoff accumulator, delta and mode nibble live in **self-modified operands**, which is exactly why an operand scan for a table address had never named them — the addresses being written are inside the code. A second consequence fell out of the same three bytes: voices are stepped **2, 1, 0**, and the model had been stepping them 0, 1, 2. No measured number moved, because per-voice registers cannot care — but a global filter takes whichever note-on lands last, so the bug was invisible right up until the moment it would not have been. The result is byte-exact: cutoff and resonance/routing **100.00% on 32,967 frames across all 33 files**. Which is precisely where the story stops being about a filter. **A uniform 100.00% across a whole corpus is the shape this project has twice been wrong about**, so every assumption was broken on purpose to check the measurement could still see it: clamping the 8-bit accumulator instead of letting it wrap drops it to 24.75%, freezing the program 23.90%, swapping fields 6 and 7 28.34%, and reading `f12 == 0` as "skip" rather than *actively clearing this voice's routing bit* costs 52 points of `$D417`. Four of five controls bite. **The fifth moved nothing at all** — ignoring the field-5 bit-4 re-arm gate changes no number on this corpus — and it is written down as a negative result rather than quietly counted as a pass, because the reason is countable: only **21 instruments** set the bit and just **one** of those has a filter to re-arm. The reading stands on its only consumer in the disassembly and the corpus can neither confirm nor refute it, so a test pins both counts and says so. The same discipline caught the column that would have flattered itself: `$D418` also reads 100.00%, but **23 of the 33 files hold it at one constant on both sides** and are withheld by `exercised()` — the identical shape `HUBBARD.md` had to retract a published "filter 100%" for, arriving in the same player's docs one release later. Two smaller corrections came from re-reading consumers rather than measuring: `$D415` is never written **anywhere in the corpus**, so the cutoff is one byte and not eleven bits, and the runtime volume is whatever `init` stores rather than what the image saved — a distinction that only shows up on `Tribute_to_Laxity`, whose allocation shifts by one instruction and hands back a neighbouring variable to anyone reading at a fixed offset. The previous story: v3.24.0 (2026-08-09) — **HardTrack Composer decoded and converted, and five hypotheses killed on the way — one of them by a falsification that was itself false.** A 1,126-file player family with no support at all got a relocation-safe parser validated byte-exact against the frequency register. Two things are worth carrying forward. First, the initial validation scored a *correct* model at **0.0%** because it matched siddump's note-**name** column — every HardTrack instrument opens with a one-frame attack transient, so siddump reports `E-6` as the onset row for nearly every note whatever its pitch; the frames were already exactly right, three frames of attack apart. Comparing the raw register instead turned 0% into 100% on the first file without touching the decoder. Second, the corpus number (**88.39%**, 8 of 33 files at exactly 100.0) is only meaningful **split**: a per-instrument breakdown came back bimodal — 180 pairs at 99.49%, 76 at 0.61%, 19 in between — and the disassembly named the bit, field 5 bit 7, which hands `$D400/$D401` to the instrument's own wave program. Those notes score **2.61%** and *cannot* score higher; pooling them would report the unimplemented synth engine as a parser error. The 11.6% residual was characterised rather than shrugged at — flat across all ten 100-frame buckets, so no drift; flag bits 0-1/4/5/6 each tested and none partitions it — and what remains is written down as a hypothesis, not a finding. 5 of 38 files refuse to decode rather than guess, after the ungated version ran away and produced 2,997 phantom onsets against the wrong song. RE stage only; no converter. The previous story: v3.23.0 (2026-08-09) — **A way to listen, and a calibration that priced it.** The assistant could compare registers and onset timing and nothing else, so this cycle built an audio-domain summary it can actually read — level, brightness, noisiness, pitch-class chroma, per-section windows — plus spectrograms it views directly. Then it did the thing the tooling had never had done to it: **checked the metrics against a human's recorded verdict.** The one on file is B13, where a register-exact Blackbird build of Glyptodont drew "something with the perc or drums" from a real listening pass. Rebuilding that exact artifact took git archaeology — the defect had since been fixed — and the result was more limited than the tool's own guide implied: against the original, the floor for *itself* re-rendered is 85–91%, the known-GOOD build scores **64.7%**, and the B13 build **56.9%**. The tool ranks them correctly, so it has ordinal sensitivity; it has no usable absolute gate, because a near-perfect build sits 20+ points below the original's own floor and any threshold flagging B13 condemns the good build too. Of five improvements built from a plan, only **A-weighting** proved out on the case (5.4× better separation); **mel is unproven**, **chroma is a correct null** (a percussive defect has no pitch shift to find, and none was reported), and the **windowed** search turned out to score sparse metrics *infinitely* — `silence_frac` is zero in most windows, so its median is zero, so it divided by zero and won every ranking, flagging good and bad builds identically. The through-line is sharper than any feature: **four of the five would have shipped a confidently wrong number that the plan's own test passed** — an octave collapsing to `-0.0 Hz`, a +12.8 dB A-weighting error from a band-centre lookup, a z-score with a hard ceiling of 2.5 against a 3.0 threshold, a 55 Hz bass tone confidently called G instead of A. None were caught by review; all four fell out of running against real material and checking against an external reference. The same discipline killed a bigger idea: CLAP audio embeddings were bridged in out-of-process (they pin `numpy<2.0`, and numpy 1.x has no wheels above cp312, so the venv needed a different *Python version*) and then **failed their own validation gate** — different compositions scored 0.947 while a same-tune pair fell to 0.534, and longer renders made it worse, a general-audio model collapsing toward a genre centroid. A 4-tune run had passed by +0.006; widening to 9 tunes inverted it to −0.413. That negative result is written down so it is not retried blindly. The honest summary: this does not replace a human listening, and now there is a measurement saying so rather than an opinion. The previous story: v3.22.0 (2026-07-25) — **Three settled answers that were wrong.** The Blackbird (lft) player closed out at **99.96% corpus mean, 11 of 16 files at exactly 100.0** — but the story is how it got there. Every one of the four fixes overturned something the repo had already written down as understood. A hotfix had disabled the combo-command encoding because it "crashes SF2II on play"; re-measured against the same editor build, the combo build executed **120 combo commands across ten screenshot-verified trials with zero crashes** — the original verdict rested on one observation per arm of an editor already documented as heap-flaky. `To_Die_For_II`'s 5.8-point gap had been filed as an intractable "later-frame pulse/filter residual"; it was **one missed note-on**, because `prepare1`'s byte allowance is *forfeited* the moment `prepare2` consumes, and B9's free-running pulse accumulator turns a two-frame stall into a permanent 66.7% loss. Its remaining filter residual looked like a parser bug; the parser was right and the **row grammar was missing a row type** — hardware rewrites `$D418`/`$D417` every frame whatever the cutoff op is, so an ADD row0 had no way to state its own passband. And `Into_the_Unknown`'s last part was wrong because **B7 built a resume-mid-cycle primitive and `window_steps` never learned it existed** — a docstring that was true when written and false when read, with the two mechanisms silently cancelling. The through-line is a method, not a player: *derive the rule from the code that works, then check the consumer, not just the producer.* Twice the answer was in the driver, not the data. The cautionary half belongs beside the gate-that-certified-nothing: **my own first crash measurement came back 16/16 clean and was worthless** — a 6-second play window over a build whose first combo command fires at 8.2 seconds, testing none of the thing under test. An all-green result measured outside the window where the effect lives is not evidence of absence; `assert_window_covers()` now raises instead of reassuring. The fidelity tooling moved out of an untracked scratchpad into `pyscript/` with 34 tests, so the numbers in the accuracy tables are finally reproducible from a fresh clone, and Blackbird entered the project's own build inventories after an entire B1–B25 + E3–E6 arc of being absent from them. Two overstatements corrected on the way out: the "16-file corpus" is not a curated sample but **every LFT rip the locator supports — 16 of 61 on disk**, and the sweep's byte-identity column only ever checked part 1. The previous story: v3.21.0 — **The gate that certified nothing.** Landing an uncommitted tracer fix meant asking who consumed the tracer, and the answer was the audio gate that certifies every emitted SF2 as byte-identical to the original. It compared two traces for equality — so when the tracer couldn't drive a file, both came back empty, `len(0) == len(0)`, the compare loop never ran, and it passed. Sixty-four bytes of zeros were certified as byte-identical audio to a file in the primary supported corpus. The lesson belongs beside the SBC carry bug: **an equality check over evidence must first assert the evidence exists** — `compare(a, b)` where both are empty is not a match, it's "no test ran". Fake successes across the RSID corpus: 20 → 0; honest failures 3 → 22. The three regression tests were each watched failing against the old gate before being trusted. Alongside it, two players: **Jeroen Kimmel** (first RE of a Hubbard-derived engine with its own player-id — 11/12 voice-medians exactly 100%, Radax's six subtunes unlocked by finding that its init relocates a player copy to `$E000` and seeds *that* copy's orderlist, and an effects engine cracked by one byte doing two jobs: the pulse-width high byte *is* the freq-slide rate; ships as the first Stage-A builder to emit Driver 11 sequence commands, verified through the real driver) and **Charles Deenen** (forty files that aren't one player — a third of the useful corpus was already supported and merely misfiled, so eight wins at ~100% came from classification alone; the fresh engine's timing key was that the groove rate *varies per file*, and the builder refuses to emit what it can't decode). The previous story: v3.20.0 — **The pitch that wasn't in the notes.** The SDI strict campaign decoded six variants of one editor (the melody hides in the instrument's waveform program — a different row per generation), shipped 254 Stage A SF2s with zero failures, and turned the recurring lessons into a knowledge base (docs/players/PATTERNS.md + sidm2/player_idioms.py). The previous story: v3.19.0 — **Pockets, part-bloat, and Glenn's own source.** The pocket fixes each ended at 100.0 (a banned freerun stream, a reversal-blind filter-drive detector, FM loops for vibratos that outlive the capture window); the part-bloat levers landed (DMC's step grid took Balloon from 77 files to ONE 400-second SF2 at wf/pulse 100×3; Sound Monitor's chord arps and pulse canonicals settled its corpus at 11 songs / 27 parts, 99.23% strict); and the Gallefoss arc opened on the best ground truth in the project's history — GRG co-wrote SID Duzz' It, and his own commented player source came off the staged d64s readable. The previous story: v3.18.0 — **The instrument-cap optimizer.** The user looked at Dance's cap numbers and said the thing the pipeline needed to hear: original C64 songs never had more than 32 instruments — the editors wrapped a few instruments in wave/filter/pulse *code*, so our per-note capture must be manufacturing the variety. Measured: 10 source instruments exploding into 31 driver slots, almost all duration-positioned release tails and per-drive filter captures. Three register-exact-guarded transforms later (a RELEASE_WF driver feature that writes the instrument's release waveform verbatim on gate-off rows, first-row boundary-bleed normalization, and filter drives credited by the $D417 routing bits *on the attack frame* because Sound Monitor switches routing per section), Dance's worst window went 32 → 14 slots and its filter table 247 → 60 rows. The optimizer also flushed out two blindness-class bugs: every grid part ≥ 2 had shipped with all three voices starting at row 0 (an inter-voice desync no per-voice-delay metric could see — Dance part05 went from 28-65% freq to 100.0 on every register once a global-delay sweep existed to catch it), and 24 of 88 DMC files retrigger within one play call, which not only built notes legato but silently dumped whole files onto the tick-grid fallback (Balloon: waveform 0% → 100% with write-stream onsets). End state: SM corpus 11 songs / 28 parts at 99.08% corpus-wide strict freq+wf, crown jewel proven byte-identical twice, 1524 tests green. The previous story: v3.17.0 — **Sound Monitor, start to finish in one day.** The seventh from-scratch player (Chris Hülsbeck's 1986 64'er listing, the Fun_Fun cluster): format RE'd, parser onset-validated 99.9% corpus-wide, Stage A editable Driver-11 SF2s at 32/33 voices note-accurate, and Stage B native builds through the shared MoN engine — Final_Luv plays its entire 161 seconds from ONE SF2 at 98.1-99.9 skew-tolerant on every register. The engine hid its best jokes in plain sight: the "digi player" was the arpeggio engine, the "pitch bend" was the pulse engine (its dispatcher lands Y=2/9/16 on the pulse registers), rests are release-waveform writes that can keep a voice ringing, and the chord tables live inside the row headers. One gotcha for the ages: SID Factory II's editor and its own Driver 11 disagree about tie bytes — the editor writes $90-$9F durations the runtime driver can't parse. The previous story: v3.16.0 — **The part-reduction + fidelity-truth campaign.** The user asked for two things at once: fewer SF2 files per song, and the last one or two percent of fidelity — with a measuring stick honest enough to tell them apart. The campaign delivered both. First the splits: instrumenting the window packer showed ~95 % of all song splits were forced by one thing — the 64-slot command channel that couples every note's (FM, pulse) pair. Decomposing it revealed the fix was *player-shaped*: Hubbard reuses ~24 structural FM shapes per song (pulse drives its explosion), so collapsing pulse to one canonical per instrument took **Commando from 45 files to 4 and Monty from 22 to 4, byte-exact** — while DMC is genuinely diverse in both axes, so the same trick provably does nothing there. Then the monsters: Shockway Rider "needed" 638 files because the parser was reading a later revision's **bit7 track-transpose commands as pattern numbers**, decoding one voice ten times too long and faithfully emitting an hour of repeats — three ROM-verified encodings later it builds 21 files, correctly, for the first time. On the fidelity side, the upgraded analyser earned its keep twice over: its new mismatch-cluster report localized Supremacy's whole waveform residual to three frame-runs and root-caused it (the engine writes wave-off `$00` mid-rest; the capture now extends across rests) — **Supremacy sub1 94.3 → 99.9 on every register, Cybernoid II 100/100/100** — and its skew-vs-content classification exposed that some "failing" voices are audibly exact (±1-frame write jitter: 47.7 strict, 100.0 skew-tolerant). The same skepticism caught pulse_canon being lossy where it mattered (gated), caught the measuring tool itself lying twice (part-loop tails, wrong window offsets), and left the CI security scan green after 199 consecutive red runs. DMC reached 41/88 eligible. Honest state: the transposed-track class plays its pulse right on two voices of three (its self-modifying-rails engine is decoded, the fetch-frame/arp-refetch semantics are the documented residual); Sound Monitor is the next player, arc already open.

**Previous:** v3.15.0 (2026-07-09) — **DMC — the format that unlocks hundreds of tunes.** The sixth from-scratch native player, and the highest-leverage yet: Johannes Bjerregaard's own **DMC (Demo Music Creator)** — the DMC4 Player by Brian/Graffity, one of the most-used C64 editors ever, so the parser generalises far beyond the 88-file `SID/JohannesBjerregaard/` corpus (Blue Monday, Billie Jean, Domino Dancing covers included). The format is a clean **Track → Sector → Sound** hierarchy (RE'd from Balloon.sid, relocation-safe like Hubbard V2), but its signature is the **wavetable**: a per-frame arp table (`$1A00`/`$1B00`) advanced one step every frame, where the freq-table *row* is the wavetable value — so DMC plays notes far above its own table via octave shift, and any voice's true pitch jumps octaves frame to frame. That per-frame arp is exactly what the trace-driven pipeline's Hz-delta FM capture is built to reproduce — *if* the note is triggered on the right frame. The whole build hinges on **onset alignment**: `measure_onsets` emulates the player (siddump CPU + a raster fake) and records every `$D404` gate-rise, and the native builder places one note per emulated onset so the captured arp starts in phase. That took **Rockbuster from ~65% to ~97%** (waveform/pulse 100). 21/43 files build onset-eligible, most two of three voices at 90–100%. The residual is honest and documented: fast-arp voices whose base note is pinned to a 1-frame attack spike (Wanna_Get_Sick osc1), and a proven *dead end* — the Galway/MoN semitone-arp model is strictly *less* exact here because DMC's freq table isn't PAL. New `docs/players/DMC.md`.

**Earlier:** v3.14.0 (2026-07-08) — **Rob Hubbard, two generations.** The fifth from-scratch native player, and the first that was two formats sharing one codebase. V1 (~12 tunes: Monty, Commando, Zoids, Last V8, …) plays **pulse + freq + filter byte-exact on all three voices** — the pulse via a per-instrument engine that *is* Hubbard's own ROM pulsework re-implemented in the driver, so it's exact by construction. Then the "64 unparseable" files turned out to be incremental deltas, not a rewrite: V2 (the Delta class) adds split song tables, a fractional-tempo "swallow counter," and a 4-byte note format — Delta's title theme (PSID song 12/13, caught by the user after we'd polished the wrong subtune) now plays freq/pulse/filter 100%. Three source-of-truth tools earned their keep this cycle: the VICE register dump caught a 2-frame bass chop the ear heard and the % metric didn't; a *vacuous-100.0* bug (a silent SF2 measured perfect) is fixed and guarded; and a batch-killing 3-hour emulation replay became 27 seconds. New `docs/players/HUBBARD.md` (source), `docs/players/NATIVE_DRIVER.md` (the target drivers we authored), `docs/players/HUBBARD_V2_PLAN.md` (the open front). Honest state: ~19 tunes build, ~28 decode ≥95%; the V2 swallow-class state-region relocation and six no-signature files remain.

**And earlier:** v3.13.1 (2026-07-05) — **The knowledge consolidation.** Four player ports in five weeks left the project's know-how scattered across memory files, session handoffs, and code comments — while the user-facing docs still described the pre-native-driver world (README claimed "filter 0%, no multi-subtune, Galway 88-96%"; the accuracy matrix hadn't been reviewed in twelve releases; the entire Maniacs-of-Noise line was documented nowhere). This release writes it all down before the next player: **`docs/players/PLAYBOOK.md`** distills the staged porting method — parser RE, Stage-A Driver-11 transpile, Stage-B native driver, Stage-C structural rebuild — with the technique catalog, the SF2II hard caps, the fidelity-measurement ladder, and every gotcha that cost a session (the SF2II CMP-carry rule, the siddump SBC bug, the argv Heisenbug). New `docs/players/MON.md` and `CLUSTERS.md`, a rewritten `ACCURACY_MATRIX.md`, and a fresh `docs/ROADMAP.md` that names the debt: the native driver exists as three near-identical ~1,300-line copies, the table packer and validators are copy-pasted (including a semitone-reference drift between them), and the road to "any SID → 99% fidelity, 100% editable" runs through a universal trace-first fallback the Myth build already proved possible. Docs only — no code changed.

**And before:** v3.13.0 (2026-06-29) — **Three new composers join the bridge — and Jeroen Tel's Hawkeye plays back byte-perfect.** The trace-driven native-driver machinery built for Galway turned out to be a general engine, and this release pointed it at three more players. **Future Composer** ("The Beat-Machine") and **ROMUZAK** (Oliver Blasnik) both gained SID→SF2 paths — ROMUZAK going all the way to a from-scratch native SF2 driver that reproduces waveform, pulse and AD-SR byte-exactly. But the headline is **Hawkeye / Maniacs of Noise** (Jeroen Tel). Its two-level orderlist→pattern engine was reverse-engineered frame-exact (a *split* frequency table — low bytes and high bytes in separate arrays — was the first puzzle), then rebuilt as a native driver. The hard-won fidelity trick from Galway carried straight over: per-note (FM, pulse) bundles ride a command channel, per-note waveform envelopes become distinct instruments, and the one global resonant filter is restarted per note by a flagged instrument. Result: **subtunes 2 and 3 reproduce frequency, waveform, pulse and filter at 100% byte-exact on all three voices, full song length, in a single editable SF2** — GUI-confirmed in stock SID Factory II. The ~6.4-minute main theme (subtune 0, ~6000 notes) overflows every SF2 table cap and the `$D000` memory wall, so it ships split into thirteen 30-second parts, each ~100% on pitch/waveform/pulse (the filter seam across window boundaries is the one open thread). Net: the converter now speaks four composers' dialects with native-driver fidelity, not just Laxity's.

*(v3.12.0 and earlier: see the [per-version index](#per-version-index).)*
...
### v3.28.0 — The register nothing was scoring (2026-08-22)

114 commits. One register, `$D418`, and the discovery that every "filter 100%"
this project had published was **cutoff only** — the passband (the mode bits)
was scored by nothing, on any player.

**The measurement was wrong before the builds were, three separate ways.**
`compare()`'s fitted offset bought agreement by *discarding frames* (`00893cd`).
The scorer's scratch probe was ONE shared file, so at `-j16` it scored the wrong
song — worth 99.5 → 96.4 on the DMC median with 1130 of 1194 artifacts
byte-identical (`3ffdadb`, ported to the checker in `c5f07a8`). And artifacts
were compared over a window that crossed a part boundary, manufacturing 41
UNCONFIRMED rows, until the builder began writing each part's own span beside it
(`0ae0738`). Every corpus figure below is measured *after* those three, which is
why none of them matches an earlier published number.
...
| HardTrack | 32 of 33 |
| MoN | 22 of 27 |
| Blackbird | 16 of 16 — against its **simulator**; siddump cannot drive an LFT rip, and comparing against that silence had already produced three confident wrong readings |

**Then the last defect was in the driver.** `F_MODE` is zeroed at init, so every
native build opened with the filter **off** and declared its real passband only
when the first filter program ran (`71a7024`). `INIT_FMODE` now carries the
passband the original holds at the window's first frame. It is opt-in, and that
is load-bearing: `emit_one` is shared by **eight** players, and emitting
`lda #$00 / sta F_MODE` unconditionally would add 4 bytes, shift every address
after it, and move all eight corpora at once — even at a zero value. So the
constant is always emitted (64tass needs the symbol to test it) and the two
instructions are guarded, which is what makes the flag-off path byte-identical
rather than merely equivalent.

**Concurrency was folklore, and it was four files.** PATTERNS **F12**: parallel
corpus builders share `drivers_src/mon/layout.inc` + `freqtable.inc`, the
**hardcoded** `drivers_src/romuzak/layout.inc`, and `out/romuzak_driver.prg` —
the assembler output read straight back, the worst of them, because the PRG *is*
the driver image (`4d01910`, `20c506e`). A cross-process lock around the whole
gen→copy→freqtable→assemble section took DMC from 3.5 h to 14 min at `-j16`;
SDI's 11-hour serial sweep gained `--jobs` and longest-first scheduling
(`6023ca8`, `282e4d1`).

**Three contracts landed underneath the builders.** A *refused* native build must
leave the disk as it found it — it did not; it emitted, then refused (`2bdbb71`,
`280b6cc`). A failed `siddump` returned `''` and six builders built from it
(`dd67bee`), and `run_siddump` gained a timeout scaled to the trace (`7003e9b`).
And every artifact now carries a `.prov` sidecar naming the commit and flags that
built it (`a1e6f9a`), because "the corpus is rebuilt" was quotable and false.

**A native dispatcher, shipped as advisory rather than authoritative**
(`8c76e23`). The inversion that made it work: rank a parser on whether it can
**reject**, not on whether it can accept — 0 of 48 useful before, 6 after
(`d8a7072`). HardTrack's accept set *is* its shipped corpus (`9f2263b`); the SDI
probe claimed foreign files and 4 of them are provably refused (`840f0f5`).

**Player work.** Matt Gray: the track-table stride is per-build, not the constant
2 (6/55 → 11/55 parse, `ad57a39`); the pattern table was one byte wide and tempo
was eating it (`e245ff5`); a mis-located table accused the walk 512 bytes later
(`f797fca`). HardTrack: instrument **mode 2** is the SR-restart selector, shipped
on with the corpus rebuilt (`d0ec20c`) — and it is explicitly *not* the
brightness fix (`7c89ab5`). SDI: `Juba-Jazz` 52.8% → 100.0%, its filter enabled
by routing and `$D418` alone with the cutoff held at 0, so a detector keyed on
cutoff jumps credited nothing (`5c5c357`); the variant-D walk is bounded by song
length, not the tick cap (`6aa2162`). DMC: the pitch residual was the SCALED
vibrato marker (`ca9678c`), and the timing fixes moved 39 voices up and 26 down
in **raw** with **zero** audible regressions (`0c3aadc`).

**The honest part: this cycle retracted more than it claimed.** "18 of 20" was a
subset, not a smaller corpus (`47e7902`). "All 88 DMC songs rebuilt" was false
(`0344d85`). "Every build opens on `$D418` off" was refuted by its own author the
same day — 2 of 33, not 33 of 33 (`295d691`). Myth's 77% filter did not reproduce
and A4's premise was refuted (`313acf7`); the replacement premise was refuted too.
"262 of 441 build" is unquotable — the disk says 281, and the rebuild meant to
settle it died in under five minutes having written nothing (`455e709`). Every one
of those came from running the thing rather than reading the doc that stated it.

### v3.25.0 — The filter that was global all along (2026-08-10)

HardTrack's filter was the last register group `simulate_registers()` did not
predict. It now does, which leaves nothing in this player's SID register file
unmodelled.

**The shape was the question, not the code.** Instrument fields 6, 7 and 12 are
read per voice at note-on, which invites a per-voice filter envelope. The engine
that consumes them is global: it sits past the `dex / bmi` at `$1583` that ends
the voice loop, so it runs once per frame after all three voices, and its
cursor, accumulator, delta and `$D418` mode nibble live in self-modified
operands — the reason an operand scan for a table address had never named them.
The same three bytes showed the voice loop runs **2, 1, 0** where the model ran
**0, 1, 2**: immaterial for per-voice registers, decisive for a global filter,
and therefore a bug that was invisible exactly until it wouldn't have been.

**Measured** (20 s, 33 files, byte-exact against the real playroutine): cutoff
`$D416` and resonance/routing `$D417` both **100.00% on 32,967 frames**;
`$D418` **100.00%** on the **10 of 33 files that exercise it**, the other 23
withheld by `exercised()` rather than reported as a vacuous pass — the same
shape `HUBBARD.md` had to retract a published "filter 100%" for.

**Five negative controls, four of which bite.** Clamping the accumulator instead
of wrapping: 24.75%. Freezing the program: 23.90%. Swapping fields 6 and 7:
28.34%. Reading `f12 == 0` as "skip" instead of *actively clearing this voice's
routing bit*: `$D417` falls to 48.28%. **The fifth moved nothing** — ignoring
the field-5 bit-4 re-arm gate changes no measured number — and that is recorded
as a negative result with its reason: 21 instruments corpus-wide set the bit,
one of them has a filter to re-arm. The reading rests on the bit's only consumer
in the disassembly; the corpus cannot test it, and a test pins the counts so the
claim can be upgraded if that ever changes.

Two corrections came from re-reading consumers rather than measuring: `$D415`
has zero stores anywhere in the corpus, so the cutoff is 8-bit and not 11; and
the runtime volume is what `init` stores, not what the image saved — visible
only on `Tribute_to_Laxity`, whose allocation shifts by one instruction.

---

### v3.24.0 — HardTrack Composer, and a number that only means something split in two (2026-08-09)

Picked up the seventh-largest player family in HVSC's `MUSICIANS/` tree — 1,126 files, but
only 45 composers, a Polish-scene editor that circulated locally rather than widely — and
decoded it from nothing to a validated parser. The format turned out to be cleanly
designed and full of small tells: patterns are a byte *stream* rather than a row grid, so
note length is a separate `$67 n` rest command; orderlist transpose is signed mod 128, so
the `$F4` that reads as "+116 semitones" is really an octave down; and instrument byte
`$00` means *keep the current instrument*, which is why the editor emits `$80` for
instrument 0 — a literal zero would be indistinguishable from "no change". Init even
points every voice's pattern pointer at a hardcoded `$FF` byte sitting inside the player's
own code, so the first duration expiry falls straight through into the orderlist fetch
without a special case.

The part worth remembering is not the format, it is the measurement. The first validation
run scored a **correct** model at 0.0%, three times in a row, because it matched siddump's
note-name column — and every HardTrack instrument opens with a one-frame attack transient,
so siddump dutifully reports `E-6` as the onset row for essentially every note whatever
its pitch. The model's frames were already exactly right, offset by a constant three
frames of attack. Switching to a byte-exact comparison against the raw frequency register
turned 0% into 100% on the first file without changing a line of the decoder. The same
trap has now bitten this project on Matt Gray and here; the lesson is that a
*derived* column — a note name computed from a register — can be wrong in a way the
register itself never is.

Then the corpus came back at 88.4% and the honest question was what the other 11.6% was.
It would have been easy to average and move on. Instead: a per-instrument breakdown, which
came back sharply **bimodal** — 180 (file, instrument) pairs at 99.49%, 76 at 0.61%, only
19 anywhere in between. A spread like that is not a noisy parser, it is a binary property,
so the disassembly got asked which bit. It answered: field 5 bit 7, which makes the
instrument's wave program write `$D400/$D401` itself. Tested against the corpus, the bit
splits it 88.39% / 2.61%. Those are now two columns in every document, with an explicit
instruction never to average them — the second column is the unimplemented synth engine,
and a pooled figure would drift with a tune's drum-to-melody ratio for reasons having
nothing to do with the decode.

The residual got the same treatment rather than a shrug. Match rate by 100-frame bucket is
flat at 87.1–90.2% across all ten buckets, so the sequencer walk does not drift over 1,000
frames × 33 files; the weak files are already weak in their *first* bucket. Flag bits 0–1,
4, 5 and 6 were each tested as candidate second overrides and none partitions anything.
What is left is a hypothesis about slide commands and wave-program pitch modulation, and
it is labelled a hypothesis in the docs instead of being quietly folded into a headline.

Five of the 38 corpus files now refuse to decode at all — four carry several player
instances, one has a PSID init that is not the module entry. That guard exists because
without it the simulator ran away on exactly those files and emitted a note on every frame
of every voice: 2,997 phantom onsets, confidently scored against the wrong song. And every
table address is recovered by matching a byte signature against the player's own code,
because the editor patches the absolute operands per file — the same table sits at `$198b`,
`$1a35` and `$17ab` in three different tunes, two files do not load at `$1000` at all, and
two player variants ship five bytes apart. `load + constant` would have worked on the first
file and lied on the rest.

No converter yet: this is the RE rung of the ladder, deliberately shipped alone.

Then the converter followed in the same version, and the rest of the arc was
about measurement more than about code. Stage A — an editable Driver 11 SF2 —
mostly fell out of the parser, except that the waveform/arpeggio table turned out
to transliterate **1:1**: both formats are two columns with a jump row, and both
use the same rule in the second column (`$00-$7F` a relative semitone, `$80+` an
absolute note), so the whole table copies across with its indices intact. What
did *not* fall out was `$6F`, which I had been treating as a synonym for `$00`.
Both keep the current instrument; only `$6F` also declines to restart its
programs. It carries a quarter of every note event in the corpus and was causing
58% of all the losses, and the fix — give it a duplicate instrument whose wave
program starts where the original's would have *settled* — took those losses to
zero, because runtime Driver 11 cannot express a tie at all.

The measurement is the part worth remembering. **Five hypotheses were falsified,
and one of the falsifications was itself false.** Cross-pattern gate state died
because the worst-losing voice had no ambiguous pattern at all while a
100%-scoring file had three. The hard-restart flag died because three different
flag values produced byte-identical histograms. The "wrong play address" idea
died because the alternative was total silence. Telling two swapped instrument
fields apart by plausibility died most instructively: the check scored **both**
readings at ~91.6% and therefore discriminated nothing — reading the consumers
settled it. And a `$62`-freeze explanation was published as *refuted* for two
commits before turning out to be right; the refutation had come from a tagger
whose cursor heuristic mis-aligned on that exact file, and which I had already
flagged as unreliable and used anyway. Recording the raw instrument byte and the
pattern position on every onset made the test exact and reversed the verdict.
...
not Stage A's to fix: Driver 11 has neither `$62`'s stepper freeze nor HardTrack's
habit of sounding the bare note for a frame before the wave program takes over.
Both are Stage B.

The through-line: every wrong answer this arc failed the same way — a measurement
nobody had verified. Including the one that was used to overturn a correct one.

### v3.23.0 — A way to listen, and a calibration that priced it (2026-08-09)

Built the audio-domain tooling the assistant lacked — feature summary, spectrograms, chroma,
windowed sections, A-weighted loudness, per-voice stems — then calibrated it against the only
human verdict on record (B13/Glyptodont) and found it weaker than its own guide claimed: ordinal
sensitivity, no absolute gate, a near-perfect build 20+ points under the original's own floor.
Four of five planned improvements would have shipped a confidently wrong number their own tests
passed. CLAP embeddings were tried and rejected on a measured negative result. Tests 1,900 → 2,065.

### v3.21.0 — The gate that certified nothing (2026-07-16)

Two players landed — but the version is named for what was found on the way to committing them.

A tracer fix sat uncommitted in the tree: the zig64 tracer used to print "installed" and emit an empty trace when a self-installing IRQ vector never resolved, so an untraceable file looked exactly like a silent tune. Good fix. Landing it meant asking who consumed the tracer, and the answer was `zig64_audio_gate.py` — the gate that certifies emitted SF2s as byte-identical to the original. It compared two traces for equality. When the tracer couldn't drive a file, *both* came back empty. `len(0) == len(0)`. The compare loop never ran. `return True`. Sixty-four bytes of zeros, certified as byte-identical audio to `SID/Laxity/Broken_Ass.sid` — a file in the primary supported corpus. The tracer fix could never have closed it either: `_trace` ignored the exit code, and the tracer's `FAILED:` line contains commas, so the row filter dropped it and it read as a clean empty trace. Two independent layers, each quietly converting "I could not check this" into "this passed".

The lesson generalizes past this gate, and it belongs next to the SBC carry bug in the project's small museum of metrics that lied: **an equality check over evidence must first assert that the evidence exists.** `compare(a, b)` where both are empty is not a match — it is "no test ran". The fix fails closed on empty or failed traces (the missing-tracer fail-open stays, deliberately, for machines without zig64); the tracer now rejects handlers below `$0200` — zero page is never an entry point, whatever `A_Mind_Is_Born` claimed with its `$0031` — counts writes on both paths, and exits non-zero so `mcp-siddump` surfaces it. Fake successes across the RSID corpus: twenty to zero. Honest failures: three to twenty-two, including the six Matt Gray files that all resolve the same stale `$CBD4`. The three regression tests were each watched failing against the old gate before being trusted, because a passing test proves nothing until you've seen it fail.

The mundane detail that makes the point: mid-session the empty trace fooled *me*. Ran the tracer with its default `$1000/$1003` against a PRG whose real addresses were `$1573/$13D7`, got nothing back, and briefly read it as a regression I'd caused. The bug demonstrating itself on the person fixing it is the best argument for fail-closed anyone could ask for.

The two players: **Jeroen Kimmel** — four files, one Hubbard-derived engine wearing its own player-id, and the first RE of the variant anywhere. Eleven of twelve voice-medians land exactly on 100%. Radax's six subtunes needed the discovery that its init *relocates* a player copy to `$E000` and seeds that copy's orderlist, leaving the file-image tables untouched and every subtune returning sub0's stale stream. Then the user listened and said "notes fine, fidelity not there" — which was the `$12CC` effects engine going unported, and the finding that cracked it was one byte doing two jobs: the instrument's pulse-width high byte *is* the freq-slide rate. It ships as the first Stage-A builder to emit Driver 11 sequence commands, verified through the real driver rather than a model. **Charles Deenen** — forty files that turned out not to be one player at all, and a third of the useful corpus was already supported and merely misfiled: eight wins at ~100% arrived from classification alone, before a line of new RE. The fresh engine gave four clean wins, and its timing key was that the groove clock's rate *varies per file* — a constant, hardcoded at 2.5, was the largest single source of onset error. The builder refuses to emit what it can't decode. Which, this version, is the theme.

### v3.20.0 — The pitch that wasn't in the notes (2026-07-12)

Six binary generations of one editor, and in every one of them the melody hid somewhere else. SID Duzz' It sequences can play a *constant* note while the music moves — the pitch lives in the instrument's waveform program, and each rip generation stashes it differently: variant A in row 1's argument, variant D in the row where the walk parks ($FE stops on its last row — twelve files went to 100.0 strict the moment the decoder walked there too), variant E in row 0 applied on the very note-on frame (byte-verified against the authors' own 1994 source), variant V in an octave nibble that doubles the frequency word. The campaign's villain was the false locate: the six "multispeed" files scored 0.0 because a wrapper's byte pattern matched variant D's exactly — same opcodes, different engine — and the fix (dump the operands, check they point at sane offsets) got paid for twice before it got a name. It has a name now: the session's other deliverable was the knowledge base the user asked for — PATTERNS.md, where the pitch-carrying instrument is entry P1 with all five sightings listed, and player_idioms.py, where the locate library lives as tested code. 254 Stage A SF2s emitted, zero failures. The next parser starts from a catalog, not a blank disassembly.

### v3.19.0 — Pockets, part-bloat, and Glenn's own source code (2026-07-11)

The same day's second act. First the residual pockets, each one a small detective
story: Dance's bass pulse read 4% because the freerun-stream detector saw two
"different" pulse starts that were really the same reset value split across a
frame boundary — Sound Monitor never free-runs, its note-set resets the pulse
every time, so the whole stream mechanism was banned for pulse-tie engines and
the voice snapped to 100. The filter read 33% on a whole part because a fast
envelope sweeps faster than the attack threshold, so the detector treated entire
descents as one "jump" and skipped every re-attack buried inside; recognizing a
direction reversal mid-run took the filter to 100. A vibrato that outlived the
256-frame capture froze at its peak until FM programs learned to loop. And the
measuring stick lied twice more — rounded-second window labels wrecked a sweep
of perfectly good parts, and a 4-frame grid residue made a byte-faithful build
read as garbage until the delay search widened. Then the bloat: DMC's
onset-aligned builds had been writing one editor row per FRAME; snapping to the
real tick grid took Balloon from 77 files to ONE — a 400-second song in a single
SF2 at waveform/pulse 100/100/100. Sound Monitor's chord arps moved out of
per-note Hz captures into the row-header chord tables they came from (and the
arp acceptance guard turned out to accept anything on short notes — Fuck_Off's
drums had been playing garbage steps under a tolerance larger than the compared
window). The corpus settled at 27 parts and 99.23% strict. And in the gaps while
corpora rebuilt, the next arc opened the way every RE wishes it could: Glenn Rune
Gallefoss co-wrote the SID Duzz' It editor, and his own commented 1994-line
player source came off the staged floppy images readable — feature-flag
assembly, track and sequence bytes, instrument tables, all documented by the
man himself twenty years after the rips.

### v3.18.0 — The instrument-cap optimizer (2026-07-11)

The user supplied the theory: the original editors never had more than 32
instruments, so anything past that in our builds is capture artifact, not music.
The diagnostic proved it in one window — Dance part 1 held 10 real Sound Monitor
instruments spread across 31 driver slots, and the excess decomposed into exactly
the predicted classes: release waveforms landing at note-length-dependent
positions (one program per duration), frame-0 boundary bleed from within-frame
retriggers, and one filter capture per drive. Each got a register-exact-guarded
fix: gate-off rows now write the instrument's release waveform verbatim (a new
flag-gated driver feature — proven byte-identical when off), frame 0 normalizes
to the note's own waveform, and filter drives follow the $D417 routing bits
frame-by-frame because Sound Monitor re-routes its filter per section — the old
dominant-voice restriction had been silently discarding every re-attack in
re-routed sections. The deeper value was what the measuring exposed. A
global-delay sweep across EVERY part (not just part 1) caught all three voices
of every grid part ≥ 2 starting at row zero — an inter-voice desync that
per-voice-delay metrics are constitutionally unable to see, shipped unnoticed in
the v3.17.0 corpus. And porting Sound Monitor's write-stream onset lesson to DMC
found 24 of 88 files whose retriggers happen inside a single play call: the
state-based scan didn't just build them legato, it failed the onset-agreement
gate and dumped whole files onto the inferior tick-grid fallback — Balloon's
waveform channel went from 0% to 100% the moment the write stream became the
default. End state: 11 songs / 28 parts (was 52 at the arc's start, 32 at this
session's), 99.08% corpus-wide strict freq+wf, and the binder on every remaining
window is now the 64-bundle command channel — the next lever, already mapped in
the part-reduction plan.

### v3.17.0 — Sound Monitor: the seventh player, start to finish in one day (2026-07-10)

Chris Hülsbeck was eighteen when 64'er magazine printed Sound Monitor as eleven
kilobytes of hexadecimal numbers readers typed in by hand. Forty years later its
eleven Fun_Fun rips became the fastest player arc in this project's history: format
reverse-engineering, a validated parser, editable Driver-11 SF2s, AND native
byte-capture SF2s — all in one session. The luck: the whole cluster shares
byte-identical player code at a fixed load, so for once there was no relocation
puzzle — every table sits at a hardcoded address. The skill was in the traps it
still set. The row index wraps like the 6502 byte it is (four files "decoded empty"
until `row_start > row_count` was recognized as normal). The arpeggios don't live in
the pattern data or the instruments — they live in a chord bank INSIDE the row
header record, selected by the LAST data byte of each bar (the mysterious "+1
octave" mismatch was an octave arp caught mid-cycle, never a bug). A REST doesn't
mean silence: it writes the instrument's byte-8 release waveform, and some
instruments carry a GATED release — whole voices play legato through their "rests".
And the routine labeled "digi player" for half the session turned out to be the
arpeggio engine, while the "pitch bend" at $C31B was the pulse engine all along —
its dispatcher passes Y=2/9/16, landing STA $D400,Y on the pulse registers. Two
lessons earned the do-not-retry list: the runtime Driver 11 cannot parse the
editor's $90-$9F tie bytes (emitting them desyncs the sequencer into noise — the
editor and the driver disagree about their own file format), and the fix for
mixed-legato voices was already in the toolbox: the Supremacy $FB tie mechanism,
gated behind FM_CAP so it only splits notes the capture can't reach (Fuck_Off's
lead went 66.6 → 100 strict without doubling the part count). End state: Final_Luv
plays its entire 161 seconds from ONE native SF2 at 98.1–99.9 skew-tolerant on
every register; 11 songs, 52 parts, 1524 tests green.

### v3.16.0 — The part-reduction + fidelity-truth campaign (2026-07-10)

Two user goals, one campaign: fewer SF2 files per song, and the last 1–2 % of
fidelity — with measurement honest enough to tell a real defect from a metric
artifact. Split analysis showed ~95 % of song splits came from the 64-slot
(FM, pulse) bundle channel; the decompose probe showed the remedy is
player-shaped (Hubbard: 24 structural FM shapes/song, pulse-driven explosion →
the per-instrument pulse canonical takes Commando 45→4 and Monty 22→4 files,
byte-exact; DMC: diverse in both axes → clustering provably useless). The
Shockway-class "monsters" (638/274/188/112 parts) turned out to be a decode
bug: bit7 track TRANSPOSE commands read as pattern numbers — three ROM-verified
encodings later, Shockway builds 21 parts correctly for the first time, and the
class's self-modifying-rails pulse engine is decoded (fetch-frame step +
fx-arp re-fetch = the open residual, osc2 of 3 already at pulse 100).

The analyser grew four instruments — per-voice delay, skew-vs-content
classification, mismatch-cluster report, part-loop auto-cap — and immediately
paid for itself: the cluster report localized Supremacy's waveform residual to
three frame-runs and root-caused it (the engine writes wave-off $00 mid-rest;
the wave capture now extends across rests, cap 96→256) → **Supremacy sub1
94.3→99.9 on every register, Cybernoid II part01 100×3, Cybernoid I wf/pulse
100**, crown jewel intact. The same skepticism proved pulse_canon lossy for
captured-pulse classes (gated to hp_engine), caught two measurement lies
(part-loop tails; wrong window offsets), and validated that some "failing"
voices are ±1-frame write jitter — audibly exact (47.7 strict / 100.0
skew-tolerant). DMC reached 41/88 eligible (four more generations decoded);
builders auto-prune stale parts; docs/SF2.md carries the full auto-generated
inventory; the CI security scan is green after 199 consecutive red runs (eight
live findings repaired, archive/ excluded). Sound Monitor's arc is open.

### v3.15.0 — DMC (Demo Music Creator), the format that unlocks hundreds of tunes (2026-07-09)

The sixth from-scratch native player — and the highest-leverage so far. The
`SID/JohannesBjerregaard/` corpus (88 files, ~all Bjerregaard) is written in **DMC
(Demo Music Creator)**, Johannes Bjerregaard's own editor family (the DMC4 Player by
Brian/Graffity, '91) and one of the most-used C64 music tools ever — so a DMC parser
generalises across hundreds of HVSC tunes, not just this corpus. Ground truth: the DMC4
editor (disk images in `bin/DMC/`); the RE exemplar was **Balloon.sid**.

The format is a clean **Track → Sector → Sound** hierarchy — orderlist of patterns, per
voice; pattern events with a command byte (duration in the low 5 bits, flags/effect/sound
in the top three); 8-byte instrument records with a classic DMC pulse-width sweep. Like
Hubbard V2 the player relocates to many load addresses, so `sidm2/dmc_parser.py`
signature-locates every table rather than hardcoding it. But the character of DMC lives in
its **wavetable**: an arp table (`$1A00`) and a waveform table (`$1B00`) advanced *one step
per frame*, where the freq-table row is the wavetable value (`$80|note`, absolute). That
means DMC plays notes far above its own freq table via octave shift, and a single held note
can leap octaves frame to frame — a fast per-frame arp is the norm, not the exception.

That per-frame arp is precisely what the shared trace-driven pipeline's Hz-delta FM capture
reproduces bit-for-bit — but only if each note is triggered on the correct frame. So the
whole build hinges on **onset alignment**. `measure_onsets` emulates the player (the siddump
CPU plus a `$D012` raster fake and banking) and records every per-voice `$D404` gate-rise as
the exact onset frames; the native builder (`bin/build_dmc_native_song.py`, DMCShim → the
MoN `build_native_song`/`emit_one` engine) then places one note per emulated onset (`fpt=1`,
pitch = the trace-resolved absolute semitone via the full-range PAL table), so the captured
arp starts *in phase*. Getting the onset schedule right — and identifying the **global**
tempo counter rather than one of the per-voice ones — took **Rockbuster from ~65% to ~97%**
(waveform 87→100, pulse 100/100/100). The parser+decoder validate at **29/43 main-player
files ≥90%** per-voice onsets (the correct metric — a single global phase undercounts because
voices trigger a few frames apart within a tick); 21/43 files build onset-eligible, most two
of three voices at 90–100%.

The residual is documented honestly. **Fast-arp voices** whose base note gets pinned to a
1-frame attack spike at onset+1 (Wanna_Get_Sick osc1 @33%, Omega_Force_One) need per-voice
*adaptive* base-note resolution — no single fixed onset frame works for every voice. And a
tempting fix is a proven **dead end**: the Galway/MoN structural semitone-arp model *regresses*
here, because DMC's freq table isn't PAL-tuned and its arp steps aren't on-semitone, so
quantising to whole semitones is strictly less exact than the Hz-delta capture that already
reproduces DMC's per-frame frequency exactly. New `docs/players/DMC.md`; memory
`johannes-bjerregaard-player.md`. Rob Hubbard shipped the prior release (v3.14.0).
...
Docs-only. Consolidated four player ports' knowledge into `docs/players/PLAYBOOK.md`
(the staged porting method + technique catalog + caps + gotchas + checklist), wrote
the missing `MON.md`/`CLUSTERS.md`, rewrote `ACCURACY_MATRIX.md`/`README.md`/
`docs/INDEX.md` to the v3.13 state, and replaced `docs/ROADMAP.md` with the
prioritized optimization plan (native-driver unification, shared build/fidelity
libraries, registry-wiring the `bin/` players, universal trace-first fallback).

### v3.9.0 — Teaching the driver to read its own tables (2026-05-30)

v3.8.0 played Wizball, but it cheated. The "instruments" were a single waveform
byte; the pulse width was hidden in an instrument column; the pulse and filter
tables in the editor were empty. It *sounded* right because the driver had
hand-tuned synth code baked in — a player wearing an editor's clothes. Open the
wave or pulse view and there was nothing to edit, and editing what little showed
changed nothing. The user's verdict was blunt: *"missing instruments, wave."*

The fix was to stop cheating. A real SID Factory II driver doesn't hard-code its
sound — it *interprets tables*. So we read the actual ground truth (Galway's
real Wizball, traced cycle-accurately) and discovered its sound is three small
programs: a pulse-width that ramps `+8` every frame, and a resonant low-pass
whose cutoff sweeps down and restarts on every lead note. Then we read SF2II's
own Driver-11 documentation for the exact byte format of those programs, and
**ported the interpreters into the 6502 driver**: a wave-program runner, a
pulse-program runner (`8X` set / `0X` add / `7f` jump), and a global
filter-program runner (`9Y YY RB` set passband/cutoff/res/bitmask, then sweep).
The build now emits those tables in standard form, with instruments carrying
real pointers into them — and names, so the list isn't blank.

Every program was checked byte-for-byte against the original SID: the pulse ramp
`$800 → $808 → $810…`, the filter `$F1`/`$1F` with cutoff `$89 → $78`. The same
numbers Galway's own player produces, now coming out of editable tables. The
user confirmed it in stock SF2II: the wave, pulse, and filter tables all show
programs, and **editing a value changes the playback**. The driver finally reads
its own sheet music instead of playing from memory.

Two crashes were exorcised along the way, both invisible to the headless py65
harness and only fatal inside SF2II. One was SF2II's sequence unpacker
overflowing a fixed 1024-event buffer (Wizball's patterns expanded past it); the
other was the note-frequency table drifting, as the driver grew, across the
exact bytes SF2II rewrites every frame for playback state. The first was capped;
the second pinned out of the way with a build guard that now refuses to ship a
driver whose code spills into the danger zone.

What's still ahead: wiring this native driver in as the default Galway
conversion (it lives in the `bin/` build scripts today), faithful per-frame FM,
and full-length songs.

### v3.8.0 — Building a driver from nothing (2026-05-30)

Stage A made Galway *editable* by borrowing SID Factory II's Driver 11. v3.8.0
asks the harder question the user actually wanted answered: can we author our
**own** driver — the real thing, like the ones that ship with SF2II — so the
sound is Galway's and not Driver 11's approximation of it?

The first surprise was that the answer is *yes, without touching SF2II*. The
editor is data-driven: a driver `.prg` carries a descriptor (RE'd from SF2II's
own C++) that tells the editor where the tables are and how to edit them. So a
new driver is a pure 6502 + descriptor artifact. We stood up 64tass, wrote a
skeleton, and watched stock SID Factory II announce **"Driver loaded: Galway."**

Then came the sequencer — by hand, in 6502: three voices each walking an
orderlist into the sequence-pointer table into packed sequences; note durations;
per-instrument ADSR and waveform; transpose. The packed format was ported
byte-for-byte from SF2II's own `datasource_sequence.cpp`. Every increment was
validated two ways — py65 for register-exact behavior, and the in-repo
instrumented `bin/SIDFactoryII.exe` for "does it actually load and render."

That instrumented build earned its keep. A load attempt threw *"Emulation of
6510 code exceeded cycle window"* — the editor emulates the driver each frame
until it returns, and a parser loop could run unbounded. The fix was a guard;
the lesson (now in memory) was sharper: the editor *runs your driver*, so bound
every loop, and never trust a test harness whose own return-detection is off by
one byte (ours was, and it had been quietly lying).

The payoff: `bin/build_galway_native_song.py` takes a real Wizball SID, extracts
it through the Stage A IR, packs it into the native driver's tables, and
**Wizball plays through a driver we built from scratch** — its actual opening,
the three held A#4 notes, loaded in unmodified SID Factory II. A pulse-width
sweep gave the lead some Galway shimmer. What's left is the big one: porting
Galway's real FM/PM `SOUNDn` synth so it doesn't just play the right notes but
sounds like him. But the concept is proven end to end.

### v3.7.0 — Galway becomes *editable*: the Driver 11 transpile (2026-05-29)

For two releases Galway "worked" by **embedding the original player**: the
audio was faithful, but the SF2 editor view was a museum exhibit — you could
look at the notes, but editing them changed nothing, because the embedded
player, not the tables, made the sound. v3.7.0 is the first half of a deliberate
two-stage answer to "make a real Galway driver like the others."

The insight that shaped the plan: Galway's engine is a per-voice **bytecode
interpreter**, not flat tables, so it can never be *losslessly* remapped. But
the *score* (notes, durations, instruments) the extractor already recovers
**can** be transposed onto a real driver — and SID Factory II's own **Driver
11** is a real, editable, playable driver. So Stage A transpiles Galway → Driver
11: the editor edits the tables, and Driver 11 plays them. Faithful timbre is
deferred to Stage B (a native Galway driver running his actual synth).

Three things had to be gotten exactly right. **Pitch:** Galway's frequency
table turned out to be the standard PAL table at the same indices, so a note is
just `index + 1` (`$00` is reserved for note-off) — calibrated and asserted per
file. **Rhythm:** the raw duration bytes are already musical frame counts, so a
GCD-of-the-*common*-durations tempo reproduces the groove (a naive GCD collapses
to 1 on a single odd outlier). **The packed sequence format:** rather than trust
the repo's flaky legacy injector, the encoder was ported byte-for-byte from
SID Factory II's own `datasource_sequence.cpp`.

Then the GUI test taught the lesson the headless tests couldn't. The file
opened, showed notes and instruments — and was **silent**. The cause was a
single missing idea: Galway's leads are *pulse* waves, and a pulse with width
`$000` makes no sound. Adding a pulse-width program per instrument (from
Galway's `PINIT`) made it play, with correct notes, pitch, and timing —
**user-confirmed in SF2II**. What it still *doesn't* nail is timbre: pulse-width
modulation, vibrato, and filter live in Galway's per-frame synth, not in any
static table. That gap is exactly the case for Stage B.

1357 → 1389 tests. The honest accuracy line — notes correct, timbre approximated
— finally replaces the long-fictional "88-96%."
