...
**And earlier:** v3.10.0 (2026-06-20) — **Full-fidelity, full-length Ocean Loader.** The trace-driven build reproduces a **9-minute Ocean Loader 1 at ~100% on every SID register** by decoupling the per-note synth program from the instrument: the sequence command channel (`$c0-$ff`) selects a per-note **bundle** of (FM slide + pulse-width envelope), so a long song's many slide shapes no longer have to share the 32-instrument table (which had forced pitch-corrupting slide-clustering). The fidelity was *measured*, not assumed — the new `bin/sf2ii_vs_real.py` "listen" tool diffs an instrumented SF2II's real per-frame SID output against the original SID and exposed a pulse gap the pitch-only metric had hidden.

**And before:** v3.9.0 (2026-05-30) — Stage B2: the from-scratch native Galway driver becomes a **full standard table-driven SF2 driver**. The real Driver-11 table interpreters are ported into the 6502 driver — `wave_step` (waveform/semitone programs), `pulse_step` (set/add/jump pulse-width programs), and `filt_prog_step` (a global filter program started by an instrument flag). Standard-format wave/pulse/filter tables, real instrument columns + program pointers, instrument names; every table verified **byte-for-byte against the real Wizball SID**. User-confirmed in stock SID Factory II: all three tables render as editable programs, **editing a table changes playback**, and the instrument list is populated.
