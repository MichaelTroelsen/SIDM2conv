...
**Current version:** v3.10.0 (2026-06-20) — **Full-fidelity, full-length Galway.** The trace-driven native-driver build now reproduces a **9-minute Ocean Loader 1 at ~100% on every SID register** in stock SID Factory II. The breakthrough: **decouple the per-note synth program from the instrument.** Previously each instrument was keyed on `(waveform, AD, SR, FM-shape)`, so a long song's many slide shapes had to share the 32-instrument table — forcing slide-clustering that reused one note's slide for a whole cluster and corrupted pitch. Now the sequence command channel (`$c0-$ff`, previously a skipped no-op) selects a per-note **bundle** of (FM slide + pulse-width envelope); the instrument carries timbre only. Ocean Loader fits in 18 instruments + 32 bundles with no clustering. A 1-frame FM-trigger alignment fix (the driver had applied a note's first slide delta a frame ahead of the real SID) plus per-note pulse decoupling closed the last gaps. The fidelity was *measured*, not assumed: a new "listen" tool (`bin/sf2ii_vs_real.py`) diffs an instrumented SF2II's actual per-frame SID output against the real SID, per voice — it exposed a pulse gap (71/8/13%) the pitch-only metric had hidden. Result: osc1/osc2/osc3 all ~100% on freq, waveform, pulse, and AD/SR. Not yet the default conversion path (lives in `bin/` scripts); the C64 `$D000` memory wall caps one SF2 at ~9.2 minutes.

**Previous:** v3.9.0 (2026-05-30) — Stage B2: the from-scratch native Galway driver becomes a **full standard table-driven SF2 driver**. The real Driver-11 table interpreters are ported into the 6502 driver — `wave_step` (waveform/semitone programs), `pulse_step` (set/add/jump pulse-width programs), and `filt_prog_step` (a global filter program started by an instrument flag). Standard-format wave/pulse/filter tables, real instrument columns + program pointers, instrument names; every table verified **byte-for-byte against the real Wizball SID**. User-confirmed in stock SID Factory II: all three tables render as editable programs, **editing a table changes playback**, and the instrument list is populated.
**Earlier:** v3.8.0 (2026-05-30) — Stage B milestone: a **from-scratch native Galway SF2 driver** (6502 + descriptor, built with 64tass) loads in stock SID Factory II and plays a real Galway tune (Wizball) from editable tables — the full SF2 format implemented by hand (3 voices, orderlist chaining, durations, instruments, transpose), a self-diagnosed emulation-cycle bug fixed, and a pulse-width-modulation start on timbre. Stage A (v3.7.0) transpiled Galway onto the existing Driver 11 and stays the shipped converter.
