...
| Matt Gray → Driver 11 | **Driller build only**: sequencer onset **1513/1513 = 100%**, pitch **1513/1513 = 100%** on *plain* instruments over 240 s. Pitch-modulated notes (n=468) are **NOT claimed** — the player rewrites `$d400` every frame there, so onset is vacuously 100%; base-pitch agreement is 65.4% and that residual is the synth side | 🚧 Stage A only (`bin/`, not default). His OWN from-scratch driver (not Hubbard/Galway-derived), refined **per game** — 1 of 55 HVSC files located; `verify()` refuses unknown variants loudly. Driller loops at 11:05 > the SF2II memory wall, so Stage A emits **2 parts**; sequences round-trip byte-exact, and both parts **play-test clean in real SF2II** (6/6 short trials + full-duration passes, screenshot-verified via the editor's own "Playing time"). That clears the load/crash hazard only — Stage A is knowingly missing the slide/arp/PWM/drum engine, so it will NOT sound like the original; timbre is a Stage B claim. Gotchas: PSID `load=0`; `music_init` ignores the accumulator (both subtunes play tune 1); Tusker's `play=$e002` is under KERNAL ROM. See `docs/players/MATTGRAY.md` |
...
| Sound Monitor (Hülsbeck) → native driver | corpus strict sweep **99.25% freq+wf** over **all 27 of 27 parts** (`pyscript/soundmonitor_sweep.py`, re-derived 2026-07-30 — supersedes the prior 99.23%/26-parts figure). freq+wf = best 2 of 4: pulse 96.67, filter 97.33 ✻ (doc-carried, not yet re-aggregated by the new sweep). 100.0 on every register: Fuck_Off 242s + Dance parts **03/04 only** | ✅ 11/11 Fun_Fun files build, 11 songs/27 parts (`bin/`, not default); **reproducible from a fresh clone** (R21) via tracked `pyscript/soundmonitor_sweep.py` + `test_soundmonitor_sweep.py`; see `docs/players/SOUNDMONITOR.md` |
