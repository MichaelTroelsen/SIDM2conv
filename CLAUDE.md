...
| HardTrack Composer (Longhair/Brush) → Driver 11 (Stage A) | Sequencer decode only, vs siddump on byte-exact freq registers: **88.4%** where the instrument lets the sequencer own the pitch, **2.6%** where the instrument's wave program drives `$D400/$D401` itself (field-5 bit 7). **Two columns, never averaged** — the second is the unmodelled synth engine. 8/33 files exactly 100.0%; residual is flat across time (no drift) | 🚧 RE + Stage A (`bin/hardtrack_to_sf2.py`, not default); no Stage B. **Stage A 90.6% is ABOVE the parser's 88.4% — an artifact, not an improvement** (the original's wave program bends pitch off the table value and misses; Stage A doesn't port that modulation and hits). Compare only in the LOSING direction. 6 of 8 parser-100% files are Stage A 100.0%. The apparent losses are **diagnosed as a systematic +1-frame lag** (68.9% of notes) clipped by the 8-frame window, not lost music — the renders play the identical arpeggio one frame apart. **Root cause: the shared Driver 11 template ships `$16CC=$40`; bit 6 makes the driver spend its FIRST play call on state init — so EVERY Driver 11 Stage A build in this repo starts one frame late.** Removing that phase puts Zakplus/Hopscotch exactly on the parser's score (never losses). **Over the notes the PARSER resolves, lag-corrected, Stage A retains 99.59%** (21 lost of 5,167) — quote that, not the diluted 90.9%. `$6F` legato (pitch changes WITHOUT restarting the instrument's programs; 24.7% of all note events) was 58% of the losses and is now fixed via a duplicate instrument slot starting at the wave program's SETTLED step: 25→3 lost, plain-note losses unchanged at 18. 18 losses unattributed (Ritual_II_tune_2, Walk_to_Soul). Three hypotheses were FALSIFIED on the way (cross-pattern gate state; hard-restart flag; wrapper play address). **The window has no plateau — always quote it.** Table addresses are **patched per file**, so everything is recovered by code signature — never by load+constant. 5 of 38 corpus files are refused (wrapped/multi-instance rips). `SID/Shogoon/` is **mixed-player**: 38 of 150 files. See `docs/players/HARDTRACK.md` |
