...
| SID Duzz' It (Gallefoss/Tjelta) → Driver 11 (Stage A) | strict onset+pitch medians: A 98.3, D 100, **C 86.0** (was 66.7), B 74.8, **E 50.8** (was 47.5; 114 files incl. the harder wrapper gen), **DELTA win 89.8/strict 55.5** (8 files, zp + page-$03 sub-variant), V 21.8 | 🚧 **320 located, 344 Stage A SF2s** (0 failures, `bin/`, not default); C walk decoded + E $Cx trailing-delay fix + DELTA-class play+3 wrapper cracked + multi-subtune (A/C/E) + "sixth layout" wrapper cracked; native Stage B TODO; see `docs/players/SDI.md` |
