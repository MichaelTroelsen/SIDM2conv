...
| Mainstream MoN / Jeroen Tel (SID/Tel_Jeroen, 179 files) → mon_parser | B1-indirect bucket: 6/24 onset-EXACT (05-09-87, Ikari_Union, Lost_in_China, Scout, Trying_Out_2_v1, Trying_Out_3); Alloyrun 61%, Starball 76%, Bantam 73% (all up from ~1-3%) | 🚧 NEW MoN GENERATION cracked (ab99032): `_locate_b1` + `_voice_blocks_tel`. Same-day follow-up `_locate_b1_row_variant`: orderlist REPEAT op + pattern-index off-by-one + classic `_pattern()`-grammar reuse + row-ctrl off-by-one (Bantam), all signature-gated (disassembly-confirmed, zero change to untouched bucket files each time). Middle tier (60-88%) + Alloyrun/Bantam's own residuals = instrument arp/legato (Stage-B); remaining broken files (Zynon_Zak, Tel_1, Monitor_Madness_1/2, Trying_Out_2, Chrome_Met1) are each their own compiled micro-variant TODO; 85-file no-copy bucket untouched. See `memory/mainstream-mon-tel.md` |
