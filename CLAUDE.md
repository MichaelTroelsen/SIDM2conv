...
| Blackbird / lft → native driver | 16-file v1.2-exact corpus **mean 99.84** register-accuracy vs the validated simulator; **10 files at exactly 100.0**, only `Into_the_Unknown` (98.1) below 99. Glyptodont **162/162 note-ons** (0 missing, 0 spurious), arming 100% on every file | ✅ B1-B25 + E3e/E3f/E3g + E4 + E5 (`bin/`, not default); **not wired into `DriverSelector`**. E3f combo fx values were briefly disabled on a since-**falsified** SF2II-crash report (`BB_NO_COMBO=1` disables). E4 (prepare1's byte allowance is forfeited by prepare2) + E5 (a third filter row type: hardware writes `$D418`/`$D417` every frame whatever the cutoff op is, so an ADD row0 must still set mode/res) took `To_Die_For_II` 94.2→98.2→**100.0** and `Revolutions_Delivered` 99.0→**100.0**. Corpus sweep is tracked + tested (`pyscript/blackbird_sweep.py`, `pyscript/blackbird_crash_probe.py`, 34 tests) — **reproducible from a fresh clone**; see `docs/players/BLACKBIRD.md` |
