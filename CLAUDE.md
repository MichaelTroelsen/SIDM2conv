...
| Mainstream MoN / Jeroen Tel (SID/Tel_Jeroen, 179 files) → mon_parser | B1-indirect bucket: **11/24 onset-EXACT** (was 6): 05-09-87, Ikari_Union, Lost_in_China, Scout, Trying_Out_2_v1, Trying_Out_3, Happy_JT, Beginning, Beginning_v2, DemoSong, Reggae_Example. Plus Orion_Intro 92%, Trying_Out 78%, Zynon_Zak 84%, Bantam 77%, Starball 76%, Alloyrun 61% | 🚧 NEW MoN GENERATION cracked (ab99032): `_locate_b1` + `_voice_blocks_tel`. Same-day `_locate_b1_row_variant`, 3 rounds: orderlist REPEAT op + pattern-index off-by-one + classic `_pattern()`-grammar reuse (Alloyrun/Starball) → row-ctrl off-by-one (Bantam) → **the row length-mask was hardcoded `$1F`, real per-file masks vary `$3F`/`$7F`** — 5 files filed as "Stage-B middle tier" turned out to be THIS decode bug, not Stage-B, and jumped straight to EXACT. All signature-gated (disassembly-confirmed, zero change to untouched files each round). Remaining broken files (Tel_1, Monitor_Madness_1/2, Trying_Out_2, Chrome_Met1 — confirmed genuinely `$1F`) are each their own compiled micro-variant TODO; 85-file no-copy bucket untouched. See `memory/mainstream-mon-tel.md` |
