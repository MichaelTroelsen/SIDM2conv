...
| Matt Gray → Driver 11 (Stage A) + native (Stage B) | **Stage B: 16 tunes across 3 games.** Register state vs the original (Last Ninja 2 sub 0, all frames): waveform/AD/SR/filter/`$D418` **100.00%**, freq **92.7/92.9/100.0** — the residual is 192 frames/voice where BOTH sides carry waveform `$00`, so the oscillator is silent and it is inaudible. Stage A: Driller onset+pitch **1513/1513** on *plain* instruments; pitch-modulated notes (n=468) **NOT claimed** (base-pitch 65.4%) | 🚧 `bin/`, not default. **Read `docs/players/MATTGRAY.md` before quoting any of this** — two of its own published claims have been retracted by measurement. Non-negotiables: the builder's **`audible` column is gate-on only and is BLIND to release-tail defects** (it read 99.5-100% through 236 wrong release runs); a **raw waveform correlation has a 0.58-0.74 floor** on byte-identical material, so it is never evidence — quote the phase-invariant measures (0.97-0.99 floor). The audio gap is **sub-millisecond write PHASE and is inaudible** — level, harmonics, per-octave energy and envelope all sit at the floor, while the per-window spectrogram loses 0.035 for **23 microseconds** of jitter, so **never quote it as an audio-fidelity figure**; measure the floor **per voice** (0.937-0.999 on one tune). All 16 decode via `layout='signature'`, not the validated `driller` path, but that decode is **no longer unverified** — the paths agree in every field where both run (n=1), and the located tables explain **100.0%** of sounded AD/SR pairs on all 8 subtunes; that validates the **tables**, not the sequencer walk. 1 of 55 HVSC files located by the fast path; sub 7 refused |
| DMC (Demo Music Creator, Johannes Bjerregaard) → native driver | **Balloon 77 parts → ONE 400s SF2, wf/pulse 100×3 over the FULL 400s** (n=19996/voice, freq 80.6/100/97.7); Rockbuster ~97/100/100 *on part 1 of 16*. **Every % is window-dependent** (one file reads 100/89/95 @6s, 44/38/39 @20s) — quote the window | 🚧 **Corpus (2026-08-15, all 88): 74 scored, medians freq 97.3 / wf 100.0 / pulse 100.0 over 222 voices, 64 below 90.** The previous figure (94.7, 87 below 90) is **superseded**: a quarter of that tail was the sweep scoring a 7 s part over 90 s, because it re-parsed the builder's log and took a DISCARDED trial split. The 64 split as **22 underpowered / 6 pitch-only / 22 whole-build / 14 mixed**, and the **pitch-only class is now SOLVED**: DMC is the third player to collide with the driver's `$40-$43` SCALED-vibrato marker (`Balloon`'s two-octave arpeggio emits `+16833 = $41C1`), after Hubbard's drum dives and HardTrack's percussion. `no_fm_scale = 1` — `Balloon` v0 **80.6 → 100.0** over n=19,996, corpus freq median **98.4**, 56 below 90, **12 voices improved and 0 regressed**. `Flimbos_Quest_main` turned out **not to be a fidelity defect**: its parser decodes NO notes (4,100 rests per voice), so it shipped 34 parts of silence and was scored 0.2 — `build_native_song` now refuses a decode with no note on any voice — **3 files** (it, `Kamikaze`, `Nightdawn`; the last read a **vacuous 100.0** from a constant held on both sides over 46 frames, so two separate counts of this were wrong before the corpus run settled it). Corpus: scored 74 → 70, freq median **98.7**, 47 below 90. What remains is the rest of the 17-song whole-build queue. Part 1 only. 56/88 build-eligible = a **mode count, NOT accuracy** (an eligible file can still score 39%); all build (`bin/`, not default). **`$D418` passband: 26 of 57 shipped builds were wrong** (rebuilt → 55/57); `bin/_dmc_fidelity.py` scores freq/wf/pulse so it is BLIND to it, and it is **untracked** — every DMC number here comes from a script not in the repo. 3 files emit no passband at all (static filter → zero bundles) but **`$D417` routes NOTHING**, so it is inaudible — quote the routed fraction, not just the mode match. See `docs/players/DMC.md` |
...
| SID Duzz' It (Gallefoss/Tjelta) → Driver 11 (Stage A) + native (Stage B) | strict onset+pitch medians: A 98.3, D 100, C 86.0, B 74.8, E 50.8, DELTA 89.8 win / 55.5 strict, V 21.8. n: A 50 B 43 C 80 D 18 E 118 DELTA 8 V 6. **Only A+D are unfitted** (C/E/DELTA/V pick a timing model best-of-N) | 🚧 **343 locate → 348 Stage A SF2s → 324 sweep-validated — three different denominators, don't conflate.** "0 failures" = emitted OK, **not** fidelity: 274/324 ship some default instrument data. ⚠️ **`$D418` passband was NEVER captured until 2026-08-13** — 15 of a 30-file sample rendered low-pass where the original selects BP/LP+BP/LP+HP/LP+BP+HP, routed 100%; builder fixed and **corpus rebuilt**: **258 of 281 now select the original's passband** (7 unexercised, **0 unconfirmed**, 7 failed). The 29 unconfirmed rows are gone because the builder now records each part's window beside the artifact (`.span`, PATTERNS F8) — and resolving them **raised** the failure count 3 → 7, which is the guard working: over-run manufactures disagreement, it never hides it. `Juba-Jazz` **FIXED** (52.8% → 100.0%): its filter is enabled by `$D417` routing + `$D418` alone with the **cutoff held at 0**, and `detect_filter_drives` keys on cutoff jumps, so nothing was credited — PATTERNS **F9**, a detector blind to the same event in another register. The `VE-4x` **V path is CLOSED (5/5)**: `v_traces` now records `$D418`, but half the defect was the *measurement* — siddump calls the player once per frame where a V rip's own IRQ runs `v_mult`, so it samples a different instant of a passband that alternates within the frame. `passband_check` gained a per-FILE `sdi_v` reference (Blackbird's `sim` pattern); the other 276 rows are untouched. Build outcomes unchanged (261/62/118 vs 262/62/117). Native **Stage B: 262 of 441 build, 786 voices**, medians A/B/DELTA 99.9, E 99.7, C 98.1, V 96.8, **D 95.9 (7 of its 15 voices <90)** — tracked corpus sweep. The builder now prints `n`, but it is the **song length, identical on all 3 voices** (siddump holds freq through rests), so it does NOT say whether a given voice carried information. See `docs/players/SDI.md` |
...
**Size**: **34.4 KB / ~8.6k tokens** — this file is loaded into **every** session, so measure it in **bytes**, not lines. It had drifted to 32.5 KB while *staying inside* a ~226-line budget, because prose migrates into table cells and a line count cannot see that: **one** HardTrack table row was 6,696 bytes, 21% of the whole file. Trimmed back in v3.26.0 after verifying all 23 of its claims survive in `docs/players/HARDTRACK.md`. Keep per-player rows to a **verdict + the caveats needed to quote the numbers correctly + a `docs/players/` link** — mechanism, evidence and history belong in the player doc. | **For full docs**: See README.md and docs/
