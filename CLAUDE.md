...
| Sound Monitor (Hülsbeck) → native driver | corpus strict sweep **99.25% freq+wf** over **all 27 of 27 parts** (`pyscript/soundmonitor_sweep.py`, re-derived 2026-07-30 — supersedes the prior 99.23%/26-parts figure). freq+wf = best 2 of 4: pulse 96.67, filter 97.33 ✻ (doc-carried, not yet re-aggregated by the new sweep). 100.0 on every register: Fuck_Off 242s + Dance parts **03/04 only** | ✅ 11/11 Fun_Fun files build, 11 songs/27 parts (`bin/`, not default); **reproducible from a fresh clone** (R21) via tracked `pyscript/soundmonitor_sweep.py` + `test_soundmonitor_sweep.py`. NB the native (Stage B) path is clean; **2 STAGE A files are missing music** (`Dance_at_Night_remix` −93 sequences, `Dreamix` −12) via the Driver 11 128-slot cap — a different builder, does not touch the 99.25%; see `docs/players/SOUNDMONITOR.md` |
