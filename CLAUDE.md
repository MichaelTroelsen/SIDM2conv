...
audio-tightness.bat orig.sid conv.sf2 --driver-init 0x1000 --driver-play 0x1003 --voice all  # + per-voice sweep, isolation guard, registers x audio cross-tab
...
| Maniacs of Noise (Jeroen Tel) → native driver | 100% byte-exact (Hawkeye sub 2/3) on freq/wf/pulse/**cutoff**. `$D418` was scored by NOTHING until 2026-08-07 and was wrong on 3 of 8 targets — now Cybernoid_II 0→99.7%, Hawkeye 82.5→99.9%, Supremacy sub0 0→exact | ✅ Hawkeye + Cybernoid I/II + Myth + Supremacy all build (`bin/`, not default). The passband was never captured (builder hardcoded low-pass; the driver could always carry all 3 bits) and the master volume was hardcoded `$0f` (Supremacy sub0/sub1 actually play at 6 and 8). Both fixed. Read older "filter 100%" figures as **cutoff only**. Open: Supremacy `adsr` 2.3%/4.5% on osc2/osc3, visible only now the envelope is scored; see `docs/players/MON.md` |
