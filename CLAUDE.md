...
| Mainstream MoN / Jeroen Tel (SID/Tel_Jeroen, 179 files) → mon_parser | B1-indirect bucket: 6/24 onset-EXACT (05-09-87, Ikari_Union, Lost_in_China, Scout, Trying_Out_2_v1, Trying_Out_3) | 🚧 NEW MoN GENERATION cracked (ab99032): `_locate_b1` + `_voice_blocks_tel` (bit7=transpose, bit6=gate-off, bit7-row=instr cmd). Middle tier (60-88%) = instrument arp/legato (Stage-B); broken tier (<40%, Alloyrun) = orderlist $40-$7F + $C0-$FF ctrls TODO; 85-file no-copy bucket untouched. See `memory/mainstream-mon-tel.md` |
