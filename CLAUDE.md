...
| SID Duzz' It (Gallefoss/Tjelta) → Driver 11 (Stage A) + native (Stage B) | strict onset+pitch medians: A 98.3, D 100, C 86.0, B 74.8, E 50.8, DELTA 89.8 win / 55.5 strict, V 21.8. n: A 50 B 43 C 80 D 18 E 118 DELTA 8 V 6. **Only A+D are unfitted** (C/E/DELTA/V pick a timing model best-of-N) | 🚧 **343 locate → 348 Stage A SF2s → 324 sweep-validated — three different denominators, don't conflate.** "0 failures" = emitted OK, **not** fidelity: 274/324 ship some default instrument data. ⚠️ **`$D418` passband was NEVER captured until 2026-08-13** — 15 of a 30-file sample rendered low-pass where the original selects BP/LP+BP/LP+HP/LP+BP+HP, routed 100%; builder fixed and **corpus rebuilt**: **258 of 281 now select the original's passband** (7 unexercised, **0 unconfirmed**, 7 failed). The 29 unconfirmed rows are gone because the builder now records each part's window beside the artifact (`.span`, PATTERNS F8) — and resolving them **raised** the failure count 3 → 7, which is the guard working: over-run manufactures disagreement, it never hides it. `Juba-Jazz` 52.8%/661 audible is the real one. The `VE-4x` **V path is CLOSED (5/5)**: `v_traces` now records `$D418`, but half the defect was the *measurement* — siddump calls the player once per frame where a V rip's own IRQ runs `v_mult`, so it samples a different instant of a passband that alternates within the frame. `passband_check` gained a per-FILE `sdi_v` reference (Blackbird's `sim` pattern); the other 276 rows are untouched. Build outcomes unchanged (261/62/118 vs 262/62/117). Native **Stage B: 262 of 441 build, 786 voices**, medians A/B/DELTA 99.9, E 99.7, C 98.1, V 96.8, **D 95.9 (7 of its 15 voices <90)** — tracked corpus sweep. The builder now prints `n`, but it is the **song length, identical on all 3 voices** (siddump holds freq through rests), so it does NOT say whether a given voice carried information. See `docs/players/SDI.md` |
...
**Size**: **34.0 KB / ~8.5k tokens** — this file is loaded into **every** session, so measure it in **bytes**, not lines. It had drifted to 32.5 KB while *staying inside* a ~226-line budget, because prose migrates into table cells and a line count cannot see that: **one** HardTrack table row was 6,696 bytes, 21% of the whole file. Trimmed back in v3.26.0 after verifying all 23 of its claims survive in `docs/players/HARDTRACK.md`. Keep per-player rows to a **verdict + the caveats needed to quote the numbers correctly + a `docs/players/` link** — mechanism, evidence and history belong in the player doc. | **For full docs**: See README.md and docs/
