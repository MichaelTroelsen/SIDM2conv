...
| DMC (Demo Music Creator, Johannes Bjerregaard) → native driver | Corpus (all 88 rebuilt 2026-08-15): 70 scored / 14 no-tables / 4 refused, **210 voices**. Raw freq median **99.5** (34 below 90); **audible** (gate-on) median **100.0**, 14 below 90, **128 voices at exactly 100**. `Balloon` 400 s in ONE SF2, 100/100/100 ×3 over n=19,996 | 🚧 `bin/`, not default. **Quote BOTH columns.** The 2026-08-15 timing fixes (`PATTERNS.md` **F10** capture-onset snapped to the *previous* note; **F11** only legato voices got the leading rest that records where a voice starts) moved 39 voices up and 26 down in **raw** — and **not one of the 26 is an audible regression** (22 at ≥99.5): a voice at its true entry frame spends its pre-entry frames on rest rows, inaudible on both sides. Raw alone reads the fix as 26 regressions; audible alone hides `Dreaming_2` v3, the corpus's one genuinely silent voice (raw 61.4, **audible 0.0 wf** over n=75 — our gate never opens). Biggest gains `Chase_v2` v0 **0.0 → 100.0**, `Hit_the_Baze` v0 0.0 → 99.2, `Blue_Monday_88` v0 8.1 → 98.8. **Part 1 only, and every % is window-dependent** — quote the window. `$D418` is scored by NONE of these columns (`pyscript/passband_check.py --player dmc`). Build-eligibility is a **mode count, not accuracy**. The 3 refused decodes are a **false locate**, not a variant (tables outside the loaded image, or `sector_lo == trk_lo`). See `docs/players/DMC.md` |
