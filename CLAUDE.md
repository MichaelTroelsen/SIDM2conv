...
| Blackbird / lft → native driver | 16-file v1.2-exact corpus **mean 99.67** register-accuracy vs the validated simulator; **8 files at exactly 100.0**, none below 98.1. Glyptodont **162/162 note-ons** (0 missing, 0 spurious), arming 100% on every file | ✅ B1-B25 + E3e/E3f/E3g + E4 (`bin/`, not default); **not wired into `DriverSelector`**. E3f combo fx values were briefly disabled on a since-**falsified** SF2II-crash report (`BB_NO_COMBO=1` disables). E4 (prepare1's byte allowance is forfeited by prepare2) took `To_Die_For_II` 94.2→98.2; the two laggards `To_Die_For_II` 98.2 + `Into_the_Unknown` 98.1 are now **filter/freq only**, not envelope or pulse. Corpus sweep is **untracked scratch**, not reproducible from a fresh clone; see `docs/players/BLACKBIRD.md` |
