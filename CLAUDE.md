...
ab-listen.bat stage orig.sid conv.sf2 [--voices] && ab-listen.bat chips [--onset] && ab-listen.bat patterns && ab-listen.bat instrmap && ab-listen.bat build --chips build\listen\chips.json && ab-listen.bat serve  # gapless BLIND A/B listening page: both renders play in lock-step and a button swaps which is audible (a media player can't do that, and a comparison that loses its place is a comparison of two memories) + blind scoring + auto sync-offset + amplitude envelopes (whole song and per voice) + precomputed dual spectrogram. SERVE it -- fetch() is blocked on file://, so an opened file plays but never draws. Rail numbers are quoted from a --chips JSON, never scraped from a report; the page measures nothing itself
...
**Note**: "SidFactory_II/Laxity" routes to the **Laxity driver**, and the older claim that it goes to Driver 11 was **wrong** — MEASURED 2026-09-09, **63 of 64** such files embed a locatable Laxity frequency table, so they carry NP21 player code. `SidFactory/Laxity` is the opposite case (**0 of 28**) and still routes to Laxity only because nothing has measured whether Driver 11 serves it better — it carries no SF2 payload either (0 of 28 contain magic `0x1337`). Outputs: `output.sf2` + `output.txt`.
...
**Fidelity harness** (`sidm2/fidelity_common.py`): the shared measurement plumbing every scorer should route through — **do not write a new one**. Three guards that answer different questions, and you need all three: `score_pct(ok, tot)` returns **None** (never 100.0/0.0) when `tot == 0` — *were there any frames?* — and `exercised(a, b)` returns False when both series are the same single constant — *did those frames carry information?* The second is not optional: **siddump force-displays every register on its first row** whether the playroutine wrote it or not, so a tune that never filters yields a full-length non-None series of zeroes on both sides and scores a confident 100%. That exact bug was fixed, re-appeared one layer down, and was caught by *running the tool* both times. Five separate copies of the same weighted-accuracy scheme existed in this repo, each independently broken — one scored **two identical captures at 50%**. The third, `underpowered(n)`, asks *were there ENOUGH frames?* — a voice compared over 46 frames printed a 100.0 identical to one compared over 6,000, so a corpus table averaged a 1-second stinger against a 2-minute song; `fmt_pct(p, n=...)` suffixes those with `!` and callers that omit `n` are byte-identical. Also here: an A/B baseline mode (`result_row`/`ab_pair`/`compare_runs`) that refuses on mismatched *measurement* settings but surfaces mismatched *build options* as "the change under test", a dimension registry so a report can generate — not hand-maintain — the list of registers **nothing it measured reads**, and `output_digest` so "no number moved" can be told apart from "the build never changed". A third helper, `shape_agreement`, is the phase-invariant companion for a **swept** register (pulse width, cutoff): movement count + travel, so a sweep that is correct but a few frames late stops reading as a dead engine (`5_Title_Tunes` osc3 pulse 4.5% strict = a -3-frame offset). Necessary, not sufficient — print it beside the strict number, never instead. See `PATTERNS.md` D4/D9.
...
**A/B page measures + cards (`pyscript/abpage_chips.py`, `abpage.py chips|patterns|instrmap`)**: four subcommands feed the page, and **each refuses rather than guesses**. `chips` measures the ALREADY-STAGED WAVs (no renderer — enforced by a test that makes every `subprocess` entry point raise), routing through `audio_listen.extract_features` + `audio_tightness.analyze_tightness_files`; **no chip is a score**, and each carries a tooltip naming what it is blind to, because *which feature is informative is DEFECT-DEPENDENT* — A-weighted level is the strongest discriminator (1.5-1.6x) on timing/percussive defects and **measured noise (0.011) on a pitch defect**, where chroma fires instead (0.072), so a null dBA does **not** clear a tuning error. **`--onset` is opt-in and carries NO GATE**: a 99.8%-register-exact build measured **64.7%** against an 85-91% original-vs-itself floor, so it is meaningful only against a baseline of the same tune (the floor costs 9 re-renders and is not computed). Two silent renders emit "both silent", never `0.0` — the vacuous-agreement shape `fidelity_common.exercised()` exists to catch; an all-zero chroma emits "no pitched energy", never a distance. `instrmap` (opt-in, two emulations/song) shells to `instrument_map_report.py --json` and **emits no table when `key_reliability()` fails** — roughly a third of files; a row marked `unused both sides` is **NO EVIDENCE**, neither agreement nor defect (Angular: 0 differ / 9 agree / **18 no-evidence**). `patterns` reads rows via `sf2_viewer_core.SF2Parser` (a **path**, not bytes; `.sequences`, *not* `get_sequences()` which returns 0) and places them by the tempo **written in the file** — drift against the audio is left VISIBLE because that drift *is* the row rate being wrong; a track decoding to rows that advance no time is **refused** (Angular seq 1: 667 entries, 663 zero-duration, against a declared length of 75 — a decoder over-read). `stage` refuses a silent renderer change over existing stems (`--allow-renderer-change` to override).

**A/B listening pages (`pyscript/abpage.py`, `ab-listen.bat`)**: the human-in-the-loop end of the same question — a page that plays both renders in lock-step and swaps which is audible, with blind scoring. **stdlib only** (its own FFT), so `build`/`serve` work without this repo's requirements installed; `stage` is the only half that needs them, and it renders through `audio_tightness_tool`'s `resolve_input`/`_render_muted` rather than any new render path, so ONE renderer drives both sides. Ported from the h2g repo's `abpage.py` (analysis: `docs/H2G_FIDELITY_APP_ANALYSIS.md`, tracked despite the repo-wide `*_ANALYSIS.md` ignore via a `!docs/*_ANALYSIS.md` negation in `.gitignore`). Three things it deliberately does NOT do: quote a number it computed itself (the rail comes from a `--chips` JSON; a scraped Markdown table degrades *silently* into an empty rail that reads like a clean result), claim its `mean |Δ|` is a score (it is amplitude — blind to pitch, timbre and filter), or claim `--voices` is isolation (it is a muted render; CLAUDE.md's standing caveat, now printed on the page itself). Serve it — `fetch()` is blocked on `file://`, so an opened file plays but never draws, and the bundled server honours HTTP Range because a 200-for-everything handler makes an `<audio>` element replay its opening seconds, which reads as a stale render.
...
| Maniacs of Noise (Jeroen Tel) → native driver | 100% byte-exact (Hawkeye sub 2/3) on freq/wf/pulse/**cutoff**, and now `$D418` | ✅ Hawkeye + Cybernoid I/II + Myth + Supremacy build (`bin/`, not default). ✅ **`$D418` passband: 22 of 27** (2026-08-20, after the glob was widened to every scoreable artifact — 4 never exercise the filter, and `Cybernoid_II_sub0_native` **fails at 84.6%**, a real defect no earlier run could see). **The older 17/19 and 18/20 are RETRACTED**: both were measured over `*_sub0_part01` only, which silently dropped every non-sub0 subtune and every `_native` build, so they were a non-representative subset, not a smaller corpus. 7 were stale artifacts; **`Myth` was a BUILDER gap** — still 0.0% after rebuilding with its own builder, whose py65 `capture()` never recorded `$D418` (fixed there, same clock as `ftr`). See `PATTERNS.md` F7. **Read older "filter 100%" figures as cutoff only** — `$D418` was scored by nothing until 2026-08-07. **Myth needs `bin/build_myth_native_song.py`**, not the generic `build_mon_native_song.py`. A "register-exact but SYNTHESIS on every voice" reading was **FALSIFIED** — it was metric noise; quote `--repeat-floor` beside any per-voice audio number (PATTERNS.md F5b). See `docs/players/MON.md` |
...
| Matt Gray → Driver 11 (Stage A) + native (Stage B) | **Stage B: 8 songs, 37 built (78 parts).** Register state vs the original (Last Ninja 2 sub 0, all frames): waveform/AD/SR/filter/`$D418` **100.00%**, freq **92.7/92.9/100.0** — the residual is 192 frames/voice where BOTH sides carry waveform `$00`, so the oscillator is silent and it is inaudible. Stage A: Driller onset+pitch **1513/1513** on *plain* instruments; pitch-modulated notes (n=468) **NOT claimed** (base-pitch 65.4%) | 🚧 `bin/`, not default. **Read `docs/players/MATTGRAY.md` before quoting any of this** — it has retracted 2 published claims. Non-negotiables: the builder's **`audible` column is gate-on only and is BLIND to release-tail defects** (it read 99.5-100% through 236 wrong release runs); a **raw waveform correlation has a 0.58-0.74 floor** on byte-identical material, so it is never evidence — quote the phase-invariant measures (0.97-0.99 floor). The audio gap is **sub-millisecond write PHASE and is inaudible** — level, harmonics, per-octave energy and envelope all sit at the floor, while the per-window spectrogram loses 0.035 for **23 microseconds** of jitter, so **never quote it as an audio-fidelity figure**; measure the floor **per voice** (0.937-0.999 on one tune). 36/37 decode via `layout='signature'` (1 `driller`); that decode is **no longer unverified** — the paths agree in every field where both run (n=1), and the located tables explain **100.0%** of sounded AD/SR pairs on all 8 subtunes; that validates the **tables**, not the sequencer walk. Probe 13/55 = decode 13/55, built 8/55 (MATTGRAY.md); `driller`: 2/55 (1/54 excl. itself). Sub 7 refused |
| DMC (Demo Music Creator, Johannes Bjerregaard) → native driver | Corpus (85/88 rebuilt 2026-08-15; `Test`/`Fourth_Dimension`/`First_Try_PSX` predate it): 70 scored / 14 no-tables / 4 refused, **210 voices**, **991** on-disk parts (not 988, a `-j8` flake). Raw freq median **99.5** (34 below 90); **audible** (gate-on) median **100.0**, 14 below 90, **128 voices at exactly 100**. `Balloon` 400s in ONE SF2, 100/100/100 ×3 | 🚧 `bin/`, not default. **Quote BOTH columns.** The 2026-08-15 timing fixes (`PATTERNS.md` **F10** capture-onset snapped to the *previous* note; **F11** only legato voices got the leading rest that records where a voice starts) moved 39 voices up and 26 down in **raw** — and **not one of the 26 is an audible regression** (22 at ≥99.5): pre-entry frames sit on rest rows, inaudible both sides. Raw alone reads the fix as 26 regressions; audible alone hides `Dreaming_2` v3's one silent voice (raw 61.4, audible 0.0). **Zero of 70 DMC songs respond to `FILT_LEAD`/`FILT_EXACT_PB`** — 169ef09's "+3 parts" cost was this `-j8` flake; scoped to SDI only. **Part 1 only, and every % is window-dependent** — quote the window. `$D418` is scored by NONE of these columns (`pyscript/passband_check.py --player dmc`). Build-eligibility is a **mode count, not accuracy**. The 3 refused decodes are a **false locate**, not a variant (tables outside the loaded image, or `sector_lo == trk_lo`). See `docs/players/DMC.md` |
...
| Blackbird / lft → native driver | 16-file v1.2-exact corpus **mean 99.96** vs the validated simulator; **11 of 16 at exactly 100.0**, none below 99.8. NOTE the 16 are **not a chosen subset** — they are **every LFT rip `locate_blackbird` supports** (16 of 61 on disk; the other 45 are unlocatable variants). Glyptodont **162/162 note-ons** | ✅ B1-B25 + E3e/f/g + E4/E5/E6 (`bin/`, not default). `$D418` passband **16/16 correct** — but only measurable against the **simulator**: siddump cannot drive an LFT rip at all, and comparing against that silence produced 3 confident wrong readings. Corpus sweep tracked + tested, **reproducible from a fresh clone**; see `docs/players/BLACKBIRD.md` |
| SID Duzz' It (Gallefoss/Tjelta) → Driver 11 (Stage A) + native (Stage B) | strict onset+pitch medians: A 98.3, D 100, C 86.0, B 74.8, E 50.8, DELTA 89.8 win / 55.5 strict, V 21.8. n: A 50 B 43 C 80 D 18 E 118 DELTA 8 V 6. **Only A+D are unfitted** (C/E/DELTA/V pick a timing model best-of-N) | 🚧 **343 locate → 348 Stage A SF2s → 324 sweep-validated — three different denominators, don't conflate.** "0 failures" = emitted OK, **not** fidelity: 274/324 ship some default instrument data. ⚠️ **`$D418` passband was NEVER captured until 2026-08-13** — 15 of a 30-file sample rendered low-pass where the original selects BP/LP+BP/LP+HP/LP+BP+HP, routed 100%; builder fixed and **corpus rebuilt**: **267 of 281 select the original's passband** (7 unexercised, 2 unconfirmed, 5 failed; re-measured after the `00893cd` offset-fit fix, which superseded an earlier 258/281 — see SDI.md). The 29 unconfirmed rows are gone because the builder now records each part's window beside the artifact (`.span`, PATTERNS F8) — and resolving them **raised** the failure count 3 → 7 (now 5, after `Tanks_3000` and `Juba-Jazz` were fixed), which is the guard working: over-run manufactures disagreement, it never hides it. `Juba-Jazz` **FIXED** (52.8% → 100.0%): its filter is enabled by `$D417` routing + `$D418` alone with the **cutoff held at 0**, and `detect_filter_drives` keys on cutoff jumps, so nothing was credited — PATTERNS **F9**, a detector blind to the same event in another register. The `VE-4x` **V path is CLOSED (5/5)**: `v_traces` now records `$D418`, but half the defect was the *measurement* — siddump calls the player once per frame where a V rip's own IRQ runs `v_mult`, so it samples a different instant of a passband that alternates within the frame. `passband_check` gained a per-FILE `sdi_v` reference (Blackbird's `sim` pattern); the other 276 rows are untouched. Build outcomes unchanged (261/62/118 vs 262/62/117). Native **Stage B: `out/sdi` holds 281 built songs (2026-08-20 disk count) — not the 262 headline**, which a rebuild meant to reconcile then died in <5min/0 files written, gap still open, see SDI.md. Medians (last completed sweep, not re-verified) A/B/DELTA 99.9, E 99.7, C 98.1, V 96.8, **D 89.9 (8 of 15 <90, rescored post-walk-fix 2026-08-20 — the prior 95.9/7 predated `6aa2162` and is retracted)**. The builder prints `n`, but it is the **song length, identical on all 3 voices** (siddump holds freq through rests), so it does NOT say whether a given voice carried information. See `docs/players/SDI.md` |
| HardTrack Composer (Longhair/Brush) → Driver 11 (Stage A) + native (Stage B) | Parser vs siddump, byte-exact freq: **88.7%** where the sequencer owns the pitch, **2.6%** where the instrument's wave program does (field-5 bit 7). Register model: **100.00%** freq/wf/pulse on the 18 layout-seeded files (53,569 frames), **99.96%** on the 15 second-build files. Filter `$D416`/`$D417` **100.00%** (32,967 frames, 33 files). Stage B builds 33/33 | 🚧 RE + Stage A + Stage B (`bin/`, neither default). **Read `docs/players/HARDTRACK.md` before quoting any of this** — the numbers each carry a condition, and getting one wrong has produced published retractions here. Non-negotiables: **two columns, never averaged** (the second is the unmodelled synth engine); **always quote the match window** (it has no plateau); Stage A's 91.3% is an **artifact** above the parser — quote the 99.69% retention instead; `$D418` is 100% but **only 10 of 33 files exercise it** — quote the file count; the filter is **global, not per-voice**; Stage B needs **raw AND audible**, never net alone; the render sits at **−3** so judge any alignment against that. Tables are recovered **by code signature, never load+constant** (the last such constant, `vib_depth`, was wrong on 15 files). 5 of 38 refused (player-id ground truth); `SID/Shogoon/` is mixed-player, 38 of 150. The `native_dispatch.py` opcode-signature probe finds 39 candidates/6 refused — the extra one is `Dune_Cover`, which `player-id.exe` calls `Comer/Digi`, not HardTrack, so it is outside the 38-file population though the probe still declines it correctly (PSID init `$4000` ≠ module entry `$0900`); see HARDTRACK.md. Rungs 3 and 4 both **pass/run**; rung 4 found the `$D418` passband defect (also fixed in DMC). The **brightness gap is ONE FILE** — across 9 tunes, each measured inside its own part-1 span, ours is darker on 2 and **brighter on 7** (to +172 Hz); no corpus-wide deficit exists to close. The named decisive experiment is **DONE and SHIPPED ON**: the per-note SR lookahead (`SR_PREKILL`) models the player's own hard restart, selected by instrument **field-5 mode 2** on the note that is **ENDING** (disassembly-confirmed; 242/242, 158/158, 375/375 note-ons predicted, 0 false positives in 1,260). Corpus **12,397 → 1,794** mismatching `$D406` frames over 33 files, **worse on 0**; corpus rebuilt (33/33, 313 parts; passband then 25/33, now **32/33** once each file is measured over its own part-1 span — see PATTERNS F8). Gate it on the ENDING note or `Teekkno` goes 42 → 468. **It is a correctness fix, not a brightness one** — mean |centroid| error 67→51 Hz but `Love_tune_2` reads **darker** (−57→−103 Hz); its release-tail level error halves (+1.26→−0.42 dB). A/B: `pyscript/hardtrack_sr_prekill_ab.py`; rebuild: `pyscript/hardtrack_native_rebuild.py`. **The `$D418` passband fix is CORRECTNESS, not audio** — an 8-tune A/B refutes "missing passband = darker" (ours was *brighter* on 6 of 8, rolloff worse on 6 of 8). Check artifacts with `pyscript/passband_check.py`, **not** the builder's report: 31 of 33 shipped builds predated the fix while this row quoted the builder | 
