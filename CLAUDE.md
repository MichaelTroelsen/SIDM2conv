...
| Mainstream MoN / Jeroen Tel (SID/Tel_Jeroen, 179 files) → mon_parser | B1-indirect bucket: 6/24 onset-EXACT (05-09-87, Ikari_Union, Lost_in_China, Scout, Trying_Out_2_v1, Trying_Out_3); Alloyrun 61%, Starball 76% (up from ~1%) | 🚧 NEW MoN GENERATION cracked (ab99032): `_locate_b1` + `_voice_blocks_tel`. Same-day follow-up `_locate_b1_row_variant`: orderlist REPEAT op + pattern-index off-by-one + classic `_pattern()`-grammar reuse, all signature-gated (disassembly-confirmed, zero change to the other 21 bucket files). Middle tier (60-88%) + Alloyrun's own residual = instrument arp/legato (Stage-B); remaining broken files are each their own compiled micro-variant (Bantam ≠ Alloyrun, confirmed) TODO; 85-file no-copy bucket untouched. See `memory/mainstream-mon-tel.md` |
