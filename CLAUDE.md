...
| Blackbird / lft → native driver | 16-file v1.2-exact corpus **mean 99.42** register-accuracy vs the validated simulator; **8 files at exactly 100.0**, none below 94.2. Glyptodont **162/162 note-ons** (0 missing, 0 spurious), arming 100% on every file | ✅ B1-B25 + E3e/E3f/E3g (`bin/`, not default); **not wired into `DriverSelector`**. E3f combo fx values were briefly disabled on a since-**falsified** SF2II-crash report (`BB_NO_COMBO=1` disables); laggards `To_Die_For_II` 94.2 + `Into_the_Unknown` 98.1 are freq/pulse/filter, not envelope. Corpus sweep is **untracked scratch**, not reproducible from a fresh clone; see `docs/players/BLACKBIRD.md` |
