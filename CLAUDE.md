...
| Mainstream MoN / Jeroen Tel (SID/Tel_Jeroen, 179 files) → mon_parser | B1-indirect bucket: **12/24 onset-EXACT** (was 6): 05-09-87, Ikari_Union, Lost_in_China, Scout, Trying_Out_2_v1, Trying_Out_3, Happy_JT, Beginning, Beginning_v2, DemoSong, Reggae_Example, Chrome_Met1. Plus Orion_Intro 92%, Trying_Out 78%, Zynon_Zak 84%, Bantam 77%, Starball 76%, Alloyrun 61%, Tel_1 25% | 🚧 NEW MoN GENERATION cracked (ab99032): `_locate_b1` + `_voice_blocks_tel`. Same-day `_locate_b1_row_variant` + `_locate_tel_instr_fields`, 4 rounds: orderlist REPEAT op + pattern-index off-by-one + classic `_pattern()`-grammar reuse (Alloyrun/Starball) → row-ctrl off-by-one (Bantam) → **row length-mask hardcoded `$1F`, real per-file masks vary `$3F`/`$7F`** (5 "Stage-B middle tier" files were actually this bug) → **`_silent_instr` assumed row-major instrument tables, several compiles are column-major** (bonus win Chrome_Met1, wasn't even the target). All signature-gated + disassembly-confirmed; one round caused a real regression mid-session (over-loose proximity match), caught + fixed with a stride-consistency check before shipping — verify against the FULL bucket, not just the target file. Remaining: Tel_1 (partially fixed, 2nd bug open), Monitor_Madness_1/2, Trying_Out_2; 85-file no-copy bucket untouched. See `memory/mainstream-mon-tel.md` |
