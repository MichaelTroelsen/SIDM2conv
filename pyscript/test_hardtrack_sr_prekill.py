...
must hold for that experiment to be safe to leave in the tree:

  * it ships OFF, so no existing artifact changes;
  * every byte of it is inside a `.if SR_PREKILL` block, which is why builds for
    the other six players that share this driver came out byte-identical;
  * it refuses to assemble against `RELEASE_WF`, whose gate it cannot read.

The last one matters more than it looks. `sr_pre` must not fire while a voice is
still SOUNDING: `SR=$00` zeroes SUSTAIN as well as release, the envelope falls to
zero, and only a gate RISE re-attacks it -- so one mistimed kill silences the
rest of a held note while every later register still reads correct. Four such
frames cost -24 dB on `Love_tune_2`, and the register comparison could not see it.
"""
import io
import os
import re
...
def test_the_flag_ships_off():
    """A build that does not ask for it must be byte-identical to before."""
    assert "SR_PREKILL = {getattr(B, 'SR_PREKILL', 0)}" in read(EMITTER)
    assert "os.environ.get('HT_SR_PREKILL', '0')" in read(SHIM)
