...
def gen_includes():
    ...
    with open(os.path.join(BB, "layout.inc"), "w") as f:
        ...
        f.write("WAVE_INIT_ROW = 0\n")        # B8 default wave program
        # Stage A's pitch calibration (note_index + 9), user-audio-confirmed.
        # Hardcoded rather than imported: the source of truth is
        # build_blackbird_native_song.SF2_NOTE_OFS, but that module imports
        # THIS one, so importing it back would be circular.
        f.write("NOTE_OFS = 9\n")
        # B10 fx/pitch engine tables. The skeleton has no song, so these just
        # need to be well-formed addresses inside the edit area for the
        ...
        # B7/B9/B10 boundary-priming constants. The real per-song values come
        # from build_blackbird_native_song.py's _write_prime_consts(); the
        # skeleton self-test has no song and therefore no boundary to resume
        # from, so it emits the COLD-START defaults (what part 1 of any song
        # uses, and what do_init hardcoded before B7 existed): everything 0
        # except the gate mask, which starts gated-off at $fe.
        #
        # These were missing entirely until now, which silently broke this
        # self-test from B7 onward -- `py -3 bin/build_blackbird_driver_full.py`
        # died with "not defined symbol 'PRIME_VIFXR1'" and friends. The song
        # builds were unaffected (they generate their own complete layout.inc),
        # so only this standalone developer entry point -- the one named in
        # blackbird_driver.asm's own header comment -- was broken.
        for v in range(3):
            for fld in ("VWF", "PW_ACC", "D402", "FXPOS", "BASEPITCH",
                        "VIFXS", "VIFXR", "VWI", "VIWAVE", "AD", "SR",
                        "FLAGS", "VIFILT"):
                f.write(f"PRIME_{fld}{v} = 0\n")
            f.write(f"PRIME_VGMASK{v} = $fe\n")   # start gated off
        for fld in ("F_IDX", "F_CNT", "F_ADHI", "F_CLO", "F_CHI", "F_MODE",
                    "D417"):
            f.write(f"PRIME_{fld} = 0\n")
    ...
    # (B10 deleted the freqtable.inc write that used to live here. The FM
    # engine it fed is gone -- the driver now computes frequency directly from
    # Blackbird's own freq_lsb/freq_msb via FREQBLOB, so a generic PAL
    # note->freq table is no longer included by blackbird_driver.asm at all.
    # Leaving the write in place regenerated an orphan, untracked
    # drivers_src/blackbird/freqtable.inc on every run.)
...
def main():
    ...
    # B10 NOTE -- this pitch assertion is no longer meaningful for the SKELETON
    # and is reported informationally rather than asserted.
    #
    # Before B10 the driver derived pitch from a generic note->PAL-freq table,
    # so a bare skeleton with no song still produced the expected frequencies.
    # B10 replaced that with a port of lft's own fx interpolator: pitch is now
    # computed from FXTAB + FREQBLOB, and `fxtable[fxpos] == 0` is Blackbird's
    # genuine `fixedfreq` sentinel meaning "write $FFFF" (player.s ~line 240).
    # The skeleton has no song and therefore no fx program, so FXTAB reads 0
    # and every voice correctly emits $FFFF. That is the driver behaving as
    # designed, not a regression -- but it means this check can no longer
    # distinguish a working driver from a broken one.
    #
    # Deliberately NOT faked into a pass: the comparison is still printed so
    # the values are visible, and the headline is honest about what was and
    # was not verified. Real pitch fidelity IS covered, far more strictly,
    # by build_blackbird_native_song.py's verify_fx_engine() (300k-700k exact
    # comparisons against the validated simulator per build) and by its
    # register-trace comparison. Restoring a genuine assertion here would mean
    # emitting a minimal real fx program into the skeleton's edit area and
    # deriving its expected output through the same 4-mode blend -- worth
    # doing, but it is a new self-test, not a one-line fix.
    print("  row:  V0 freq  V1 freq  V2 freq    (pre-B10 note-table expectation)")
    ...
    all_sentinel = all(rows[r][v] == 0xFFFF for r in range(N_ROWS) for v in range(3))
    print("ASSEMBLE + RUN OK (skeleton self-test; pitch check N/A post-B10 -- "
          + ("all voices at the expected $FFFF fixedfreq sentinel, no fx program"
             if all_sentinel else
             "UNEXPECTED: not all voices are at the $FFFF sentinel, investigate")
          + ")")
